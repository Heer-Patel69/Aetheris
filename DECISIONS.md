# UniVoid Brain OS — Architecture Decisions
...
|---|---|---|---|---|---|
| D-001 | Source lives at `c:\AI\Agency owner\brain-os\` | Keeps Brain OS alongside the Agency roster it orchestrates | Dedicated `c:\AI\brain-os\`; home dir `~\.univoid\brain-os\` | Couples source to agency-agents workspace | APPROVED |
...
| D-004 | Separate `univoid-*` skill names (not one monolithic skill) | Each module has distinct trigger surface; prevents loading entire runtime | Single `univoid-brain-os` skill with deep references | More files to manage; more install complexity | APPROVED |
...
| ADR-006 | Skill Decomposition | Decomposing runtime into 8 distinct global skills | Monolithic `univoid-brain-os` skill; local configs | Minimizes loaded context, allows stage-level updates | APPROVED |
