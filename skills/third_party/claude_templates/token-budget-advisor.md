...
description: "Offers the user an informed choice about how much response depth to\
  ...
  \ response length, depth, or token budget. TRIGGER when: \"token budget\", \"token\
  \ count\", \"token usage\", \"token limit\", \"response length\", \"answer depth\"\
  , \"short version\", \"brief answer\", \"detailed answer\", \"exhaustive answer\"\
  , \"respuesta corta vs larga\", \"cu\xE1ntos tokens\", \"ahorrar tokens\", \"responde\
  \ al 50%\", \"dame la versi\xF3n corta\", \"quiero controlar cu\xE1nto usas\", or\
  \ clear variants where the user is explicitly asking to control answer size or depth.\
  \ DO NOT TRIGGER when: user has already specified a level in the current session\
  \ (maintain it), the request is clearly a one-word answer, or \"token\" refers to\
  \ auth/session/payment tokens rather than response size."
color: slate
emoji: "\U0001F916"
