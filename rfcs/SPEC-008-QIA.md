# SPEC-008: Query And Impact Analysis Engine (QIA)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-001
Layer: Query Intelligence
Scope: Natural-language engineering query and impact traversal
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/qia.py` (validated from source metadata)
Primary Class: `QueryAndImpactAnalysisEngine`
Test Reference: `tests/test_qia.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Query And Impact Analysis Engine exists to make Query Intelligence deterministic, reviewable, and safe at production scale. Answers engineering questions by resolving query intent and traversing the engineering graph to report impacted nodes and dependent artifacts.

This document upgrades SPEC-008 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-008 is part of the Engineering Knowledge Compiler, where evidence is discovered, normalized, stored, and queried before planning begins.

======================================================================
2. PURPOSE
======================================================================
The purpose of QIA is to own the full engineering responsibility for natural-language engineering query and impact traversal. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding QIA behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct natural-language engineering query and impact traversal was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for natural-language engineering query and impact traversal.
- Accept only validated upstream artifacts from SPEC-007.
- Emit structured output that can be consumed by SPEC-009 EDPE.
...
======================================================================
In scope:
- Natural-language engineering query and impact traversal
- Validation of all QIA input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-008` input contract to `SPEC-008` output contract.
- Use `src/intelligence/qia.py` as the implementation boundary and `QueryAndImpactAnalysisEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
QIA follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- QIA validates and transforms that evidence.
...
======================================================================
- FR-001: QIA shall load the upstream artifacts declared by SPEC-007.
- FR-002: QIA shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: QIA shall execute the primary behavior owned by `QueryAndImpactAnalysisEngine`.
- FR-004: QIA shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-008 sits in `Query Intelligence` under `RFC-001`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat QIA output as the authoritative contract for natural-language engineering query and impact traversal unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/qia.py`.

Primary implementation class: `QueryAndImpactAnalysisEngine`.
...
======================================================================
QIA is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-008` owns how the engine is implemented.
...
QIA must exchange data by contract, not by implicit global state.
...
4. Execute `QueryAndImpactAnalysisEngine` behavior for natural-language engineering query and impact traversal.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-007.
...
- `QueryAndImpactAnalysisEngine`: Owns the core natural-language engineering query and impact traversal behavior.
...
- Input interface: `QIAInput` JSON object.
- Output interface: `QIAOutput` JSON object.
- Source interface: `src/intelligence/qia.py`.
- Public class boundary: `QueryAndImpactAnalysisEngine`.
...
|---|---|---|
| `query(query_text: str) -> dict` | Executes the `query` responsibility for QIA. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `analyze_impact(node_id: str) -> dict` | Executes the `analyze_impact` responsibility for QIA. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-008-example",
  "spec_id": "SPEC-008",
  ...
  "payload": {
    "engine": "Query And Impact Analysis Engine",
    "source_class": "QueryAndImpactAnalysisEngine",
    "public_methods": [
      "query(query_text: str) -> dict",
      "analyze_impact(node_id: str) -> dict"
    ]
  }
}
```
...
- `AETHERIS_QIA_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_QIA_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_QIA_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. QIA may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `QIA_STARTED` with request ID, spec ID, source path, and upstream references.
- `QIA_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `QIA_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- QIA must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
QIA is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. QIA should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_qia.py`.
...
======================================================================
- Unit tests should instantiate `QueryAndImpactAnalysisEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-007` | Upstream | Must provide validated evidence before QIA starts. |
| `SPEC-009 EDPE` | Downstream | Must consume only the validated QIA output contract. |
| `src/intelligence/qia.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_qia.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_qia.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `QueryAndImpactAnalysisEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: QIA grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/qia.py
tests/test_qia.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-008-QIA.md
```

Guidance:
- Keep `QueryAndImpactAnalysisEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_qia(payload: dict) -> dict:
    input_contract = validate_qia_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = QueryAndImpactAnalysisEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_qia_output(input_contract, result)
    validate_qia_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/qia.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-007"] --> I["Input Validator"]
    I --> E["SPEC-008 QIA: Query And Impact Analysis Engine"]
...
graph LR
    ...
    V --> Core["QueryAndImpactAnalysisEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["QIA Payload"]
...
```mermaid
graph TD
    Parent["RFC-001"] --> This["SPEC-008"]
    This --> Source["src/intelligence/qia.py"]
    This --> Test["tests/test_qia.py"]
    This --> Next["SPEC-009 EDPE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute QIA"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-007" as U
rectangle "Input Validator" as V
rectangle "SPEC-008 QIA" as E
database "EKB / Artifacts" as K
rectangle "SPEC-009 EDPE" as D
U --> V
V --> E
E --> K
K --> D
...
class "QIAInputContract"
class "QIAOutputContract"
QueryAndImpactAnalysisEngine --> "QIAInputContract"
QueryAndImpactAnalysisEngine --> "QIAOutputContract"
@enduml
```
