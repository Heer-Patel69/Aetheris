# SPEC-003: Product Discovery Engine (PDE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-001
Layer: Product Intelligence
Scope: Product domain, personas, flows, and complexity discovery
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/pde.py` (validated from source metadata)
Primary Class: `ProductDiscoveryEngine`
Test Reference: `tests/test_pde.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Product Discovery Engine exists to make Product Intelligence deterministic, reviewable, and safe at production scale. Merges requirement and workspace evidence into a product plan containing domain classification, personas, user journeys, core flows, and complexity estimates.

This document upgrades SPEC-003 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-003 is part of the Engineering Knowledge Compiler, where evidence is discovered, normalized, stored, and queried before planning begins.

======================================================================
2. PURPOSE
======================================================================
The purpose of PDE is to own the full engineering responsibility for product domain, personas, flows, and complexity discovery. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PDE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct product domain, personas, flows, and complexity discovery was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for product domain, personas, flows, and complexity discovery.
- Accept only validated upstream artifacts from SPEC-002.
- Emit structured output that can be consumed by SPEC-004.
...
======================================================================
In scope:
- Product domain, personas, flows, and complexity discovery
- Validation of all PDE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-003` input contract to `SPEC-003` output contract.
- Use `src/intelligence/pde.py` as the implementation boundary and `ProductDiscoveryEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PDE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PDE validates and transforms that evidence.
...
======================================================================
- FR-001: PDE shall load the upstream artifacts declared by SPEC-002.
- FR-002: PDE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PDE shall execute the primary behavior owned by `ProductDiscoveryEngine`.
- FR-004: PDE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-003 sits in `Product Intelligence` under `RFC-001`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PDE output as the authoritative contract for product domain, personas, flows, and complexity discovery unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/pde.py`.

Primary implementation class: `ProductDiscoveryEngine`.
...
======================================================================
PDE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-003` owns how the engine is implemented.
...
PDE must exchange data by contract, not by implicit global state.
...
4. Execute `ProductDiscoveryEngine` behavior for product domain, personas, flows, and complexity discovery.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-002.
...
- `ProductDiscoveryEngine`: Owns the core product domain, personas, flows, and complexity discovery behavior.
...
- Input interface: `PDEInput` JSON object.
- Output interface: `PDEOutput` JSON object.
- Source interface: `src/intelligence/pde.py`.
- Public class boundary: `ProductDiscoveryEngine`.
...
|---|---|---|
| `discover(requirement: dict, wde_inventories: Dict[str, dict]) -> Dict[str, any]` | Executes the `discover` responsibility for PDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-003-example",
  "spec_id": "SPEC-003",
  ...
  "payload": {
    "engine": "Product Discovery Engine",
    "source_class": "ProductDiscoveryEngine",
    "public_methods": [
      "discover(requirement: dict, wde_inventories: Dict[str, dict]) -> Dict[str, any]"
    ]
  }
}
```
...
- `AETHERIS_PDE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PDE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PDE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. PDE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PDE_STARTED` with request ID, spec ID, source path, and upstream references.
- `PDE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PDE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PDE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PDE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PDE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_pde.py`.
...
======================================================================
- Unit tests should instantiate `ProductDiscoveryEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-002` | Upstream | Must provide validated evidence before PDE starts. |
| `SPEC-004` | Downstream | Must consume only the validated PDE output contract. |
| `src/intelligence/pde.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_pde.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_pde.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `ProductDiscoveryEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PDE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/pde.py
tests/test_pde.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-003-PDE.md
```

Guidance:
- Keep `ProductDiscoveryEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_pde(payload: dict) -> dict:
    input_contract = validate_pde_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ProductDiscoveryEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_pde_output(input_contract, result)
    validate_pde_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/pde.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-002"] --> I["Input Validator"]
    I --> E["SPEC-003 PDE: Product Discovery Engine"]
...
graph LR
    ...
    V --> Core["ProductDiscoveryEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PDE Payload"]
...
```mermaid
graph TD
    Parent["RFC-001"] --> This["SPEC-003"]
    This --> Source["src/intelligence/pde.py"]
    This --> Test["tests/test_pde.py"]
    This --> Next["SPEC-004"]
```
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PDE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-002" as U
rectangle "Input Validator" as V
rectangle "SPEC-003 PDE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-004" as D
U --> V
V --> E
E --> K
K --> D
...
class "PDEInputContract"
class "PDEOutputContract"
ProductDiscoveryEngine --> "PDEInputContract"
ProductDiscoveryEngine --> "PDEOutputContract"
@enduml
```
