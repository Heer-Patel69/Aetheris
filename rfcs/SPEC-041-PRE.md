# SPEC-041: Patch Recovery Engine (PRE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Failure classification and patch recovery
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/pre.py` (validated from source metadata)
Primary Class: `PatchRecoveryEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Patch Recovery Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Classifies failures, analyzes root causes, builds patch plans, applies corrections, rolls back unsafe changes, retries, and resumes execution.

This document upgrades SPEC-041 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-041 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of PRE is to own the full engineering responsibility for failure classification and patch recovery. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PRE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct failure classification and patch recovery was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for failure classification and patch recovery.
- Accept only validated upstream artifacts from SPEC-040.
- Emit structured output that can be consumed by SPEC-042.
...
======================================================================
In scope:
- Failure classification and patch recovery
- Validation of all PRE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-041` input contract to `SPEC-041` output contract.
- Use `src/execution/pre.py` as the implementation boundary and `PatchRecoveryEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PRE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PRE validates and transforms that evidence.
...
======================================================================
- FR-001: PRE shall load the upstream artifacts declared by SPEC-040.
- FR-002: PRE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PRE shall execute the primary behavior owned by `PatchRecoveryEngine`.
- FR-004: PRE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-041 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PRE output as the authoritative contract for failure classification and patch recovery unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/pre.py`.

Primary implementation class: `PatchRecoveryEngine`.
...
======================================================================
PRE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-041` owns how the engine is implemented.
...
PRE must exchange data by contract, not by implicit global state.
...
4. Execute `PatchRecoveryEngine` behavior for failure classification and patch recovery.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-040.
...
- `PatchRecoveryEngine`: Owns the core failure classification and patch recovery behavior.
...
- Input interface: `PREInput` JSON object.
- Output interface: `PREOutput` JSON object.
- Source interface: `src/execution/pre.py`.
- Public class boundary: `PatchRecoveryEngine`.
...
|---|---|---|
| `recover(failure: dict) -> dict` | Executes the `recover` responsibility for PRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `patch(file_path: str, correction: str) -> bool` | Executes the `patch` responsibility for PRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `rollback(file_path: str) -> bool` | Executes the `rollback` responsibility for PRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `retry(task_id: str) -> dict` | Executes the `retry` responsibility for PRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `resume() -> dict` | Executes the `resume` responsibility for PRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `classify_failure(error_msg: str) -> str` | Executes the `classify_failure` responsibility for PRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `analyze_root_cause(error_msg: str) -> str` | Executes the `analyze_root_cause` responsibility for PRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `validate_patch(patch_plan: dict) -> bool` | Executes the `validate_patch` responsibility for PRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-041-example",
  "spec_id": "SPEC-041",
  ...
  "payload": {
    "engine": "Patch Recovery Engine",
    "source_class": "PatchRecoveryEngine",
    "public_methods": [
      "recover(failure: dict) -> dict",
      "patch(file_path: str, correction: str) -> bool",
      "rollback(file_path: str) -> bool",
...
- `AETHERIS_PRE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PRE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PRE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. PRE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PRE_STARTED` with request ID, spec ID, source path, and upstream references.
- `PRE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PRE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PRE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PRE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PRE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `PatchRecoveryEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-040` | Upstream | Must provide validated evidence before PRE starts. |
| `SPEC-042` | Downstream | Must consume only the validated PRE output contract. |
| `src/execution/pre.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `PatchRecoveryEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PRE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-041-PRE.md
```

Guidance:
- Keep `PatchRecoveryEngine` as the primary orchestration class for this SPEC.
...
def run_pre(payload: dict) -> dict:
    input_contract = validate_pre_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = PatchRecoveryEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_pre_output(input_contract, result)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/pre.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-040"] --> I["Input Validator"]
    I --> E["SPEC-041 PRE: Patch Recovery Engine"]
...
graph LR
    ...
    V --> Core["PatchRecoveryEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PRE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-041"]
    This --> Source["src/execution/pre.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PRE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-040" as U
rectangle "Input Validator" as V
rectangle "SPEC-041 PRE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-042" as D
U --> V
V --> E
E --> K
K --> D
...
class "PREInputContract"
class "PREOutputContract"
PatchRecoveryEngine --> "PREInputContract"
PatchRecoveryEngine --> "PREOutputContract"
@enduml
```
