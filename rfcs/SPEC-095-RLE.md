# SPEC-095: Recovery Learning Engine (RLE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Recovery strategy learning and resilience improvement
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/recovery_learning.py` (source implementation pending)
Primary Class: `RecoveryLearningEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Recovery Learning Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Learns from patch plans, rollback decisions, checkpoint restores, incident records, and recovery outcomes to improve future resilience behavior.

This document upgrades SPEC-095 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-095 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of RLE is to own the full engineering responsibility for recovery strategy learning and resilience improvement. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding RLE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct recovery strategy learning and resilience improvement was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for recovery strategy learning and resilience improvement.
- Accept only validated upstream artifacts from SPEC-041 PRE, SPEC-042 SPE, SPEC-089 FKE.
- Emit structured output that can be consumed by SPEC-041 PRE, SPEC-046 EO.
...
======================================================================
In scope:
- Recovery strategy learning and resilience improvement
- Validation of all RLE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-095` input contract to `SPEC-095` output contract.
- Use `src/learning/recovery_learning.py` as the implementation boundary and `RecoveryLearningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
RLE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- RLE validates and transforms that evidence.
...
======================================================================
- FR-001: RLE shall load the upstream artifacts declared by SPEC-041 PRE, SPEC-042 SPE, SPEC-089 FKE.
- FR-002: RLE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: RLE shall execute the primary behavior owned by `RecoveryLearningEngine`.
- FR-004: RLE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-095 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat RLE output as the authoritative contract for recovery strategy learning and resilience improvement unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/recovery_learning.py`.

Primary implementation class: `RecoveryLearningEngine`.
...
======================================================================
RLE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-095` owns how the engine is implemented.
...
RLE must exchange data by contract, not by implicit global state.
...
4. Execute `RecoveryLearningEngine` behavior for recovery strategy learning and resilience improvement.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-041 PRE, SPEC-042 SPE, SPEC-089 FKE.
...
- `RecoveryLearningEngine`: Owns the core recovery strategy learning and resilience improvement behavior.
...
- Learns from patch plans, rollback decisions, checkpoint restores, incident records, and recovery outcomes to improve future resilience behavior.
...
- Input interface: `RLEInput` JSON object.
- Output interface: `RLEOutput` JSON object.
- Source interface: `src/learning/recovery_learning.py`.
- Public class boundary: `RecoveryLearningEngine`.
...
|---|---|---|
| `RecoveryLearningEngine.run(payload: dict) -> dict` | Executes the `RecoveryLearningEngine.run` responsibility for RLE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/recovery_learning.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-095-example",
  "spec_id": "SPEC-095",
  ...
  "payload": {
    "engine": "Recovery Learning Engine",
    "source_class": "RecoveryLearningEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_RLE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_RLE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_RLE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/recovery_learning.py` implementation is primarily in-memory. The enterprise contract still requires RLE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. RLE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `RLE_STARTED` with request ID, spec ID, source path, and upstream references.
- `RLE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `RLE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- RLE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
RLE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. RLE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `RecoveryLearningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-041 PRE` | Upstream | Must provide validated evidence before RLE starts. |
| `SPEC-042 SPE` | Upstream | Must provide validated evidence before RLE starts. |
| `SPEC-089 FKE` | Upstream | Must provide validated evidence before RLE starts. |
| `SPEC-041 PRE` | Downstream | Must consume only the validated RLE output contract. |
| `SPEC-046 EO` | Downstream | Must consume only the validated RLE output contract. |
| `src/learning/recovery_learning.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `RecoveryLearningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: RLE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/recovery_learning.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-095-RLE.md
```

Guidance:
- Keep `RecoveryLearningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_rle(payload: dict) -> dict:
    input_contract = validate_rle_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = RecoveryLearningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_rle_output(input_contract, result)
    validate_rle_output(output)
...
- `RecoveryLearningEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/recovery_learning.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-041 PRE"] --> I["Input Validator"]
    I --> E["SPEC-095 RLE: Recovery Learning Engine"]
...
graph LR
    ...
    V --> Core["RecoveryLearningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["RLE Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-095"]
    This --> Source["src/learning/recovery_learning.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-041 PRE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute RLE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-041 PRE" as U
rectangle "Input Validator" as V
rectangle "SPEC-095 RLE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-041 PRE" as D
U --> V
V --> E
E --> K
K --> D
...
class "RLEInputContract"
class "RLEOutputContract"
RecoveryLearningEngine --> "RLEInputContract"
RecoveryLearningEngine --> "RLEOutputContract"
@enduml
```
