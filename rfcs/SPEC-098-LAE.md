# SPEC-098: Learning Analytics Engine (LAE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Learning metrics, dashboards, and effectiveness analysis
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/learning_analytics.py` (source implementation pending)
Primary Class: `LearningAnalyticsEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Learning Analytics Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Measures whether learning improves quality, cost, reliability, delivery speed, recovery success, prompt quality, skill routing, architecture choices, and production readiness.

This document upgrades SPEC-098 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-098 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of LAE is to own the full engineering responsibility for learning metrics, dashboards, and effectiveness analysis. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding LAE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct learning metrics, dashboards, and effectiveness analysis was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for learning metrics, dashboards, and effectiveness analysis.
- Accept only validated upstream artifacts from SPEC-086 through SPEC-097.
- Emit structured output that can be consumed by SPEC-099 AFE, SPEC-100 LSO.
...
======================================================================
In scope:
- Learning metrics, dashboards, and effectiveness analysis
- Validation of all LAE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-098` input contract to `SPEC-098` output contract.
- Use `src/learning/learning_analytics.py` as the implementation boundary and `LearningAnalyticsEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
LAE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- LAE validates and transforms that evidence.
...
======================================================================
- FR-001: LAE shall load the upstream artifacts declared by SPEC-086 through SPEC-097.
- FR-002: LAE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: LAE shall execute the primary behavior owned by `LearningAnalyticsEngine`.
- FR-004: LAE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-098 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.

Upstream dependencies:
- SPEC-086 through SPEC-097

Downstream consumers:
- SPEC-099 AFE
- SPEC-100 LSO

The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat LAE output as the authoritative contract for learning metrics, dashboards, and effectiveness analysis unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/learning_analytics.py`.

Primary implementation class: `LearningAnalyticsEngine`.
...
======================================================================
LAE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-098` owns how the engine is implemented.
...
LAE must exchange data by contract, not by implicit global state.
...
4. Execute `LearningAnalyticsEngine` behavior for learning metrics, dashboards, and effectiveness analysis.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-086 through SPEC-097.
...
- `LearningAnalyticsEngine`: Owns the core learning metrics, dashboards, and effectiveness analysis behavior.
...
- Input interface: `LAEInput` JSON object.
- Output interface: `LAEOutput` JSON object.
- Source interface: `src/learning/learning_analytics.py`.
- Public class boundary: `LearningAnalyticsEngine`.
...
|---|---|---|
| `LearningAnalyticsEngine.run(payload: dict) -> dict` | Executes the `LearningAnalyticsEngine.run` responsibility for LAE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/learning_analytics.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-098-example",
  "spec_id": "SPEC-098",
  ...
  "payload": {
    "engine": "Learning Analytics Engine",
    "source_class": "LearningAnalyticsEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_LAE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_LAE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_LAE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/learning_analytics.py` implementation is primarily in-memory. The enterprise contract still requires LAE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. LAE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `LAE_STARTED` with request ID, spec ID, source path, and upstream references.
- `LAE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `LAE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- LAE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
LAE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. LAE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `LearningAnalyticsEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-086 through SPEC-097` | Upstream | Must provide validated evidence before LAE starts. |
| `SPEC-099 AFE` | Downstream | Must consume only the validated LAE output contract. |
| `SPEC-100 LSO` | Downstream | Must consume only the validated LAE output contract. |
| `src/learning/learning_analytics.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `LearningAnalyticsEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: LAE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-098-LAE.md
```

Guidance:
- Keep `LearningAnalyticsEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_lae(payload: dict) -> dict:
    input_contract = validate_lae_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = LearningAnalyticsEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_lae_output(input_contract, result)
    validate_lae_output(output)
...
- `LearningAnalyticsEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/learning_analytics.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-086 through SPEC-097"] --> I["Input Validator"]
    I --> E["SPEC-098 LAE: Learning Analytics Engine"]
...
graph LR
    ...
    V --> Core["LearningAnalyticsEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["LAE Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-098"]
    This --> Source["src/learning/learning_analytics.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-099 AFE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute LAE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-086 through SPEC-097" as U
rectangle "Input Validator" as V
rectangle "SPEC-098 LAE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-099 AFE" as D
U --> V
V --> E
E --> K
K --> D
...
class "LAEInputContract"
class "LAEOutputContract"
LearningAnalyticsEngine --> "LAEInputContract"
LearningAnalyticsEngine --> "LAEOutputContract"
@enduml
```
