# SPEC-088: Best Practice Extraction Engine (BPE2)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Validated best-practice extraction from historical outcomes
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/best_practice_extraction.py` (source implementation pending)
Primary Class: `BestPracticeExtractionEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Best Practice Extraction Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Extracts durable engineering practices from high-confidence success patterns and converts them into reusable guidance for planning, execution, verification, and production operations.

This document upgrades SPEC-088 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-088 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of BPE2 is to own the full engineering responsibility for validated best-practice extraction from historical outcomes. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding BPE2 behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct validated best-practice extraction from historical outcomes was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for validated best-practice extraction from historical outcomes.
- Accept only validated upstream artifacts from SPEC-087 PME, SPEC-090 SKE, SPEC-055 FVE.
- Emit structured output that can be consumed by SPEC-091 PRE2, SPEC-092 SLE, SPEC-096 ARSU.
...
======================================================================
In scope:
- Validated best-practice extraction from historical outcomes
- Validation of all BPE2 input and output structures.
...
- Use `src/learning/best_practice_extraction.py` as the implementation boundary and `BestPracticeExtractionEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
BPE2 follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- BPE2 validates and transforms that evidence.
...
======================================================================
- FR-001: BPE2 shall load the upstream artifacts declared by SPEC-087 PME, SPEC-090 SKE, SPEC-055 FVE.
- FR-002: BPE2 shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: BPE2 shall execute the primary behavior owned by `BestPracticeExtractionEngine`.
- FR-004: BPE2 shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-088 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat BPE2 output as the authoritative contract for validated best-practice extraction from historical outcomes unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/best_practice_extraction.py`.

Primary implementation class: `BestPracticeExtractionEngine`.
...
======================================================================
BPE2 is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-088` owns how the engine is implemented.
...
BPE2 must exchange data by contract, not by implicit global state.
...
4. Execute `BestPracticeExtractionEngine` behavior for validated best-practice extraction from historical outcomes.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-087 PME, SPEC-090 SKE, SPEC-055 FVE.
...
4. Run the BPE2 core transformation.
5. Generate artifacts and EKB records.
6. Validate output against the JSON schema in this SPEC.
7. Release the output to SPEC-091 PRE2, SPEC-092 SLE, SPEC-096 ARSU.
...
- `BestPracticeExtractionEngine`: Owns the core validated best-practice extraction from historical outcomes behavior.
...
- Extracts durable engineering practices from high-confidence success patterns and converts them into reusable guidance for planning, execution, verification, and production operations.
...
- Input interface: `BPE2Input` JSON object.
- Output interface: `BPE2Output` JSON object.
- Source interface: `src/learning/best_practice_extraction.py`.
- Public class boundary: `BestPracticeExtractionEngine`.
...
|---|---|---|
| `BestPracticeExtractionEngine.run(payload: dict) -> dict` | Executes the `BestPracticeExtractionEngine.run` responsibility for BPE2. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/best_practice_extraction.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-088-example",
  "spec_id": "SPEC-088",
  ...
  "payload": {
    "engine": "Best Practice Extraction Engine",
    "source_class": "BestPracticeExtractionEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_BPE2_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_BPE2_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_BPE2_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/best_practice_extraction.py` implementation is primarily in-memory. The enterprise contract still requires BPE2 to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. BPE2 may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `BPE2_STARTED` with request ID, spec ID, source path, and upstream references.
- `BPE2_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `BPE2_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- BPE2 must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
BPE2 is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. BPE2 should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `BestPracticeExtractionEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-087 PME` | Upstream | Must provide validated evidence before BPE2 starts. |
| `SPEC-090 SKE` | Upstream | Must provide validated evidence before BPE2 starts. |
| `SPEC-055 FVE` | Upstream | Must provide validated evidence before BPE2 starts. |
| `SPEC-091 PRE2` | Downstream | Must consume only the validated BPE2 output contract. |
| `SPEC-092 SLE` | Downstream | Must consume only the validated BPE2 output contract. |
| `SPEC-096 ARSU` | Downstream | Must consume only the validated BPE2 output contract. |
| `src/learning/best_practice_extraction.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `BestPracticeExtractionEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: BPE2 grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/best_practice_extraction.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-088-BPE2.md
```

Guidance:
- Keep `BestPracticeExtractionEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_bpe2(payload: dict) -> dict:
    input_contract = validate_bpe2_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = BestPracticeExtractionEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_bpe2_output(input_contract, result)
    validate_bpe2_output(output)
...
- `BestPracticeExtractionEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/best_practice_extraction.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-087 PME"] --> I["Input Validator"]
    I --> E["SPEC-088 BPE2: Best Practice Extraction Engine"]
...
graph LR
    ...
    V --> Core["BestPracticeExtractionEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["BPE2 Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-088"]
    This --> Source["src/learning/best_practice_extraction.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-091 PRE2"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute BPE2"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-087 PME" as U
rectangle "Input Validator" as V
rectangle "SPEC-088 BPE2" as E
database "EKB / Artifacts" as K
rectangle "SPEC-091 PRE2" as D
U --> V
V --> E
E --> K
K --> D
...
```plantuml
@startuml
class BestPracticeExtractionEngine {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "BPE2InputContract"
class "BPE2OutputContract"
BestPracticeExtractionEngine --> "BPE2InputContract"
BestPracticeExtractionEngine --> "BPE2OutputContract"
@enduml
```
