# SPEC-052: Long Context Engine (LCE)
...
Implementation: `src/intelligence/lce.py` (validated from source metadata)
Primary Class: `LongContextEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Long Context Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Chunks repositories and long input collections into addressable context segments suitable for retrieval, compression, and model-context assembly.

This document upgrades SPEC-052 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-052 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of LCE is to own the full engineering responsibility for wave 2 - knowledge intelligence. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding LCE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-048 PCE, Repository file set.
- Emit structured output that can be consumed by SPEC-053 KRE, SPEC-061 COE2.
...
- Validation of all LCE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-052` input contract to `SPEC-052` output contract.
- Use `src/intelligence/lce.py` as the implementation boundary and `LongContextEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
LCE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- LCE validates and transforms that evidence.
...
======================================================================
- FR-001: LCE shall load the upstream artifacts declared by SPEC-048 PCE, Repository file set.
- FR-002: LCE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: LCE shall execute the primary behavior owned by `LongContextEngine`.
- FR-004: LCE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-052 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat LCE output as the authoritative contract for wave 2 - knowledge intelligence unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/lce.py`.

Primary implementation class: `LongContextEngine`.
...
======================================================================
LCE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-052` owns how the engine is implemented.
...
LCE must exchange data by contract, not by implicit global state.
...
4. Execute `LongContextEngine` behavior for wave 2 - knowledge intelligence.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-048 PCE, Repository file set.
...
- `LongContextEngine`: Owns the core wave 2 - knowledge intelligence behavior.
...
- Chunks repositories and long input collections into addressable context segments suitable for retrieval, compression, and model-context assembly.
...
- Input interface: `LCEInput` JSON object.
- Output interface: `LCEOutput` JSON object.
- Source interface: `src/intelligence/lce.py`.
- Public class boundary: `LongContextEngine`.
...
|---|---|---|
| `chunk_repository(file_paths: list) -> list` | Executes the `chunk_repository` responsibility for LCE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-052-example",
  "spec_id": "SPEC-052",
  ...
  "payload": {
    "engine": "Long Context Engine",
    "source_class": "LongContextEngine",
    "public_methods": [
      "chunk_repository(file_paths: list) -> list"
    ]
  }
}
```
...
- `AETHERIS_LCE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_LCE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_LCE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/lce.py` implementation is primarily in-memory. The enterprise contract still requires LCE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. LCE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `LCE_STARTED` with request ID, spec ID, source path, and upstream references.
- `LCE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `LCE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- LCE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
LCE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. LCE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `LongContextEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-048 PCE` | Upstream | Must provide validated evidence before LCE starts. |
| `Repository file set` | Upstream | Must provide validated evidence before LCE starts. |
| `SPEC-053 KRE` | Downstream | Must consume only the validated LCE output contract. |
| `SPEC-061 COE2` | Downstream | Must consume only the validated LCE output contract. |
| `src/intelligence/lce.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `LongContextEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: LCE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/lce.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-052-LCE.md
```

Guidance:
- Keep `LongContextEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_lce(payload: dict) -> dict:
    input_contract = validate_lce_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = LongContextEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_lce_output(input_contract, result)
    validate_lce_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/lce.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-052 LCE: Long Context Engine"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["LCE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-052"]
    This --> Source["src/intelligence/lce.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute LCE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
class "LCEInputContract"
class "LCEOutputContract"
LongContextEngine --> "LCEInputContract"
LongContextEngine --> "LCEOutputContract"
@enduml
```
