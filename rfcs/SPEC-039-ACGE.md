# SPEC-039: Autonomous Code Generation Engine (ACGE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: AST-aware code generation and project modification
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/acge.py` (validated from source metadata)
Primary Class: `AutonomousCodeGenerationEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Autonomous Code Generation Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Creates and modifies project files from execution tasks, records implementation decisions, and emits generated-file reports.

This document upgrades SPEC-039 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-039 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of ACGE is to own the full engineering responsibility for ast-aware code generation and project modification. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding ACGE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct ast-aware code generation and project modification was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for ast-aware code generation and project modification.
- Accept only validated upstream artifacts from SPEC-038.
- Emit structured output that can be consumed by SPEC-040.
...
======================================================================
In scope:
- AST-aware code generation and project modification
- Validation of all ACGE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-039` input contract to `SPEC-039` output contract.
- Use `src/execution/acge.py` as the implementation boundary and `AutonomousCodeGenerationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
ACGE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- ACGE validates and transforms that evidence.
...
======================================================================
- FR-001: ACGE shall load the upstream artifacts declared by SPEC-038.
- FR-002: ACGE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: ACGE shall execute the primary behavior owned by `AutonomousCodeGenerationEngine`.
- FR-004: ACGE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-039 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat ACGE output as the authoritative contract for ast-aware code generation and project modification unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/acge.py`.

Primary implementation class: `AutonomousCodeGenerationEngine`.
...
======================================================================
ACGE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-039` owns how the engine is implemented.
...
ACGE must exchange data by contract, not by implicit global state.
...
4. Execute `AutonomousCodeGenerationEngine` behavior for ast-aware code generation and project modification.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-038.
...
- `AutonomousCodeGenerationEngine`: Owns the core ast-aware code generation and project modification behavior.
...
- Input interface: `ACGEInput` JSON object.
- Output interface: `ACGEOutput` JSON object.
- Source interface: `src/execution/acge.py`.
- Public class boundary: `AutonomousCodeGenerationEngine`.
...
|---|---|---|
| `generate_code(task: dict, context: dict) -> dict` | Executes the `generate_code` responsibility for ACGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `modify_file(file_path: str, edits: str) -> bool` | Executes the `modify_file` responsibility for ACGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `create_file(file_path: str, content: str) -> bool` | Executes the `create_file` responsibility for ACGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `update_project(changes: dict) -> bool` | Executes the `update_project` responsibility for ACGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `record_decision(decision: str, reason: str) -> None` | Executes the `record_decision` responsibility for ACGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `validate_output(output_payload: dict) -> bool` | Executes the `validate_output` responsibility for ACGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-039-example",
  "spec_id": "SPEC-039",
  ...
  "payload": {
    "engine": "Autonomous Code Generation Engine",
    "source_class": "AutonomousCodeGenerationEngine",
    "public_methods": [
      "generate_code(task: dict, context: dict) -> dict",
      "modify_file(file_path: str, edits: str) -> bool",
      "create_file(file_path: str, content: str) -> bool",
      "update_project(changes: dict) -> bool",
      "record_decision(decision: str, reason: str) -> None"
    ]
  }
}
```
...
- `AETHERIS_ACGE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_ACGE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_ACGE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. ACGE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `ACGE_STARTED` with request ID, spec ID, source path, and upstream references.
- `ACGE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `ACGE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- ACGE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
ACGE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. ACGE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `AutonomousCodeGenerationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-038` | Upstream | Must provide validated evidence before ACGE starts. |
| `SPEC-040` | Downstream | Must consume only the validated ACGE output contract. |
| `src/execution/acge.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `AutonomousCodeGenerationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: ACGE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/execution/acge.py
tests/test_execution.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-039-ACGE.md
```

Guidance:
- Keep `AutonomousCodeGenerationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_acge(payload: dict) -> dict:
    input_contract = validate_acge_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = AutonomousCodeGenerationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_acge_output(input_contract, result)
    validate_acge_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/acge.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-038"] --> I["Input Validator"]
    I --> E["SPEC-039 ACGE: Autonomous Code Generation Engine"]
...
graph LR
    ...
    V --> Core["AutonomousCodeGenerationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["ACGE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-039"]
    This --> Source["src/execution/acge.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute ACGE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-038" as U
rectangle "Input Validator" as V
rectangle "SPEC-039 ACGE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-040" as D
U --> V
V --> E
E --> K
K --> D
...
class "ACGEInputContract"
class "ACGEOutputContract"
AutonomousCodeGenerationEngine --> "ACGEInputContract"
AutonomousCodeGenerationEngine --> "ACGEOutputContract"
@enduml
```
