# SPEC-059: Cost Optimization Engine (COE)
...
Implementation: `src/intelligence/coe.py` (validated from source metadata)
Primary Class: `CostOptimizationEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Cost Optimization Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Calculates model and execution cost estimates, compares routing alternatives, and enforces cost-aware recommendations.

This document upgrades SPEC-059 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-059 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of COE is to own the full engineering responsibility for wave 3 - optimization. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding COE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-047 MIE, SPEC-058 TOE.
- Emit structured output that can be consumed by SPEC-060 EOE, SPEC-035 MRE.
...
In scope:
- Wave 3 - Optimization
- Validation of all COE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-059` input contract to `SPEC-059` output contract.
- Use `src/intelligence/coe.py` as the implementation boundary and `CostOptimizationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
COE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- COE validates and transforms that evidence.
...
======================================================================
- FR-001: COE shall load the upstream artifacts declared by SPEC-047 MIE, SPEC-058 TOE.
- FR-002: COE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: COE shall execute the primary behavior owned by `CostOptimizationEngine`.
- FR-004: COE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-059 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat COE output as the authoritative contract for wave 3 - optimization unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/coe.py`.

Primary implementation class: `CostOptimizationEngine`.
...
======================================================================
COE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-059` owns how the engine is implemented.
...
COE must exchange data by contract, not by implicit global state.
...
4. Execute `CostOptimizationEngine` behavior for wave 3 - optimization.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-047 MIE, SPEC-058 TOE.
...
- `CostOptimizationEngine`: Owns the core wave 3 - optimization behavior.
...
- Calculates model and execution cost estimates, compares routing alternatives, and enforces cost-aware recommendations.
...
- Input interface: `COEInput` JSON object.
- Output interface: `COEOutput` JSON object.
- Source interface: `src/intelligence/coe.py`.
- Public class boundary: `CostOptimizationEngine`.
...
|---|---|---|
| `calculate_optimal_cost(tokens_count: int, model_id: str) -> float` | Executes the `calculate_optimal_cost` responsibility for COE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-059-example",
  "spec_id": "SPEC-059",
  ...
  "payload": {
    "engine": "Cost Optimization Engine",
    "source_class": "CostOptimizationEngine",
    "public_methods": [
      "calculate_optimal_cost(tokens_count: int, model_id: str) -> float"
    ]
  }
}
```
...
- `AETHERIS_COE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_COE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_COE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/coe.py` implementation is primarily in-memory. The enterprise contract still requires COE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. COE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `COE_STARTED` with request ID, spec ID, source path, and upstream references.
- `COE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `COE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- COE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
COE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. COE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `CostOptimizationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-047 MIE` | Upstream | Must provide validated evidence before COE starts. |
| `SPEC-058 TOE` | Upstream | Must provide validated evidence before COE starts. |
| `SPEC-060 EOE` | Downstream | Must consume only the validated COE output contract. |
| `SPEC-035 MRE` | Downstream | Must consume only the validated COE output contract. |
| `src/intelligence/coe.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `CostOptimizationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: COE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/coe.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-059-COE.md
```

Guidance:
- Keep `CostOptimizationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_coe(payload: dict) -> dict:
    input_contract = validate_coe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = CostOptimizationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_coe_output(input_contract, result)
    validate_coe_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/coe.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-059 COE: Cost Optimization Engine"]
...
graph LR
    ...
    V --> Core["CostOptimizationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["COE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-059"]
    This --> Source["src/intelligence/coe.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-060 EOE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute COE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-059 COE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-060 EOE" as D
U --> V
V --> E
E --> K
K --> D
...
class "COEInputContract"
class "COEOutputContract"
CostOptimizationEngine --> "COEInputContract"
CostOptimizationEngine --> "COEOutputContract"
@enduml
```
