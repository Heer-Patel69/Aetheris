# SPEC-125: Engineering Manager Agent (EMA)
...
Implementation: `src/organization/em.py`
Primary Class: `EngineeringManagerAgent`
Test Reference: `tests/test_rfc008_core.py`

======================================================================
1. MISSION
======================================================================
The Engineering Manager Agent (EMA) exists to provide a permanent specialist role or orchestration component within the Aetheris AI Organization. It represents a professional software engineering persona designed to operate collaboratively, enforce quality, and deliver specific deliverables.
...
The agent consumes inputs defined by the `SPEC-125Input` schema. Key variables include:
- `request_id`: Unique transaction identifier.
- `spec_id`: The ID of this specification (SPEC-125).
- `payload`: Subsystem parameters.

Required Context Inputs:
| Input Source | Format | Purpose |
|---|---|---|
| Sprint backlog, developer task list, task completion logs, blocker flags. | Structured JSON | Contextual parameters for execution loops |
...
The agent produces outputs conforming to the `SPEC-125Output` schema. Outputs include:
...
|---|---|---|
| Task assignments, team velocity metrics, blocker resolutions, sprint status. | Markdown/JSON | Workspace directories & EKB objects |
...
======================================================================
Interaction Scopes:
- Partners with PM; directs Backend, Frontend, and AI/ML engineers; syncs with Scrum Master.
...
8. SUGGESTED MODULES
======================================================================
Suggested path structure: `src/organization/em.py`.
Modules in scope: `em_agent.py, team_tracker.py, workload_allocator.py, blocker_resolver.py`
...
class EngineeringManagerAgent:
    """
    Enterprise agent implementation representing the Engineering Manager Agent role.
    Handles inputs, validates context, and executes role responsibilities.
    """
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.info("Initializing EngineeringManagerAgent agent.")

    def execute_task(self, request_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        self.logger.info(f"Processing task {request_id} for EMA agent.")
        # 1. Input Validation
        if not payload:
        ...
        return {
            ...
            "result": result,
            "metadata": {
                "agent_role": "Engineering Manager Agent",
                "version": "2.0.0"
            }
        }
    ...
    def _process_domain_logic(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"message": "Processed successfully by EngineeringManagerAgent"}
```
...
======================================================================
Prometheus Metrics:
- `aetheris_agent_runs_total{role="EMA"}`: Total task runs.
- `aetheris_agent_failures_total{role="EMA"}`: Total execution errors.
- `aetheris_agent_latency_ms{role="EMA"}`: Task durations.
...
Integration Tests:
- Test message passing and handoff events between EMA and adjacent agents.

Test Command:
`pytest tests/test_rfc008_core.py -k "EngineeringManagerAgent"`
