# SPEC-093: Architecture Learning Engine (ALE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Architecture decision learning and pattern reuse
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/architecture_learning.py` (source implementation pending)
Primary Class: `ArchitectureLearningEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Architecture Learning Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Learns which architecture choices, module boundaries, schemas, and deployment patterns produce reliable and maintainable outcomes across projects.

This document upgrades SPEC-093 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-093 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of ALE is to own the full engineering responsibility for architecture decision learning and pattern reuse. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding ALE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct architecture decision learning and pattern reuse was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for architecture decision learning and pattern reuse.
- Accept only validated upstream artifacts from SPEC-004 APE, SPEC-087 PME, SPEC-088 BPE2.
- Emit structured output that can be consumed by SPEC-004 APE, SPEC-030 FEBC.
...
======================================================================
In scope:
- Architecture decision learning and pattern reuse
- Validation of all ALE input and output structures.
...
- Use `src/learning/architecture_learning.py` as the implementation boundary and `ArchitectureLearningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
ALE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- ALE validates and transforms that evidence.
...
======================================================================
- FR-001: ALE shall load the upstream artifacts declared by SPEC-004 APE, SPEC-087 PME, SPEC-088 BPE2.
- FR-002: ALE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: ALE shall execute the primary behavior owned by `ArchitectureLearningEngine`.
- FR-004: ALE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-093 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat ALE output as the authoritative contract for architecture decision learning and pattern reuse unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/architecture_learning.py`.

Primary implementation class: `ArchitectureLearningEngine`.

Implementation classes in scope:
- `ArchitectureLearningEngine`
...
======================================================================
ALE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-093` owns how the engine is implemented.
...
ALE must exchange data by contract, not by implicit global state.
...
4. Execute `ArchitectureLearningEngine` behavior for architecture decision learning and pattern reuse.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-004 APE, SPEC-087 PME, SPEC-088 BPE2.
...
- `ArchitectureLearningEngine`: Owns the core architecture decision learning and pattern reuse behavior.
...
- Learns which architecture choices, module boundaries, schemas, and deployment patterns produce reliable and maintainable outcomes across projects.
...
- Input interface: `ALEInput` JSON object.
- Output interface: `ALEOutput` JSON object.
- Source interface: `src/learning/architecture_learning.py`.
- Public class boundary: `ArchitectureLearningEngine`.
...
|---|---|---|
| `ArchitectureLearningEngine.run(payload: dict) -> dict` | Executes the `ArchitectureLearningEngine.run` responsibility for ALE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/architecture_learning.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-093-example",
  "spec_id": "SPEC-093",
  ...
  "payload": {
    "engine": "Architecture Learning Engine",
    "source_class": "ArchitectureLearningEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_ALE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_ALE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_ALE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/architecture_learning.py` implementation is primarily in-memory. The enterprise contract still requires ALE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. ALE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `ALE_STARTED` with request ID, spec ID, source path, and upstream references.
- `ALE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `ALE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- ALE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
ALE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. ALE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ArchitectureLearningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-004 APE` | Upstream | Must provide validated evidence before ALE starts. |
| `SPEC-087 PME` | Upstream | Must provide validated evidence before ALE starts. |
| `SPEC-088 BPE2` | Upstream | Must provide validated evidence before ALE starts. |
| `SPEC-004 APE` | Downstream | Must consume only the validated ALE output contract. |
| `SPEC-030 FEBC` | Downstream | Must consume only the validated ALE output contract. |
| `src/learning/architecture_learning.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ArchitectureLearningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: ALE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/architecture_learning.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-093-ALE.md
```

Guidance:
- Keep `ArchitectureLearningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ale(payload: dict) -> dict:
    input_contract = validate_ale_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ArchitectureLearningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ale_output(input_contract, result)
    validate_ale_output(output)
...
- `ArchitectureLearningEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/architecture_learning.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-004 APE"] --> I["Input Validator"]
    I --> E["SPEC-093 ALE: Architecture Learning Engine"]
...
graph LR
    ...
    V --> Core["ArchitectureLearningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["ALE Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-093"]
    This --> Source["src/learning/architecture_learning.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-004 APE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute ALE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-004 APE" as U
rectangle "Input Validator" as V
rectangle "SPEC-093 ALE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-004 APE" as D
U --> V
V --> E
E --> K
K --> D
...
class "ALEInputContract"
class "ALEOutputContract"
ArchitectureLearningEngine --> "ALEInputContract"
ArchitectureLearningEngine --> "ALEOutputContract"
@enduml
```
