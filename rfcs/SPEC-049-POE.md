# SPEC-049: Prompt Optimization Engine (POE)
...
Implementation: `src/intelligence/poe.py` (validated from source metadata)
Primary Class: `PromptOptimizationEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Prompt Optimization Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Optimizes compiled prompts by normalizing whitespace, reducing redundant instructions, tightening context, and preserving mandatory constraints.

This document upgrades SPEC-049 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-049 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of POE is to own the full engineering responsibility for wave 1 - intelligence core. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding POE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-048 PCE.
- Emit structured output that can be consumed by SPEC-050 ERE, SPEC-057 PLE.
...
- Validation of all POE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-049` input contract to `SPEC-049` output contract.
- Use `src/intelligence/poe.py` as the implementation boundary and `PromptOptimizationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
POE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- POE validates and transforms that evidence.
...
======================================================================
- FR-001: POE shall load the upstream artifacts declared by SPEC-048 PCE.
- FR-002: POE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: POE shall execute the primary behavior owned by `PromptOptimizationEngine`.
- FR-004: POE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-049 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat POE output as the authoritative contract for wave 1 - intelligence core unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/poe.py`.

Primary implementation class: `PromptOptimizationEngine`.
...
======================================================================
POE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-049` owns how the engine is implemented.
...
POE must exchange data by contract, not by implicit global state.
...
4. Execute `PromptOptimizationEngine` behavior for wave 1 - intelligence core.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-048 PCE.
...
- `PromptOptimizationEngine`: Owns the core wave 1 - intelligence core behavior.
...
- Optimizes compiled prompts by normalizing whitespace, reducing redundant instructions, tightening context, and preserving mandatory constraints.
...
- Input interface: `POEInput` JSON object.
- Output interface: `POEOutput` JSON object.
- Source interface: `src/intelligence/poe.py`.
- Public class boundary: `PromptOptimizationEngine`.
...
|---|---|---|
| `optimize_prompt(compiled_prompt: dict) -> dict` | Executes the `optimize_prompt` responsibility for POE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-049-example",
  "spec_id": "SPEC-049",
  ...
  "payload": {
    "engine": "Prompt Optimization Engine",
    "source_class": "PromptOptimizationEngine",
    "public_methods": [
      "optimize_prompt(compiled_prompt: dict) -> dict"
    ]
  }
}
```
...
- `AETHERIS_POE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_POE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_POE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/poe.py` implementation is primarily in-memory. The enterprise contract still requires POE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. POE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `POE_STARTED` with request ID, spec ID, source path, and upstream references.
- `POE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `POE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- POE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
POE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. POE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `PromptOptimizationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-048 PCE` | Upstream | Must provide validated evidence before POE starts. |
| `SPEC-050 ERE` | Downstream | Must consume only the validated POE output contract. |
| `SPEC-057 PLE` | Downstream | Must consume only the validated POE output contract. |
| `src/intelligence/poe.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `PromptOptimizationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: POE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/poe.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-049-POE.md
```

Guidance:
- Keep `PromptOptimizationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_poe(payload: dict) -> dict:
    input_contract = validate_poe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = PromptOptimizationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_poe_output(input_contract, result)
    validate_poe_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/poe.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-049 POE: Prompt Optimization Engine"]
...
graph LR
    ...
    V --> Core["PromptOptimizationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["POE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-049"]
    This --> Source["src/intelligence/poe.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute POE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
class "POEInputContract"
class "POEOutputContract"
PromptOptimizationEngine --> "POEInputContract"
PromptOptimizationEngine --> "POEOutputContract"
@enduml
```
