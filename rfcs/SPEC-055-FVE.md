# SPEC-055: Fact Verification Engine (FVE)
...
Implementation: `src/intelligence/fve.py` (validated from source metadata)
Primary Class: `FactVerificationEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Fact Verification Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Verifies engineering claims against available evidence and returns confidence, verification status, and claim-level traceability.

This document upgrades SPEC-055 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-055 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of FVE is to own the full engineering responsibility for wave 2 - knowledge intelligence. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding FVE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-050 ERE, SPEC-053 KRE.
- Emit structured output that can be consumed by SPEC-056 HDE, SPEC-051 SRE.
...
- Validation of all FVE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-055` input contract to `SPEC-055` output contract.
- Use `src/intelligence/fve.py` as the implementation boundary and `FactVerificationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
FVE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- FVE validates and transforms that evidence.
...
======================================================================
- FR-001: FVE shall load the upstream artifacts declared by SPEC-050 ERE, SPEC-053 KRE.
- FR-002: FVE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: FVE shall execute the primary behavior owned by `FactVerificationEngine`.
- FR-004: FVE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-055 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat FVE output as the authoritative contract for wave 2 - knowledge intelligence unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/fve.py`.

Primary implementation class: `FactVerificationEngine`.
...
======================================================================
FVE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-055` owns how the engine is implemented.
...
FVE must exchange data by contract, not by implicit global state.
...
4. Execute `FactVerificationEngine` behavior for wave 2 - knowledge intelligence.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-050 ERE, SPEC-053 KRE.
...
- `FactVerificationEngine`: Owns the core wave 2 - knowledge intelligence behavior.
...
- Verifies engineering claims against available evidence and returns confidence, verification status, and claim-level traceability.
...
- Input interface: `FVEInput` JSON object.
- Output interface: `FVEOutput` JSON object.
- Source interface: `src/intelligence/fve.py`.
- Public class boundary: `FactVerificationEngine`.
...
|---|---|---|
| `verify_fact(claim: str) -> dict` | Executes the `verify_fact` responsibility for FVE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-055-example",
  "spec_id": "SPEC-055",
  ...
  "payload": {
    "engine": "Fact Verification Engine",
    "source_class": "FactVerificationEngine",
    "public_methods": [
      "verify_fact(claim: str) -> dict"
    ]
  }
}
```
...
- `AETHERIS_FVE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_FVE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_FVE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/fve.py` implementation is primarily in-memory. The enterprise contract still requires FVE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. FVE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `FVE_STARTED` with request ID, spec ID, source path, and upstream references.
- `FVE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `FVE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- FVE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
FVE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. FVE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `FactVerificationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-050 ERE` | Upstream | Must provide validated evidence before FVE starts. |
| `SPEC-053 KRE` | Upstream | Must provide validated evidence before FVE starts. |
| `SPEC-056 HDE` | Downstream | Must consume only the validated FVE output contract. |
| `SPEC-051 SRE` | Downstream | Must consume only the validated FVE output contract. |
| `src/intelligence/fve.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `FactVerificationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: FVE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-055-FVE.md
```

Guidance:
- Keep `FactVerificationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_fve(payload: dict) -> dict:
    input_contract = validate_fve_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = FactVerificationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_fve_output(input_contract, result)
    validate_fve_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/fve.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-055 FVE: Fact Verification Engine"]
...
graph LR
    ...
    V --> Core["FactVerificationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["FVE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-055"]
    This --> Source["src/intelligence/fve.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-056 HDE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute FVE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-055 FVE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-056 HDE" as D
U --> V
V --> E
E --> K
K --> D
...
class "FVEInputContract"
class "FVEOutputContract"
FactVerificationEngine --> "FVEInputContract"
FactVerificationEngine --> "FVEOutputContract"
@enduml
```
