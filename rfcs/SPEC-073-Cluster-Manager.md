...
Implementation: `src/runtime/cluster.py` (source implementation pending)
Primary Class: `ClusterManager`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Cluster Manager exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Coordinates runtime nodes, health, membership, scheduling capacity, and failover boundaries for clustered Aetheris deployments.

This document upgrades SPEC-073 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-073 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of CLM is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding CLM behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-074 NDCR, SPEC-075 DCLE.
...
- Validation of all CLM input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-073` input contract to `SPEC-073` output contract.
- Use `src/runtime/cluster.py` as the implementation boundary and `ClusterManager` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
CLM follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- CLM validates and transforms that evidence.
...
======================================================================
- FR-001: CLM shall load the upstream artifacts declared by SPEC-074 NDCR, SPEC-075 DCLE.
- FR-002: CLM shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: CLM shall execute the primary behavior owned by `ClusterManager`.
- FR-004: CLM shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-073 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat CLM output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/cluster.py`.

Primary implementation class: `ClusterManager`.
...
======================================================================
CLM is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-073` owns how the engine is implemented.
...
CLM must exchange data by contract, not by implicit global state.
...
4. Execute `ClusterManager` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-074 NDCR, SPEC-075 DCLE.
...
4. Run the CLM core transformation.
...
- `ClusterManager`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Coordinates runtime nodes, health, membership, scheduling capacity, and failover boundaries for clustered Aetheris deployments.
...
- Input interface: `CLMInput` JSON object.
- Output interface: `CLMOutput` JSON object.
- Source interface: `src/runtime/cluster.py`.
- Public class boundary: `ClusterManager`.
...
|---|---|---|
| `ClusterManager.run(payload: dict) -> dict` | Executes the `ClusterManager.run` responsibility for CLM. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-073-example",
  "spec_id": "SPEC-073",
  ...
  "payload": {
    "engine": "Cluster Manager",
    "source_class": "ClusterManager",
    "public_methods": []
  }
}
```
...
- `AETHERIS_CLM_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_CLM_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_CLM_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/cluster.py` implementation is primarily in-memory. The enterprise contract still requires CLM to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. CLM may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `CLM_STARTED` with request ID, spec ID, source path, and upstream references.
- `CLM_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `CLM_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- CLM must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
CLM is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. CLM should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ClusterManager` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-074 NDCR` | Upstream | Must provide validated evidence before CLM starts. |
| `SPEC-075 DCLE` | Upstream | Must provide validated evidence before CLM starts. |
| `SPEC-071 DEE` | Downstream | Must consume only the validated CLM output contract. |
| `SPEC-085 GRO` | Downstream | Must consume only the validated CLM output contract. |
| `src/runtime/cluster.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ClusterManager` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: CLM grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/cluster.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-073-Cluster-Manager.md
```

Guidance:
- Keep `ClusterManager` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_clm(payload: dict) -> dict:
    input_contract = validate_clm_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ClusterManager(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_clm_output(input_contract, result)
    validate_clm_output(output)
...
- `ClusterManager.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/cluster.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-074 NDCR"] --> I["Input Validator"]
    I --> E["SPEC-073 CLM: Cluster Manager"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["CLM Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-073"]
    This --> Source["src/runtime/cluster.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute CLM"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-074 NDCR" as U
rectangle "Input Validator" as V
rectangle "SPEC-073 CLM" as E
...
class "CLMInputContract"
class "CLMOutputContract"
ClusterManager --> "CLMInputContract"
ClusterManager --> "CLMOutputContract"
@enduml
```
