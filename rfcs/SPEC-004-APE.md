# SPEC-004: Architecture Planning Engine (APE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-001
Layer: Architecture Intelligence
Scope: Architecture style, boundaries, storage, and security planning
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/ape.py` (validated from source metadata)
Primary Class: `ArchitecturePlanningEngine`
Test Reference: `tests/test_ape.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Architecture Planning Engine exists to make Architecture Intelligence deterministic, reviewable, and safe at production scale. Builds the architecture plan, domain boundaries, storage strategy, security boundaries, module layout, and dependency graph for the product plan.

This document upgrades SPEC-004 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-004 is part of the Engineering Knowledge Compiler, where evidence is discovered, normalized, stored, and queried before planning begins.

======================================================================
2. PURPOSE
======================================================================
The purpose of APE is to own the full engineering responsibility for architecture style, boundaries, storage, and security planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding APE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct architecture style, boundaries, storage, and security planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for architecture style, boundaries, storage, and security planning.
- Accept only validated upstream artifacts from SPEC-003.
- Emit structured output that can be consumed by SPEC-005.
...
======================================================================
In scope:
- Architecture style, boundaries, storage, and security planning
- Validation of all APE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-004` input contract to `SPEC-004` output contract.
- Use `src/intelligence/ape.py` as the implementation boundary and `ArchitecturePlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
APE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- APE validates and transforms that evidence.
...
======================================================================
- FR-001: APE shall load the upstream artifacts declared by SPEC-003.
- FR-002: APE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: APE shall execute the primary behavior owned by `ArchitecturePlanningEngine`.
- FR-004: APE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-004 sits in `Architecture Intelligence` under `RFC-001`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat APE output as the authoritative contract for architecture style, boundaries, storage, and security planning unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/ape.py`.

Primary implementation class: `ArchitecturePlanningEngine`.
...
======================================================================
APE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-004` owns how the engine is implemented.
...
APE must exchange data by contract, not by implicit global state.
...
4. Execute `ArchitecturePlanningEngine` behavior for architecture style, boundaries, storage, and security planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003.
...
- `ArchitecturePlanningEngine`: Owns the core architecture style, boundaries, storage, and security planning behavior.
...
- Input interface: `APEInput` JSON object.
- Output interface: `APEOutput` JSON object.
- Source interface: `src/intelligence/ape.py`.
- Public class boundary: `ArchitecturePlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> Tuple[Dict[str, any], Dict[str, any]]` | Executes the `plan` responsibility for APE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-004-example",
  "spec_id": "SPEC-004",
  ...
  "payload": {
    "engine": "Architecture Planning Engine",
    "source_class": "ArchitecturePlanningEngine",
    "public_methods": [
      "plan(product_plan: dict) -> Tuple[Dict[str, any], Dict[str, any]]"
    ]
  }
}
```
...
- `AETHERIS_APE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_APE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_APE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. APE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `APE_STARTED` with request ID, spec ID, source path, and upstream references.
- `APE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `APE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- APE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
APE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. APE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_ape.py`.
...
======================================================================
- Unit tests should instantiate `ArchitecturePlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003` | Upstream | Must provide validated evidence before APE starts. |
| `SPEC-005` | Downstream | Must consume only the validated APE output contract. |
| `src/intelligence/ape.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_ape.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_ape.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `ArchitecturePlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: APE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/ape.py
tests/test_ape.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-004-APE.md
```

Guidance:
- Keep `ArchitecturePlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ape(payload: dict) -> dict:
    input_contract = validate_ape_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ArchitecturePlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ape_output(input_contract, result)
    validate_ape_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/ape.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-003"] --> I["Input Validator"]
    I --> E["SPEC-004 APE: Architecture Planning Engine"]
...
graph LR
    ...
    V --> Core["ArchitecturePlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["APE Payload"]
...
```mermaid
graph TD
    Parent["RFC-001"] --> This["SPEC-004"]
    This --> Source["src/intelligence/ape.py"]
    This --> Test["tests/test_ape.py"]
    This --> Next["SPEC-005"]
```
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute APE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-003" as U
rectangle "Input Validator" as V
rectangle "SPEC-004 APE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-005" as D
U --> V
V --> E
E --> K
K --> D
...
class "APEInputContract"
class "APEOutputContract"
ArchitecturePlanningEngine --> "APEInputContract"
ArchitecturePlanningEngine --> "APEOutputContract"
@enduml
```
