# SPEC-006: Engineering Graph Engine (EGE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-001
Layer: Graph Intelligence
Scope: Digital twin dependency graph
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/ege.py` (validated from source metadata)
Primary Class: `EngineeringGraphEngine`
Test Reference: `tests/test_ege.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Engineering Graph Engine exists to make Graph Intelligence deterministic, reviewable, and safe at production scale. Maintains the graph of requirements, modules, tasks, tests, artifacts, and dependencies used for impact analysis and consistency validation.

This document upgrades SPEC-006 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-006 is part of the Engineering Knowledge Compiler, where evidence is discovered, normalized, stored, and queried before planning begins.

======================================================================
2. PURPOSE
======================================================================
The purpose of EGE is to own the full engineering responsibility for digital twin dependency graph. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EGE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct digital twin dependency graph was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for digital twin dependency graph.
- Accept only validated upstream artifacts from SPEC-005.
- Emit structured output that can be consumed by SPEC-007.
...
- Validation of all EGE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-006` input contract to `SPEC-006` output contract.
- Use `src/intelligence/ege.py` as the implementation boundary and `EngineeringGraphEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EGE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EGE validates and transforms that evidence.
...
======================================================================
- FR-001: EGE shall load the upstream artifacts declared by SPEC-005.
- FR-002: EGE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EGE shall execute the primary behavior owned by `EngineeringGraphEngine`.
- FR-004: EGE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
======================================================================
- Latency target: complete core evidence normalization, graph, storage, or query work in less than 2 seconds for a medium workspace.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-006 sits in `Graph Intelligence` under `RFC-001`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EGE output as the authoritative contract for digital twin dependency graph unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/ege.py`.

Primary implementation class: `EngineeringGraphEngine`.
...
======================================================================
EGE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `RFC-001` owns why this engine exists.
- `SPEC-006` owns how the engine is implemented.
...
EGE must exchange data by contract, not by implicit global state.
...
4. Execute `EngineeringGraphEngine` behavior for digital twin dependency graph.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-005.
...
- `EngineeringGraphEngine`: Owns the core digital twin dependency graph behavior.
...
- Input interface: `EGEInput` JSON object.
- Output interface: `EGEOutput` JSON object.
- Source interface: `src/intelligence/ege.py`.
- Public class boundary: `EngineeringGraphEngine`.
...
|---|---|---|
| `add_node(node_id: str, type: str, metadata: dict) -> None` | Executes the `add_node` responsibility for EGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `add_edge(source: str, target: str, relationship: str) -> None` | Executes the `add_edge` responsibility for EGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `detect_cycles() -> List[str]` | Executes the `detect_cycles` responsibility for EGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `get_impacted_nodes(node_id: str) -> List[str]` | Executes the `get_impacted_nodes` responsibility for EGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `get_isolated_nodes() -> List[str]` | Executes the `get_isolated_nodes` responsibility for EGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-006-example",
  "spec_id": "SPEC-006",
  ...
  "payload": {
    "engine": "Engineering Graph Engine",
    "source_class": "EngineeringGraphEngine",
    "public_methods": [
      "add_node(node_id: str, type: str, metadata: dict) -> None",
      "add_edge(source: str, target: str, relationship: str) -> None",
      "detect_cycles() -> List[str]",
      "get_impacted_nodes(node_id: str) -> List[str]",
      "get_isolated_nodes() -> List[str]"
    ]
  }
}
```
...
- `AETHERIS_EGE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EGE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EGE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. EGE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EGE_STARTED` with request ID, spec ID, source path, and upstream references.
- `EGE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EGE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
- Primary latency target: complete core evidence normalization, graph, storage, or query work in less than 2 seconds for a medium workspace.
...
======================================================================
Scalability model:
- EGE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EGE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EGE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_ege.py`.
...
======================================================================
- Unit tests should instantiate `EngineeringGraphEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-005` | Upstream | Must provide validated evidence before EGE starts. |
| `SPEC-007` | Downstream | Must consume only the validated EGE output contract. |
| `src/intelligence/ege.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_ege.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_ege.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `EngineeringGraphEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EGE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-006-EGE.md
```

Guidance:
- Keep `EngineeringGraphEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ege(payload: dict) -> dict:
    input_contract = validate_ege_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = EngineeringGraphEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ege_output(input_contract, result)
    validate_ege_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/ege.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-005"] --> I["Input Validator"]
    I --> E["SPEC-006 EGE: Engineering Graph Engine"]
...
graph LR
    ...
    V --> Core["EngineeringGraphEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EGE Payload"]
...
```mermaid
graph TD
    Parent["RFC-001"] --> This["SPEC-006"]
    This --> Source["src/intelligence/ege.py"]
    This --> Test["tests/test_ege.py"]
    This --> Next["SPEC-007"]
```
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EGE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-005" as U
rectangle "Input Validator" as V
rectangle "SPEC-006 EGE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-007" as D
U --> V
V --> E
E --> K
K --> D
...
class "EGEInputContract"
class "EGEOutputContract"
EngineeringGraphEngine --> "EGEInputContract"
EngineeringGraphEngine --> "EGEOutputContract"
@enduml
```
