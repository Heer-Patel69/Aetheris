# SPEC-048: Prompt Compiler Engine (PCE)
...
Implementation: `src/intelligence/pce.py` (validated from source metadata)
Primary Class: `PromptCompilerEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Prompt Compiler Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Compiles prompt templates and runtime variables into deterministic prompt packages with goals, constraints, context, and execution metadata.

This document upgrades SPEC-048 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-048 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of PCE is to own the full engineering responsibility for wave 1 - intelligence core. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PCE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from User goal, SPEC-001 WDE, SPEC-002 URUE.
- Emit structured output that can be consumed by SPEC-052 LCE, SPEC-049 POE.
...
- Validation of all PCE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-048` input contract to `SPEC-048` output contract.
- Use `src/intelligence/pce.py` as the implementation boundary and `PromptCompilerEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PCE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PCE validates and transforms that evidence.
...
======================================================================
- FR-001: PCE shall load the upstream artifacts declared by User goal, SPEC-001 WDE, SPEC-002 URUE.
- FR-002: PCE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PCE shall execute the primary behavior owned by `PromptCompilerEngine`.
- FR-004: PCE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-048 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PCE output as the authoritative contract for wave 1 - intelligence core unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/pce.py`.

Primary implementation class: `PromptCompilerEngine`.
...
======================================================================
PCE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-048` owns how the engine is implemented.
...
PCE must exchange data by contract, not by implicit global state.
...
4. Execute `PromptCompilerEngine` behavior for wave 1 - intelligence core.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: User goal, SPEC-001 WDE, SPEC-002 URUE.
...
- `PromptCompilerEngine`: Owns the core wave 1 - intelligence core behavior.
...
- Compiles prompt templates and runtime variables into deterministic prompt packages with goals, constraints, context, and execution metadata.
...
- Input interface: `PCEInput` JSON object.
- Output interface: `PCEOutput` JSON object.
- Source interface: `src/intelligence/pce.py`.
- Public class boundary: `PromptCompilerEngine`.
...
|---|---|---|
| `compile_prompt(template_name: str, variables: dict) -> dict` | Executes the `compile_prompt` responsibility for PCE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-048-example",
  "spec_id": "SPEC-048",
  ...
  "payload": {
    "engine": "Prompt Compiler Engine",
    "source_class": "PromptCompilerEngine",
    "public_methods": [
      "compile_prompt(template_name: str, variables: dict) -> dict"
    ]
  }
}
```
...
- `AETHERIS_PCE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PCE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PCE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/pce.py` implementation is primarily in-memory. The enterprise contract still requires PCE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. PCE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PCE_STARTED` with request ID, spec ID, source path, and upstream references.
- `PCE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PCE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PCE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PCE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PCE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `PromptCompilerEngine` directly and verify public method behavior.
...
|---|---|---|
| `User goal` | Upstream | Must provide validated evidence before PCE starts. |
| `SPEC-001 WDE` | Upstream | Must provide validated evidence before PCE starts. |
| `SPEC-002 URUE` | Upstream | Must provide validated evidence before PCE starts. |
| `SPEC-052 LCE` | Downstream | Must consume only the validated PCE output contract. |
| `SPEC-049 POE` | Downstream | Must consume only the validated PCE output contract. |
| `src/intelligence/pce.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `PromptCompilerEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PCE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-048-PCE.md
```

Guidance:
- Keep `PromptCompilerEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_pce(payload: dict) -> dict:
    input_contract = validate_pce_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = PromptCompilerEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_pce_output(input_contract, result)
    validate_pce_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/pce.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["User goal"] --> I["Input Validator"]
    I --> E["SPEC-048 PCE: Prompt Compiler Engine"]
...
graph LR
    ...
    V --> Core["PromptCompilerEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PCE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-048"]
    This --> Source["src/intelligence/pce.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-052 LCE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PCE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "User goal" as U
rectangle "Input Validator" as V
rectangle "SPEC-048 PCE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-052 LCE" as D
U --> V
V --> E
E --> K
K --> D
...
class "PCEInputContract"
class "PCEOutputContract"
PromptCompilerEngine --> "PCEInputContract"
PromptCompilerEngine --> "PCEOutputContract"
@enduml
```
