# SPEC-011: Backend Planning Engine (BPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Backend module, service, controller, and job planning
...
Primary Class: `BackendPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Backend Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Generates backend bounded modules, service and repository responsibilities, request filters, controllers, background jobs, and operational pipelines.

This document upgrades SPEC-011 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-011 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of BPE is to own the full engineering responsibility for backend module, service, controller, and job planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding BPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct backend module, service, controller, and job planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for backend module, service, controller, and job planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-010.
- Emit structured output that can be consumed by SPEC-012.
...
======================================================================
In scope:
- Backend module, service, controller, and job planning
- Validation of all BPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-011` input contract to `SPEC-011` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `BackendPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
BPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- BPE validates and transforms that evidence.
...
======================================================================
- FR-001: BPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-010.
- FR-002: BPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: BPE shall execute the primary behavior owned by `BackendPlanningEngine`.
- FR-004: BPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-011 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat BPE output as the authoritative contract for backend module, service, controller, and job planning unless a later validation SPEC rejects it.
...
Primary implementation class: `BackendPlanningEngine`.
...
======================================================================
BPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-011` owns how the engine is implemented.
...
BPE must exchange data by contract, not by implicit global state.
...
4. Execute `BackendPlanningEngine` behavior for backend module, service, controller, and job planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-010.
...
- `BackendPlanningEngine`: Owns the core backend module, service, controller, and job planning behavior.
...
- Input interface: `BPEInput` JSON object.
- Output interface: `BPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `BackendPlanningEngine`.
...
|---|---|---|
| `plan_backend(product_plan: dict, architecture_plan: dict) -> dict` | Executes the `plan_backend` responsibility for BPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-011-example",
  "spec_id": "SPEC-011",
  ...
  "payload": {
    "engine": "Backend Planning Engine",
    "source_class": "BackendPlanningEngine",
    "public_methods": [
      "plan_backend(product_plan: dict, architecture_plan: dict) -> dict"
    ]
  }
}
```
...
- `AETHERIS_BPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_BPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_BPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
- `controllers.plan.json`
- `modules.plan.json`
- `request_pipeline.json`
- `services.plan.json`

EKB object registrations owned by this SPEC:
- `backend_background_jobs`
- `backend_controllers_plan`
- `backend_modules_plan`
- `backend_request_pipeline`
- `backend_services_plan`
...
======================================================================
Recovery is conservative. BPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `BPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `BPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `BPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- BPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
BPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. BPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `BackendPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before BPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before BPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before BPE starts. |
| `SPEC-010` | Upstream | Must provide validated evidence before BPE starts. |
| `SPEC-012` | Downstream | Must consume only the validated BPE output contract. |
...
| FR-003 core execution | Direct `BackendPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: BPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-011-BPE.md
```

Guidance:
- Keep `BackendPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_bpe(payload: dict) -> dict:
    input_contract = validate_bpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = BackendPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_bpe_output(input_contract, result)
    validate_bpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-011 BPE: Backend Planning Engine"]
...
graph LR
    ...
    V --> Core["BackendPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["BPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute BPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-011 BPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-012" as D
U --> V
V --> E
E --> K
K --> D
...
class "BPEInputContract"
class "BPEOutputContract"
BackendPlanningEngine --> "BPEInputContract"
BackendPlanningEngine --> "BPEOutputContract"
@enduml
```
