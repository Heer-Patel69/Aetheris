# SPEC-087: Pattern Mining Engine (PME)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Historical pattern discovery and reusable engineering signal mining
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/pattern_mining.py` (source implementation pending)
Primary Class: `PatternMiningEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Pattern Mining Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Mines repeated successful and failed execution structures to identify reusable implementation, planning, prompt, architecture, testing, and recovery patterns.

This document upgrades SPEC-087 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-087 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of PME is to own the full engineering responsibility for historical pattern discovery and reusable engineering signal mining. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PME behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct historical pattern discovery and reusable engineering signal mining was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for historical pattern discovery and reusable engineering signal mining.
- Accept only validated upstream artifacts from SPEC-086 EME2, SPEC-007 EKB, historical artifacts.
- Emit structured output that can be consumed by SPEC-088 BPE2, SPEC-093 ALE, SPEC-094 TLE.
...
======================================================================
In scope:
- Historical pattern discovery and reusable engineering signal mining
- Validation of all PME input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-087` input contract to `SPEC-087` output contract.
- Use `src/learning/pattern_mining.py` as the implementation boundary and `PatternMiningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PME follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PME validates and transforms that evidence.
...
======================================================================
- FR-001: PME shall load the upstream artifacts declared by SPEC-086 EME2, SPEC-007 EKB, historical artifacts.
- FR-002: PME shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PME shall execute the primary behavior owned by `PatternMiningEngine`.
- FR-004: PME shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-087 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PME output as the authoritative contract for historical pattern discovery and reusable engineering signal mining unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/pattern_mining.py`.

Primary implementation class: `PatternMiningEngine`.
...
======================================================================
PME is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-087` owns how the engine is implemented.
...
PME must exchange data by contract, not by implicit global state.
...
4. Execute `PatternMiningEngine` behavior for historical pattern discovery and reusable engineering signal mining.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-086 EME2, SPEC-007 EKB, historical artifacts.
...
4. Run the PME core transformation.
5. Generate artifacts and EKB records.
6. Validate output against the JSON schema in this SPEC.
7. Release the output to SPEC-088 BPE2, SPEC-093 ALE, SPEC-094 TLE.
...
- `PatternMiningEngine`: Owns the core historical pattern discovery and reusable engineering signal mining behavior.
...
- Mines repeated successful and failed execution structures to identify reusable implementation, planning, prompt, architecture, testing, and recovery patterns.
...
- Input interface: `PMEInput` JSON object.
- Output interface: `PMEOutput` JSON object.
- Source interface: `src/learning/pattern_mining.py`.
- Public class boundary: `PatternMiningEngine`.
...
|---|---|---|
| `PatternMiningEngine.run(payload: dict) -> dict` | Executes the `PatternMiningEngine.run` responsibility for PME. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/pattern_mining.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-087-example",
  "spec_id": "SPEC-087",
  ...
  "payload": {
    "engine": "Pattern Mining Engine",
    "source_class": "PatternMiningEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_PME_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PME_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PME_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/pattern_mining.py` implementation is primarily in-memory. The enterprise contract still requires PME to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. PME may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PME_STARTED` with request ID, spec ID, source path, and upstream references.
- `PME_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PME_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PME must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PME is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PME should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `PatternMiningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-086 EME2` | Upstream | Must provide validated evidence before PME starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before PME starts. |
| `historical artifacts` | Upstream | Must provide validated evidence before PME starts. |
| `SPEC-088 BPE2` | Downstream | Must consume only the validated PME output contract. |
| `SPEC-093 ALE` | Downstream | Must consume only the validated PME output contract. |
| `SPEC-094 TLE` | Downstream | Must consume only the validated PME output contract. |
| `src/learning/pattern_mining.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `PatternMiningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PME grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/pattern_mining.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-087-PME.md
```

Guidance:
- Keep `PatternMiningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_pme(payload: dict) -> dict:
    input_contract = validate_pme_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = PatternMiningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_pme_output(input_contract, result)
    validate_pme_output(output)
...
- `PatternMiningEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/pattern_mining.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-086 EME2"] --> I["Input Validator"]
    I --> E["SPEC-087 PME: Pattern Mining Engine"]
...
graph LR
    ...
    V --> Core["PatternMiningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PME Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-087"]
    This --> Source["src/learning/pattern_mining.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-088 BPE2"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PME"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-086 EME2" as U
rectangle "Input Validator" as V
rectangle "SPEC-087 PME" as E
database "EKB / Artifacts" as K
rectangle "SPEC-088 BPE2" as D
U --> V
V --> E
E --> K
K --> D
...
class "PMEInputContract"
class "PMEOutputContract"
PatternMiningEngine --> "PMEInputContract"
PatternMiningEngine --> "PMEOutputContract"
@enduml
```
