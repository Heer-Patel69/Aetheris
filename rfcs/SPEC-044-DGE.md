# SPEC-044: Documentation Generation Engine (DGE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Documentation generation from engineering logs and decisions
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/dge.py` (validated from source metadata)
Primary Class: `DocumentationGenerationEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Documentation Generation Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Generates README updates, API docs, ADRs, documentation metrics, link validation reports, and decision-history documentation.

This document upgrades SPEC-044 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-044 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of DGE is to own the full engineering responsibility for documentation generation from engineering logs and decisions. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding DGE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct documentation generation from engineering logs and decisions was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for documentation generation from engineering logs and decisions.
- Accept only validated upstream artifacts from SPEC-043.
- Emit structured output that can be consumed by SPEC-045.
...
======================================================================
In scope:
- Documentation generation from engineering logs and decisions
- Validation of all DGE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-044` input contract to `SPEC-044` output contract.
- Use `src/execution/dge.py` as the implementation boundary and `DocumentationGenerationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
DGE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- DGE validates and transforms that evidence.
...
======================================================================
- FR-001: DGE shall load the upstream artifacts declared by SPEC-043.
- FR-002: DGE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: DGE shall execute the primary behavior owned by `DocumentationGenerationEngine`.
- FR-004: DGE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-044 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat DGE output as the authoritative contract for documentation generation from engineering logs and decisions unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/dge.py`.

Primary implementation class: `DocumentationGenerationEngine`.

Implementation classes in scope:
- `DocumentationGenerationEngine`
...
======================================================================
DGE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-044` owns how the engine is implemented.
...
DGE must exchange data by contract, not by implicit global state.
...
4. Execute `DocumentationGenerationEngine` behavior for documentation generation from engineering logs and decisions.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-043.
...
- `DocumentationGenerationEngine`: Owns the core documentation generation from engineering logs and decisions behavior.
...
- Input interface: `DGEInput` JSON object.
- Output interface: `DGEOutput` JSON object.
- Source interface: `src/execution/dge.py`.
- Public class boundary: `DocumentationGenerationEngine`.
...
|---|---|---|
| `generate_documentation(decisions: List[dict]) -> dict` | Executes the `generate_documentation` responsibility for DGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `update_documentation() -> bool` | Executes the `update_documentation` responsibility for DGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `validate_docs() -> dict` | Executes the `validate_docs` responsibility for DGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `generate_readme() -> str` | Executes the `generate_readme` responsibility for DGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `generate_api_docs() -> str` | Executes the `generate_api_docs` responsibility for DGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `generate_adrs(decisions: List[dict]) -> None` | Executes the `generate_adrs` responsibility for DGE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-044-example",
  "spec_id": "SPEC-044",
  ...
  "payload": {
    "engine": "Documentation Generation Engine",
    "source_class": "DocumentationGenerationEngine",
    "public_methods": [
      "generate_documentation(decisions: List[dict]) -> dict",
      "update_documentation() -> bool",
      "validate_docs() -> dict",
      "generate_readme() -> str",
      "generate_api_docs() -> str"
    ]
  }
}
```
...
- `AETHERIS_DGE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_DGE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_DGE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. DGE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `DGE_STARTED` with request ID, spec ID, source path, and upstream references.
- `DGE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `DGE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- DGE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
DGE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. DGE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `DocumentationGenerationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-043` | Upstream | Must provide validated evidence before DGE starts. |
| `SPEC-045` | Downstream | Must consume only the validated DGE output contract. |
| `src/execution/dge.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `DocumentationGenerationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: DGE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/execution/dge.py
tests/test_execution.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-044-DGE.md
```

Guidance:
- Keep `DocumentationGenerationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_dge(payload: dict) -> dict:
    input_contract = validate_dge_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = DocumentationGenerationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_dge_output(input_contract, result)
    validate_dge_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/dge.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-043"] --> I["Input Validator"]
    I --> E["SPEC-044 DGE: Documentation Generation Engine"]
...
graph LR
    ...
    V --> Core["DocumentationGenerationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["DGE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-044"]
    This --> Source["src/execution/dge.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute DGE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-043" as U
rectangle "Input Validator" as V
rectangle "SPEC-044 DGE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-045" as D
U --> V
V --> E
E --> K
K --> D
...
class "DGEInputContract"
class "DGEOutputContract"
DocumentationGenerationEngine --> "DGEInputContract"
DocumentationGenerationEngine --> "DGEOutputContract"
@enduml
```
