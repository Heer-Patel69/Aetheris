# SPEC-046: Execution Orchestrator (EO)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Central execution runtime orchestration
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/eo.py` (validated from source metadata)
Primary Class: `ExecutionOrchestrator`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Execution Orchestrator exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Supervises the runtime lifecycle across execution engines, status transitions, cancellation, restart, shutdown, health checks, and orchestration.

This document upgrades SPEC-046 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-046 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of EO is to own the full engineering responsibility for central execution runtime orchestration. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EO behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct central execution runtime orchestration was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for central execution runtime orchestration.
- Accept only validated upstream artifacts from SPEC-045.
- Emit structured output that can be consumed by SPEC-045 EME and runtime completion.
...
======================================================================
In scope:
- Central execution runtime orchestration
- Validation of all EO input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-046` input contract to `SPEC-046` output contract.
- Use `src/execution/eo.py` as the implementation boundary and `ExecutionOrchestrator` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EO follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EO validates and transforms that evidence.
...
======================================================================
- FR-001: EO shall load the upstream artifacts declared by SPEC-045.
- FR-002: EO shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EO shall execute the primary behavior owned by `ExecutionOrchestrator`.
- FR-004: EO shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
======================================================================
- Latency target: produce execution control artifacts in less than 1 second for a 100-task blueprint, excluding downstream model latency.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-046 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.

Upstream dependencies:
- SPEC-045

Downstream consumers:
- SPEC-045 EME and runtime completion

The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EO output as the authoritative contract for central execution runtime orchestration unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/eo.py`.

Primary implementation class: `ExecutionOrchestrator`.
...
======================================================================
EO is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `RFC-003` owns why this engine exists.
- `SPEC-046` owns how the engine is implemented.
...
EO must exchange data by contract, not by implicit global state.
...
4. Execute `ExecutionOrchestrator` behavior for central execution runtime orchestration.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-045.
...
4. Run the EO core transformation.
5. Generate artifacts and EKB records.
6. Validate output against the JSON schema in this SPEC.
7. Release the output to SPEC-045 EME and runtime completion.
...
- `ExecutionOrchestrator`: Owns the core central execution runtime orchestration behavior.
...
- Input interface: `EOInput` JSON object.
- Output interface: `EOOutput` JSON object.
- Source interface: `src/execution/eo.py`.
- Public class boundary: `ExecutionOrchestrator`.
...
|---|---|---|
| `run(workspace: str, requirement: str) -> dict` | Executes the `run` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `pause() -> bool` | Executes the `pause` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `resume() -> bool` | Executes the `resume` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `cancel() -> bool` | Executes the `cancel` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `shutdown() -> bool` | Executes the `shutdown` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `restart() -> bool` | Executes the `restart` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `execution_status() -> str` | Executes the `execution_status` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `system_health() -> dict` | Executes the `system_health` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `orchestrate() -> dict` | Executes the `orchestrate` responsibility for EO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-046-example",
  "spec_id": "SPEC-046",
  ...
  "payload": {
    ...
    "source_class": "ExecutionOrchestrator",
    "public_methods": [
      "run(workspace: str, requirement: str) -> dict",
      "pause() -> bool",
      "resume() -> bool",
      "cancel() -> bool",
      "shutdown() -> bool"
...
- `AETHERIS_EO_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EO_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EO_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
- `system.timeline.json`

EKB object registrations owned by this SPEC:
- `execution_orchestrator_lifecycle`
...
======================================================================
Recovery is conservative. EO may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EO_STARTED` with request ID, spec ID, source path, and upstream references.
- `EO_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EO_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
- Primary latency target: produce execution control artifacts in less than 1 second for a 100-task blueprint, excluding downstream model latency.
...
======================================================================
Scalability model:
- EO must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EO is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EO should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_execution.py`.
...
======================================================================
- Unit tests should instantiate `ExecutionOrchestrator` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-045` | Upstream | Must provide validated evidence before EO starts. |
| `SPEC-045 EME and runtime completion` | Downstream | Must consume only the validated EO output contract. |
| `src/execution/eo.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_execution.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_execution.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `ExecutionOrchestrator` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EO grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
Guidance:
- Keep `ExecutionOrchestrator` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_eo(payload: dict) -> dict:
    input_contract = validate_eo_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ExecutionOrchestrator(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_eo_output(input_contract, result)
    validate_eo_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/eo.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-045"] --> I["Input Validator"]
    I --> E["SPEC-046 EO: Execution Orchestrator"]
    E --> C["Contract Validator"]
    C --> A["Owned Artifacts and EKB Objects"]
    A --> D["SPEC-045 EME and runtime completion"]
```
...
graph LR
    ...
    V --> Core["ExecutionOrchestrator Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EO Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-046"]
    This --> Source["src/execution/eo.py"]
    This --> Test["tests/test_execution.py"]
    This --> Next["SPEC-045 EME and runtime completion"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EO"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-045" as U
rectangle "Input Validator" as V
rectangle "SPEC-046 EO" as E
database "EKB / Artifacts" as K
rectangle "SPEC-045 EME and runtime completion" as D
U --> V
V --> E
E --> K
K --> D
...
class "EOInputContract"
class "EOOutputContract"
ExecutionOrchestrator --> "EOInputContract"
ExecutionOrchestrator --> "EOOutputContract"
@enduml
```
