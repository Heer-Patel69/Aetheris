# SPEC-085: Global Runtime Orchestrator (GRO)
...
Implementation: `src/execution/orchestrator.py` (source implementation pending)
Primary Class: `GlobalRuntimeOrchestrator`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Global Runtime Orchestrator exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Coordinates runtime infrastructure across plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos, and distributed execution.

This document upgrades SPEC-085 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-085 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of GRO is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding GRO behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-066 through SPEC-084, SPEC-065 IO.
- Emit structured output that can be consumed by SPEC-086 EME2, RFC-006 Learning System.
...
- Validation of all GRO input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-085` input contract to `SPEC-085` output contract.
- Use `src/execution/orchestrator.py` as the implementation boundary and `GlobalRuntimeOrchestrator` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
GRO follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- GRO validates and transforms that evidence.
...
======================================================================
- FR-001: GRO shall load the upstream artifacts declared by SPEC-066 through SPEC-084, SPEC-065 IO.
- FR-002: GRO shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: GRO shall execute the primary behavior owned by `GlobalRuntimeOrchestrator`.
- FR-004: GRO shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-085 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.

Upstream dependencies:
- SPEC-066 through SPEC-084
- SPEC-065 IO
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat GRO output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/orchestrator.py`.

Primary implementation class: `GlobalRuntimeOrchestrator`.
...
======================================================================
GRO is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-085` owns how the engine is implemented.
...
GRO must exchange data by contract, not by implicit global state.
...
4. Execute `GlobalRuntimeOrchestrator` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-066 through SPEC-084, SPEC-065 IO.
...
4. Run the GRO core transformation.
5. Generate artifacts and EKB records.
6. Validate output against the JSON schema in this SPEC.
7. Release the output to SPEC-086 EME2, RFC-006 Learning System.
...
- `GlobalRuntimeOrchestrator`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Coordinates runtime infrastructure across plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos, and distributed execution.
...
- Input interface: `GROInput` JSON object.
- Output interface: `GROOutput` JSON object.
- Source interface: `src/execution/orchestrator.py`.
- Public class boundary: `GlobalRuntimeOrchestrator`.
...
|---|---|---|
| `GlobalRuntimeOrchestrator.run(payload: dict) -> dict` | Executes the `GlobalRuntimeOrchestrator.run` responsibility for GRO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/execution/orchestrator.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-085-example",
  "spec_id": "SPEC-085",
  ...
  "payload": {
    "engine": "Global Runtime Orchestrator",
    "source_class": "GlobalRuntimeOrchestrator",
    "public_methods": []
  }
}
```
...
- `AETHERIS_GRO_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_GRO_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_GRO_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/execution/orchestrator.py` implementation is primarily in-memory. The enterprise contract still requires GRO to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. GRO may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `GRO_STARTED` with request ID, spec ID, source path, and upstream references.
- `GRO_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `GRO_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- GRO must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
GRO is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. GRO should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `GlobalRuntimeOrchestrator` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-066 through SPEC-084` | Upstream | Must provide validated evidence before GRO starts. |
| `SPEC-065 IO` | Upstream | Must provide validated evidence before GRO starts. |
| `SPEC-086 EME2` | Downstream | Must consume only the validated GRO output contract. |
| `RFC-006 Learning System` | Downstream | Must consume only the validated GRO output contract. |
| `src/execution/orchestrator.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `GlobalRuntimeOrchestrator` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: GRO grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-085-Global-Runtime-Orchestrator.md
```

Guidance:
- Keep `GlobalRuntimeOrchestrator` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_gro(payload: dict) -> dict:
    input_contract = validate_gro_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = GlobalRuntimeOrchestrator(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_gro_output(input_contract, result)
    validate_gro_output(output)
...
- `GlobalRuntimeOrchestrator.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/orchestrator.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-066 through SPEC-084"] --> I["Input Validator"]
    I --> E["SPEC-085 GRO: Global Runtime Orchestrator"]
...
graph LR
    ...
    V --> Core["GlobalRuntimeOrchestrator Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["GRO Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-085"]
    This --> Source["src/execution/orchestrator.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-086 EME2"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute GRO"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-066 through SPEC-084" as U
rectangle "Input Validator" as V
rectangle "SPEC-085 GRO" as E
database "EKB / Artifacts" as K
rectangle "SPEC-086 EME2" as D
U --> V
V --> E
E --> K
K --> D
...
class "GROInputContract"
class "GROOutputContract"
GlobalRuntimeOrchestrator --> "GROInputContract"
GlobalRuntimeOrchestrator --> "GROOutputContract"
@enduml
```
