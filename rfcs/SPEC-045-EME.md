# SPEC-045: Execution Metrics Engine (EME)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Execution cost, latency, quality, and ROI telemetry
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/eme.py` (validated from source metadata)
Primary Class: `ExecutionMetricsEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Execution Metrics Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Aggregates metrics, dashboards, cost analysis, engineering quality score, performance reports, and historical comparisons.

This document upgrades SPEC-045 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-045 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of EME is to own the full engineering responsibility for execution cost, latency, quality, and roi telemetry. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EME behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct execution cost, latency, quality, and roi telemetry was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for execution cost, latency, quality, and roi telemetry.
- Accept only validated upstream artifacts from SPEC-044.
- Emit structured output that can be consumed by SPEC-046.
...
======================================================================
In scope:
- Execution cost, latency, quality, and ROI telemetry
- Validation of all EME input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-045` input contract to `SPEC-045` output contract.
- Use `src/execution/eme.py` as the implementation boundary and `ExecutionMetricsEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EME follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EME validates and transforms that evidence.
...
======================================================================
- FR-001: EME shall load the upstream artifacts declared by SPEC-044.
- FR-002: EME shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EME shall execute the primary behavior owned by `ExecutionMetricsEngine`.
- FR-004: EME shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-045 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EME output as the authoritative contract for execution cost, latency, quality, and roi telemetry unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/eme.py`.

Primary implementation class: `ExecutionMetricsEngine`.
...
======================================================================
EME is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-045` owns how the engine is implemented.
...
EME must exchange data by contract, not by implicit global state.
...
4. Execute `ExecutionMetricsEngine` behavior for execution cost, latency, quality, and roi telemetry.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-044.
...
- `ExecutionMetricsEngine`: Owns the core execution cost, latency, quality, and roi telemetry behavior.
...
- Input interface: `EMEInput` JSON object.
- Output interface: `EMEOutput` JSON object.
- Source interface: `src/execution/eme.py`.
- Public class boundary: `ExecutionMetricsEngine`.
...
|---|---|---|
| `record_metric(metric_name: str, value: float) -> None` | Executes the `record_metric` responsibility for EME. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `generate_dashboard() -> dict` | Executes the `generate_dashboard` responsibility for EME. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `engineering_score() -> float` | Executes the `engineering_score` responsibility for EME. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `cost_analysis() -> dict` | Executes the `cost_analysis` responsibility for EME. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `performance_report() -> dict` | Executes the `performance_report` responsibility for EME. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `historical_comparison() -> dict` | Executes the `historical_comparison` responsibility for EME. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-045-example",
  "spec_id": "SPEC-045",
  ...
  "payload": {
    "engine": "Execution Metrics Engine",
    "source_class": "ExecutionMetricsEngine",
    "public_methods": [
      "record_metric(metric_name: str, value: float) -> None",
      "generate_dashboard() -> dict",
...
- `AETHERIS_EME_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EME_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EME_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. EME may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EME_STARTED` with request ID, spec ID, source path, and upstream references.
- `EME_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EME_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- EME must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EME is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EME should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ExecutionMetricsEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-044` | Upstream | Must provide validated evidence before EME starts. |
| `SPEC-046` | Downstream | Must consume only the validated EME output contract. |
| `src/execution/eme.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ExecutionMetricsEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EME grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-045-EME.md
```

Guidance:
- Keep `ExecutionMetricsEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_eme(payload: dict) -> dict:
    input_contract = validate_eme_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ExecutionMetricsEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_eme_output(input_contract, result)
    validate_eme_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/eme.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-044"] --> I["Input Validator"]
    I --> E["SPEC-045 EME: Execution Metrics Engine"]
...
graph LR
    ...
    V --> Core["ExecutionMetricsEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EME Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-045"]
    This --> Source["src/execution/eme.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EME"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-044" as U
rectangle "Input Validator" as V
rectangle "SPEC-045 EME" as E
database "EKB / Artifacts" as K
rectangle "SPEC-046" as D
U --> V
V --> E
E --> K
K --> D
...
class "EMEInputContract"
class "EMEOutputContract"
ExecutionMetricsEngine --> "EMEInputContract"
ExecutionMetricsEngine --> "EMEOutputContract"
@enduml
```
