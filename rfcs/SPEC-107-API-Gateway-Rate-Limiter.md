# SPEC-107: API Gateway & Rate Limiter (AGRL)
...
Primary Class: `APIGatewayRateLimiter`
Test Reference: `tests/test_rfc007_core.py`

======================================================================
1. PURPOSE & BUSINESS VALUE
======================================================================
The API Gateway & Rate Limiter (AGRL) exists to provide a foundational enterprise capability for Aetheris. It ensures secure, production-scale, multi-tenant operations, maintaining compliance, security boundaries, and strict resource isolation.
...
======================================================================
- FR-101: The engine shall load configurations conforming to the `SPEC-107Input` schema.
- FR-102: The engine shall authorize all operations against tenant context parameters.
- FR-103: The engine shall execute the core `APIGatewayRateLimiter` behaviors.
- FR-104: The engine shall output structured results conforming to the `SPEC-107Output` schema.
...
======================================================================
Primary Path: `src/enterprise/gateway.py`.
The class `APIGatewayRateLimiter` acts as the main domain service layer, validating API structures, interacting with local storage adapters, and dispatching transaction logs to the audit service.
...
- `APIGatewayRateLimiterCore`: Executes subsystem transformations.
...
3. Call `APIGatewayRateLimiter` core behaviors.
...
======================================================================
Primary logic is defined in `src/enterprise/gateway.py`. Sub-modules handle encryption, parsing, and database schemas.
...
|---|---|---|
| `handle_request(request: dict) -> dict` | Routes, filters, and limits the incoming API request. | Validate input, enforce security boundaries, return deterministic output, and emit telemetry. |
...
18. OBSERVABILITY
======================================================================
Structured logging includes standard event codes like `AGRL_STARTED`, `AGRL_SUCCESS`, and `AGRL_FAILED` with request ID attributes.
