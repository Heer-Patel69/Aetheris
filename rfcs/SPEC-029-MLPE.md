# SPEC-029: Maintenance Lifecycle Planning Engine (MLPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Maintenance, lifecycle, and technical debt planning
...
Primary Class: `MaintenanceLifecyclePlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Maintenance Lifecycle Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Tracks dependency checks, deprecation policy, lifecycle boundaries, technical-debt categories, and maintenance responsibilities.

This document upgrades SPEC-029 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-029 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of MLPE is to own the full engineering responsibility for maintenance, lifecycle, and technical debt planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding MLPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct maintenance, lifecycle, and technical debt planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for maintenance, lifecycle, and technical debt planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-028.
- Emit structured output that can be consumed by SPEC-030.
...
======================================================================
In scope:
- Maintenance, lifecycle, and technical debt planning
- Validation of all MLPE input and output structures.
...
- Use `src/intelligence/planners.py` as the implementation boundary and `MaintenanceLifecyclePlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
MLPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- MLPE validates and transforms that evidence.
...
======================================================================
- FR-001: MLPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-028.
- FR-002: MLPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: MLPE shall execute the primary behavior owned by `MaintenanceLifecyclePlanningEngine`.
- FR-004: MLPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-029 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat MLPE output as the authoritative contract for maintenance, lifecycle, and technical debt planning unless a later validation SPEC rejects it.
...
Primary implementation class: `MaintenanceLifecyclePlanningEngine`.

Implementation classes in scope:
- `MaintenanceLifecyclePlanningEngine`
...
======================================================================
MLPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-029` owns how the engine is implemented.
...
MLPE must exchange data by contract, not by implicit global state.
...
4. Execute `MaintenanceLifecyclePlanningEngine` behavior for maintenance, lifecycle, and technical debt planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-028.
...
- `MaintenanceLifecyclePlanningEngine`: Owns the core maintenance, lifecycle, and technical debt planning behavior.
...
- Input interface: `MLPEInput` JSON object.
- Output interface: `MLPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `MaintenanceLifecyclePlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for MLPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-029-example",
  "spec_id": "SPEC-029",
  ...
  "payload": {
    "engine": "Maintenance Lifecycle Planning Engine",
    "source_class": "MaintenanceLifecyclePlanningEngine",
    "public_methods": [
...
- `AETHERIS_MLPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_MLPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_MLPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
- `technical.debt.plan.json`

EKB object registrations owned by this SPEC:
- `spec_029_maintenance`
...
======================================================================
Recovery is conservative. MLPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `MLPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `MLPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `MLPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- MLPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
MLPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. MLPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `MaintenanceLifecyclePlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before MLPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before MLPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before MLPE starts. |
| `SPEC-028` | Upstream | Must provide validated evidence before MLPE starts. |
| `SPEC-030` | Downstream | Must consume only the validated MLPE output contract. |
...
| FR-003 core execution | Direct `MaintenanceLifecyclePlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: MLPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-029-MLPE.md
```

Guidance:
- Keep `MaintenanceLifecyclePlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_mlpe(payload: dict) -> dict:
    input_contract = validate_mlpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = MaintenanceLifecyclePlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_mlpe_output(input_contract, result)
    validate_mlpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-029 MLPE: Maintenance Lifecycle Planning Engine"]
...
graph LR
    ...
    V --> Core["MaintenanceLifecyclePlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["MLPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute MLPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-029 MLPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-030" as D
U --> V
V --> E
E --> K
K --> D
...
class "MLPEInputContract"
class "MLPEOutputContract"
MaintenanceLifecyclePlanningEngine --> "MLPEInputContract"
MaintenanceLifecyclePlanningEngine --> "MLPEOutputContract"
@enduml
```
