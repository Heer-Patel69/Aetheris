# SPEC-037: Execution Scheduler (ES)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Queue ordering, schedule control, and resource allocation
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/es.py` (validated from source metadata)
Primary Class: `ExecutionScheduler`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Execution Scheduler exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Builds execution queues, orders tasks by dependency, allocates schedule steps, handles pause/resume, and reschedules failed work.

This document upgrades SPEC-037 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-037 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of ES is to own the full engineering responsibility for queue ordering, schedule control, and resource allocation. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding ES behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct queue ordering, schedule control, and resource allocation was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for queue ordering, schedule control, and resource allocation.
- Accept only validated upstream artifacts from SPEC-036.
- Emit structured output that can be consumed by SPEC-038.
...
======================================================================
In scope:
- Queue ordering, schedule control, and resource allocation
- Validation of all ES input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-037` input contract to `SPEC-037` output contract.
- Use `src/execution/es.py` as the implementation boundary and `ExecutionScheduler` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
ES follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- ES validates and transforms that evidence.
...
======================================================================
- FR-001: ES shall load the upstream artifacts declared by SPEC-036.
- FR-002: ES shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: ES shall execute the primary behavior owned by `ExecutionScheduler`.
- FR-004: ES shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-037 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat ES output as the authoritative contract for queue ordering, schedule control, and resource allocation unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/es.py`.

Primary implementation class: `ExecutionScheduler`.
...
======================================================================
ES is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-037` owns how the engine is implemented.
...
ES must exchange data by contract, not by implicit global state.
...
4. Execute `ExecutionScheduler` behavior for queue ordering, schedule control, and resource allocation.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-036.
...
- `ExecutionScheduler`: Owns the core queue ordering, schedule control, and resource allocation behavior.
...
- Input interface: `ESInput` JSON object.
- Output interface: `ESOutput` JSON object.
- Source interface: `src/execution/es.py`.
- Public class boundary: `ExecutionScheduler`.
...
|---|---|---|
| `schedule(dag_data: dict, model_assignments: dict) -> dict` | Executes the `schedule` responsibility for ES. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `reschedule_on_failure(failed_task_id: str) -> dict` | Executes the `reschedule_on_failure` responsibility for ES. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `pause_execution() -> bool` | Executes the `pause_execution` responsibility for ES. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `resume_execution() -> bool` | Executes the `resume_execution` responsibility for ES. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `critical_path() -> List[str]` | Executes the `critical_path` responsibility for ES. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `execution_order() -> List[str]` | Executes the `execution_order` responsibility for ES. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `parallel_batches() -> List[List[str]]` | Executes the `parallel_batches` responsibility for ES. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-037-example",
  "spec_id": "SPEC-037",
  ...
  "payload": {
    ...
    "source_class": "ExecutionScheduler",
    "public_methods": [
      "schedule(dag_data: dict, model_assignments: dict) -> dict",
      "reschedule_on_failure(failed_task_id: str) -> dict",
...
- `AETHERIS_ES_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_ES_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_ES_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
- `parallel.batches.json`
- `scheduler.plan.json`

EKB object registrations owned by this SPEC:
- `execution_scheduler_plan`
...
======================================================================
Recovery is conservative. ES may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `ES_STARTED` with request ID, spec ID, source path, and upstream references.
- `ES_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `ES_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- ES must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
ES is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. ES should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ExecutionScheduler` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-036` | Upstream | Must provide validated evidence before ES starts. |
| `SPEC-038` | Downstream | Must consume only the validated ES output contract. |
| `src/execution/es.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ExecutionScheduler` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: ES grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
Guidance:
- Keep `ExecutionScheduler` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_es(payload: dict) -> dict:
    input_contract = validate_es_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ExecutionScheduler(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_es_output(input_contract, result)
    validate_es_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/es.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-036"] --> I["Input Validator"]
    I --> E["SPEC-037 ES: Execution Scheduler"]
...
graph LR
    ...
    V --> Core["ExecutionScheduler Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["ES Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-037"]
    This --> Source["src/execution/es.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute ES"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-036" as U
rectangle "Input Validator" as V
rectangle "SPEC-037 ES" as E
database "EKB / Artifacts" as K
rectangle "SPEC-038" as D
U --> V
V --> E
E --> K
K --> D
...
class "ESInputContract"
class "ESOutputContract"
ExecutionScheduler --> "ESInputContract"
ExecutionScheduler --> "ESOutputContract"
@enduml
```
