# SPEC-051: Self Reflection Engine (SRE)
...
Implementation: `src/intelligence/sre.py` (validated from source metadata)
Primary Class: `SelfReflectionEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Self Reflection Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Critiques reasoning and solution proposals, verifies consistency against constraints, identifies weaknesses, and approves or rejects candidate outputs.

This document upgrades SPEC-051 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-051 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of SRE is to own the full engineering responsibility for wave 1 - intelligence core. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Accept only validated upstream artifacts from SPEC-050 ERE, SPEC-055 FVE, SPEC-056 HDE.
- Emit structured output that can be consumed by SPEC-057 PLE, SPEC-065 IO.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-051` input contract to `SPEC-051` output contract.
- Use `src/intelligence/sre.py` as the implementation boundary and `SelfReflectionEngine` as the primary class boundary.
...
======================================================================
- FR-001: SRE shall load the upstream artifacts declared by SPEC-050 ERE, SPEC-055 FVE, SPEC-056 HDE.
- FR-002: SRE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SRE shall execute the primary behavior owned by `SelfReflectionEngine`.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-051 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SRE output as the authoritative contract for wave 1 - intelligence core unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/sre.py`.

Primary implementation class: `SelfReflectionEngine`.
...
- `SPEC-051` owns how the engine is implemented.
...
4. Execute `SelfReflectionEngine` behavior for wave 1 - intelligence core.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-050 ERE, SPEC-055 FVE, SPEC-056 HDE.
...
7. Release the output to SPEC-057 PLE, SPEC-065 IO.
...
- `SelfReflectionEngine`: Owns the core wave 1 - intelligence core behavior.
...
- Critiques reasoning and solution proposals, verifies consistency against constraints, identifies weaknesses, and approves or rejects candidate outputs.
...
- Source interface: `src/intelligence/sre.py`.
- Public class boundary: `SelfReflectionEngine`.
...
|---|---|---|
| `critique_solution(proposed_solution: dict) -> dict` | Executes the `critique_solution` responsibility for SRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-051-example",
  "spec_id": "SPEC-051",
  ...
  "payload": {
    "engine": "Self Reflection Engine",
    "source_class": "SelfReflectionEngine",
    "public_methods": [
      "critique_solution(proposed_solution: dict) -> dict"
    ]
  }
}
```
...
======================================================================
The current `src/intelligence/sre.py` implementation is primarily in-memory. The enterprise contract still requires SRE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
- Unit tests should instantiate `SelfReflectionEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-050 ERE` | Upstream | Must provide validated evidence before SRE starts. |
| `SPEC-055 FVE` | Upstream | Must provide validated evidence before SRE starts. |
| `SPEC-056 HDE` | Upstream | Must provide validated evidence before SRE starts. |
| `SPEC-057 PLE` | Downstream | Must consume only the validated SRE output contract. |
| `SPEC-065 IO` | Downstream | Must consume only the validated SRE output contract. |
| `src/intelligence/sre.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `SelfReflectionEngine` public API tests |
...
```text
src/intelligence/sre.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-051-SRE.md
```

Guidance:
- Keep `SelfReflectionEngine` as the primary orchestration class for this SPEC.
...
def run_sre(payload: dict) -> dict:
    ...
    result = SelfReflectionEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/sre.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-050 ERE"] --> I["Input Validator"]
    I --> E["SPEC-051 SRE: Self Reflection Engine"]
...
graph LR
    ...
    V --> Core["SelfReflectionEngine Core"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-051"]
    This --> Source["src/intelligence/sre.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-057 PLE"]
```
...
```plantuml
@startuml
rectangle "SPEC-050 ERE" as U
rectangle "Input Validator" as V
rectangle "SPEC-051 SRE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-057 PLE" as D
U --> V
V --> E
E --> K
K --> D
...
SelfReflectionEngine --> "SREInputContract"
SelfReflectionEngine --> "SREOutputContract"
@enduml
```
