# SPEC-002: Universal Requirement Understanding Engine (URUE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-001
Layer: Requirement Intelligence
Scope: Requirement interpretation and normalization
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/urue.py` (validated from source metadata)
Primary Class: `UniversalRequirementUnderstandingEngine`
Test Reference: `tests/test_urue.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Universal Requirement Understanding Engine exists to make Requirement Intelligence deterministic, reviewable, and safe at production scale. Transforms raw user goals and workspace evidence into normalized business, functional, non-functional, constraint, and acceptance requirement models.

This document upgrades SPEC-002 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-002 is part of the Engineering Knowledge Compiler, where evidence is discovered, normalized, stored, and queried before planning begins.

======================================================================
2. PURPOSE
======================================================================
The purpose of URUE is to own the full engineering responsibility for requirement interpretation and normalization. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding URUE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct requirement interpretation and normalization was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for requirement interpretation and normalization.
- Accept only validated upstream artifacts from SPEC-001.
- Emit structured output that can be consumed by SPEC-003.
...
- Validation of all URUE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-002` input contract to `SPEC-002` output contract.
- Use `src/intelligence/urue.py` as the implementation boundary and `UniversalRequirementUnderstandingEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
URUE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- URUE validates and transforms that evidence.
...
======================================================================
- FR-001: URUE shall load the upstream artifacts declared by SPEC-001.
- FR-002: URUE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: URUE shall execute the primary behavior owned by `UniversalRequirementUnderstandingEngine`.
- FR-004: URUE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-002 sits in `Requirement Intelligence` under `RFC-001`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat URUE output as the authoritative contract for requirement interpretation and normalization unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/urue.py`.

Primary implementation class: `UniversalRequirementUnderstandingEngine`.
...
======================================================================
URUE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-002` owns how the engine is implemented.
...
URUE must exchange data by contract, not by implicit global state.
...
4. Execute `UniversalRequirementUnderstandingEngine` behavior for requirement interpretation and normalization.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-001.
...
- `UniversalRequirementUnderstandingEngine`: Owns the core requirement interpretation and normalization behavior.
...
- Input interface: `URUEInput` JSON object.
- Output interface: `URUEOutput` JSON object.
- Source interface: `src/intelligence/urue.py`.
- Public class boundary: `UniversalRequirementUnderstandingEngine`.
...
|---|---|---|
| `understand(prompt: str, wde_inventories: Dict[str, dict]) -> Dict[str, any]` | Executes the `understand` responsibility for URUE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/intelligence/urue.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-002-example",
  "spec_id": "SPEC-002",
  ...
  "payload": {
    "engine": "Universal Requirement Understanding Engine",
    "source_class": "UniversalRequirementUnderstandingEngine",
    "public_methods": [
      "understand(prompt: str, wde_inventories: Dict[str, dict]) -> Dict[str, any]"
    ]
  }
}
```
...
- `AETHERIS_URUE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_URUE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_URUE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. URUE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `URUE_STARTED` with request ID, spec ID, source path, and upstream references.
- `URUE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `URUE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- URUE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
URUE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. URUE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_urue.py`.
...
======================================================================
- Unit tests should instantiate `UniversalRequirementUnderstandingEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-001` | Upstream | Must provide validated evidence before URUE starts. |
| `SPEC-003` | Downstream | Must consume only the validated URUE output contract. |
| `src/intelligence/urue.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_urue.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_urue.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `UniversalRequirementUnderstandingEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: URUE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/urue.py
tests/test_urue.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-002-URUE.md
```

Guidance:
- Keep `UniversalRequirementUnderstandingEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_urue(payload: dict) -> dict:
    input_contract = validate_urue_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = UniversalRequirementUnderstandingEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_urue_output(input_contract, result)
    validate_urue_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/urue.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-001"] --> I["Input Validator"]
    I --> E["SPEC-002 URUE: Universal Requirement Understanding Engine"]
...
graph LR
    ...
    V --> Core["UniversalRequirementUnderstandingEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["URUE Payload"]
...
```mermaid
graph TD
    Parent["RFC-001"] --> This["SPEC-002"]
    This --> Source["src/intelligence/urue.py"]
    This --> Test["tests/test_urue.py"]
    This --> Next["SPEC-003"]
```
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute URUE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-001" as U
rectangle "Input Validator" as V
rectangle "SPEC-002 URUE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-003" as D
U --> V
V --> E
E --> K
K --> D
...
class "URUEInputContract"
class "URUEOutputContract"
UniversalRequirementUnderstandingEngine --> "URUEInputContract"
UniversalRequirementUnderstandingEngine --> "URUEOutputContract"
@enduml
```
