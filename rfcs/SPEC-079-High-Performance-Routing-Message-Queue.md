# SPEC-079: High Performance Routing Message Queue (HPRMQ)
...
Implementation: `src/runtime/message_queue.py` (source implementation pending)
Primary Class: `HighPerformanceRoutingMessageQueue`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
High Performance Routing Message Queue exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Routes high-volume runtime messages with priority, backpressure, durability, and latency controls.

This document upgrades SPEC-079 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-079 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of HPRMQ is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding HPRMQ behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-069 RPCF, SPEC-070 IPCF.
- Emit structured output that can be consumed by SPEC-071 DEE, SPEC-085 GRO.
...
- Validation of all HPRMQ input and output structures.
...
- Use `src/runtime/message_queue.py` as the implementation boundary and `HighPerformanceRoutingMessageQueue` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
HPRMQ follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- HPRMQ validates and transforms that evidence.
...
======================================================================
- FR-001: HPRMQ shall load the upstream artifacts declared by SPEC-069 RPCF, SPEC-070 IPCF.
- FR-002: HPRMQ shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: HPRMQ shall execute the primary behavior owned by `HighPerformanceRoutingMessageQueue`.
- FR-004: HPRMQ shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-079 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat HPRMQ output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/message_queue.py`.

Primary implementation class: `HighPerformanceRoutingMessageQueue`.

Implementation classes in scope:
- `HighPerformanceRoutingMessageQueue`
...
======================================================================
HPRMQ is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-079` owns how the engine is implemented.
...
HPRMQ must exchange data by contract, not by implicit global state.
...
4. Execute `HighPerformanceRoutingMessageQueue` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-069 RPCF, SPEC-070 IPCF.
...
- `HighPerformanceRoutingMessageQueue`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Routes high-volume runtime messages with priority, backpressure, durability, and latency controls.
...
- Input interface: `HPRMQInput` JSON object.
- Output interface: `HPRMQOutput` JSON object.
- Source interface: `src/runtime/message_queue.py`.
- Public class boundary: `HighPerformanceRoutingMessageQueue`.
...
|---|---|---|
| `HighPerformanceRoutingMessageQueue.run(payload: dict) -> dict` | Executes the `HighPerformanceRoutingMessageQueue.run` responsibility for HPRMQ. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/runtime/message_queue.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-079-example",
  "spec_id": "SPEC-079",
  ...
  "payload": {
    "engine": "High Performance Routing Message Queue",
    "source_class": "HighPerformanceRoutingMessageQueue",
    "public_methods": []
  }
}
```
...
- `AETHERIS_HPRMQ_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_HPRMQ_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_HPRMQ_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/message_queue.py` implementation is primarily in-memory. The enterprise contract still requires HPRMQ to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. HPRMQ may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `HPRMQ_STARTED` with request ID, spec ID, source path, and upstream references.
- `HPRMQ_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `HPRMQ_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- HPRMQ must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
HPRMQ is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. HPRMQ should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `HighPerformanceRoutingMessageQueue` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-069 RPCF` | Upstream | Must provide validated evidence before HPRMQ starts. |
| `SPEC-070 IPCF` | Upstream | Must provide validated evidence before HPRMQ starts. |
| `SPEC-071 DEE` | Downstream | Must consume only the validated HPRMQ output contract. |
| `SPEC-085 GRO` | Downstream | Must consume only the validated HPRMQ output contract. |
| `src/runtime/message_queue.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `HighPerformanceRoutingMessageQueue` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: HPRMQ grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/message_queue.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-079-High-Performance-Routing-Message-Queue.md
```

Guidance:
- Keep `HighPerformanceRoutingMessageQueue` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_hprmq(payload: dict) -> dict:
    input_contract = validate_hprmq_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = HighPerformanceRoutingMessageQueue(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_hprmq_output(input_contract, result)
    validate_hprmq_output(output)
...
- `HighPerformanceRoutingMessageQueue.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/message_queue.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-069 RPCF"] --> I["Input Validator"]
    I --> E["SPEC-079 HPRMQ: High Performance Routing Message Queue"]
...
graph LR
    ...
    V --> Core["HighPerformanceRoutingMessageQueue Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["HPRMQ Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-079"]
    This --> Source["src/runtime/message_queue.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-071 DEE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute HPRMQ"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-069 RPCF" as U
rectangle "Input Validator" as V
rectangle "SPEC-079 HPRMQ" as E
database "EKB / Artifacts" as K
rectangle "SPEC-071 DEE" as D
U --> V
V --> E
E --> K
K --> D
...
```plantuml
@startuml
class HighPerformanceRoutingMessageQueue {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "HPRMQInputContract"
class "HPRMQOutputContract"
HighPerformanceRoutingMessageQueue --> "HPRMQInputContract"
HighPerformanceRoutingMessageQueue --> "HPRMQOutputContract"
@enduml
```
