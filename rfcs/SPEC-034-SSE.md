# SPEC-034: Skill Selection Engine (SSE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Task-to-skill matching and fallback selection
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/sse.py` (validated from source metadata)
Primary Class: `SkillSelectionEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Skill Selection Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Matches tasks to specialist skills, ranks skill confidence, identifies missing skill coverage, and recommends fallback execution paths.

This document upgrades SPEC-034 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-034 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of SSE is to own the full engineering responsibility for task-to-skill matching and fallback selection. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding SSE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct task-to-skill matching and fallback selection was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for task-to-skill matching and fallback selection.
- Accept only validated upstream artifacts from SPEC-033.
- Emit structured output that can be consumed by SPEC-035.
...
======================================================================
In scope:
- Task-to-skill matching and fallback selection
- Validation of all SSE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-034` input contract to `SPEC-034` output contract.
- Use `src/execution/sse.py` as the implementation boundary and `SkillSelectionEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
SSE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- SSE validates and transforms that evidence.
...
======================================================================
- FR-001: SSE shall load the upstream artifacts declared by SPEC-033.
- FR-002: SSE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SSE shall execute the primary behavior owned by `SkillSelectionEngine`.
- FR-004: SSE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-034 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SSE output as the authoritative contract for task-to-skill matching and fallback selection unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/sse.py`.

Primary implementation class: `SkillSelectionEngine`.
...
======================================================================
SSE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-034` owns how the engine is implemented.
...
SSE must exchange data by contract, not by implicit global state.
...
4. Execute `SkillSelectionEngine` behavior for task-to-skill matching and fallback selection.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-033.
...
- `SkillSelectionEngine`: Owns the core task-to-skill matching and fallback selection behavior.
...
- Input interface: `SSEInput` JSON object.
- Output interface: `SSEOutput` JSON object.
- Source interface: `src/execution/sse.py`.
- Public class boundary: `SkillSelectionEngine`.
...
|---|---|---|
| `select_skills(tasks: List[dict]) -> dict` | Executes the `select_skills` responsibility for SSE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `rank_skills_for_task(task: dict) -> List[dict]` | Executes the `rank_skills_for_task` responsibility for SSE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `identify_missing_skills(tasks: List[dict]) -> List[str]` | Executes the `identify_missing_skills` responsibility for SSE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `recommend_fallback(task: dict) -> str` | Executes the `recommend_fallback` responsibility for SSE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-034-example",
  "spec_id": "SPEC-034",
  ...
  "payload": {
    "engine": "Skill Selection Engine",
    "source_class": "SkillSelectionEngine",
    "public_methods": [
      "select_skills(tasks: List[dict]) -> dict",
      "rank_skills_for_task(task: dict) -> List[dict]",
      "identify_missing_skills(tasks: List[dict]) -> List[str]",
      "recommend_fallback(task: dict) -> str"
    ]
  }
}
```
...
- `AETHERIS_SSE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_SSE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_SSE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. SSE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `SSE_STARTED` with request ID, spec ID, source path, and upstream references.
- `SSE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `SSE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- SSE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
SSE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. SSE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `SkillSelectionEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-033` | Upstream | Must provide validated evidence before SSE starts. |
| `SPEC-035` | Downstream | Must consume only the validated SSE output contract. |
| `src/execution/sse.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `SkillSelectionEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: SSE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/execution/sse.py
tests/test_execution.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-034-SSE.md
```

Guidance:
- Keep `SkillSelectionEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_sse(payload: dict) -> dict:
    input_contract = validate_sse_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = SkillSelectionEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_sse_output(input_contract, result)
    validate_sse_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/sse.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-033"] --> I["Input Validator"]
    I --> E["SPEC-034 SSE: Skill Selection Engine"]
...
graph LR
    ...
    V --> Core["SkillSelectionEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["SSE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-034"]
    This --> Source["src/execution/sse.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute SSE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-033" as U
rectangle "Input Validator" as V
rectangle "SPEC-034 SSE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-035" as D
U --> V
V --> E
E --> K
K --> D
...
class "SSEInputContract"
class "SSEOutputContract"
SkillSelectionEngine --> "SSEInputContract"
SkillSelectionEngine --> "SSEOutputContract"
@enduml
```
