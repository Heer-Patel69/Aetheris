...
Implementation: `src/runtime/ipc.py` (source implementation pending)
Primary Class: `IPCFramework`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
IPC Framework exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Provides local inter-process communication between runtime components with bounded queues, typed messages, and fail-closed process isolation.

This document upgrades SPEC-070 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-070 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of IPCF is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding IPCF behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-068 RPL, local runtime processes.
- Emit structured output that can be consumed by SPEC-072 WPM, SPEC-078 DLEAB.
...
- Validation of all IPCF input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-070` input contract to `SPEC-070` output contract.
- Use `src/runtime/ipc.py` as the implementation boundary and `IPCFramework` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
IPCF follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- IPCF validates and transforms that evidence.
...
======================================================================
- FR-001: IPCF shall load the upstream artifacts declared by SPEC-068 RPL, local runtime processes.
- FR-002: IPCF shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: IPCF shall execute the primary behavior owned by `IPCFramework`.
- FR-004: IPCF shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-070 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat IPCF output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/ipc.py`.

Primary implementation class: `IPCFramework`.
...
======================================================================
IPCF is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-070` owns how the engine is implemented.
...
IPCF must exchange data by contract, not by implicit global state.
...
4. Execute `IPCFramework` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-068 RPL, local runtime processes.
...
- `IPCFramework`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Provides local inter-process communication between runtime components with bounded queues, typed messages, and fail-closed process isolation.
...
- Input interface: `IPCFInput` JSON object.
- Output interface: `IPCFOutput` JSON object.
- Source interface: `src/runtime/ipc.py`.
- Public class boundary: `IPCFramework`.
...
|---|---|---|
| `IPCFramework.run(payload: dict) -> dict` | Executes the `IPCFramework.run` responsibility for IPCF. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-070-example",
  "spec_id": "SPEC-070",
  ...
  "payload": {
    "engine": "IPC Framework",
    "source_class": "IPCFramework",
    "public_methods": []
  }
}
```
...
- `AETHERIS_IPCF_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_IPCF_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_IPCF_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/ipc.py` implementation is primarily in-memory. The enterprise contract still requires IPCF to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. IPCF may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `IPCF_STARTED` with request ID, spec ID, source path, and upstream references.
- `IPCF_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `IPCF_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- IPCF must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
IPCF is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. IPCF should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `IPCFramework` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-068 RPL` | Upstream | Must provide validated evidence before IPCF starts. |
| `local runtime processes` | Upstream | Must provide validated evidence before IPCF starts. |
| `SPEC-072 WPM` | Downstream | Must consume only the validated IPCF output contract. |
| `SPEC-078 DLEAB` | Downstream | Must consume only the validated IPCF output contract. |
| `src/runtime/ipc.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `IPCFramework` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: IPCF grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-070-IPC-Framework.md
```

Guidance:
- Keep `IPCFramework` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ipcf(payload: dict) -> dict:
    input_contract = validate_ipcf_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = IPCFramework(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ipcf_output(input_contract, result)
    validate_ipcf_output(output)
...
- `IPCFramework.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/ipc.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-068 RPL"] --> I["Input Validator"]
    I --> E["SPEC-070 IPCF: IPC Framework"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["IPCF Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-070"]
    This --> Source["src/runtime/ipc.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute IPCF"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-068 RPL" as U
rectangle "Input Validator" as V
rectangle "SPEC-070 IPCF" as E
...
class "IPCFInputContract"
class "IPCFOutputContract"
IPCFramework --> "IPCFInputContract"
IPCFramework --> "IPCFOutputContract"
@enduml
```
