# SPEC-100: Learning System Orchestrator (LSO)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Learning subsystem coordination and publication pipeline
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/orchestrator.py` (source implementation pending)
Primary Class: `LearningSystemOrchestrator`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Learning System Orchestrator exists to make Learning System deterministic, reviewable, and safe at production scale. Coordinates SPEC-086 through SPEC-099, manages learning lifecycle state, resolves conflicts between learning signals, and publishes validated recommendations to downstream enterprise and self-evolution systems.

This document upgrades SPEC-100 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-100 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of LSO is to own the full engineering responsibility for learning subsystem coordination and publication pipeline. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding LSO behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct learning subsystem coordination and publication pipeline was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for learning subsystem coordination and publication pipeline.
- Accept only validated upstream artifacts from SPEC-086 through SPEC-099.
- Emit structured output that can be consumed by RFC-007 Enterprise, RFC-008 AI Organization, RFC-009 Self-Evolution.
...
======================================================================
In scope:
- Learning subsystem coordination and publication pipeline
- Validation of all LSO input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-100` input contract to `SPEC-100` output contract.
- Use `src/learning/orchestrator.py` as the implementation boundary and `LearningSystemOrchestrator` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
LSO follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- LSO validates and transforms that evidence.
...
======================================================================
- FR-001: LSO shall load the upstream artifacts declared by SPEC-086 through SPEC-099.
- FR-002: LSO shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: LSO shall execute the primary behavior owned by `LearningSystemOrchestrator`.
- FR-004: LSO shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-100 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.

Upstream dependencies:
- SPEC-086 through SPEC-099
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat LSO output as the authoritative contract for learning subsystem coordination and publication pipeline unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/orchestrator.py`.

Primary implementation class: `LearningSystemOrchestrator`.
...
======================================================================
LSO is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-100` owns how the engine is implemented.
...
LSO must exchange data by contract, not by implicit global state.
...
4. Execute `LearningSystemOrchestrator` behavior for learning subsystem coordination and publication pipeline.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-086 through SPEC-099.
...
4. Run the LSO core transformation.
5. Generate artifacts and EKB records.
6. Validate output against the JSON schema in this SPEC.
7. Release the output to RFC-007 Enterprise, RFC-008 AI Organization, RFC-009 Self-Evolution.
...
- `LearningSystemOrchestrator`: Owns the core learning subsystem coordination and publication pipeline behavior.
...
- Coordinates SPEC-086 through SPEC-099, manages learning lifecycle state, resolves conflicts between learning signals, and publishes validated recommendations to downstream enterprise and self-evolution systems.
...
- Input interface: `LSOInput` JSON object.
- Output interface: `LSOOutput` JSON object.
- Source interface: `src/learning/orchestrator.py`.
- Public class boundary: `LearningSystemOrchestrator`.
...
|---|---|---|
| `LearningSystemOrchestrator.run(payload: dict) -> dict` | Executes the `LearningSystemOrchestrator.run` responsibility for LSO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/orchestrator.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-100-example",
  "spec_id": "SPEC-100",
  ...
  "payload": {
    "engine": "Learning System Orchestrator",
    "source_class": "LearningSystemOrchestrator",
    "public_methods": []
  }
}
```
...
- `AETHERIS_LSO_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_LSO_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_LSO_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/orchestrator.py` implementation is primarily in-memory. The enterprise contract still requires LSO to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. LSO may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `LSO_STARTED` with request ID, spec ID, source path, and upstream references.
- `LSO_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `LSO_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- LSO must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
LSO is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. LSO should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `LearningSystemOrchestrator` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-086 through SPEC-099` | Upstream | Must provide validated evidence before LSO starts. |
| `RFC-007 Enterprise` | Downstream | Must consume only the validated LSO output contract. |
| `RFC-008 AI Organization` | Downstream | Must consume only the validated LSO output contract. |
| `RFC-009 Self-Evolution` | Downstream | Must consume only the validated LSO output contract. |
| `src/learning/orchestrator.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `LearningSystemOrchestrator` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: LSO grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-100-LSO.md
```

Guidance:
- Keep `LearningSystemOrchestrator` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_lso(payload: dict) -> dict:
    input_contract = validate_lso_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = LearningSystemOrchestrator(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_lso_output(input_contract, result)
    validate_lso_output(output)
...
- `LearningSystemOrchestrator.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/orchestrator.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-086 through SPEC-099"] --> I["Input Validator"]
    I --> E["SPEC-100 LSO: Learning System Orchestrator"]
    E --> C["Contract Validator"]
    C --> A["Owned Artifacts and EKB Objects"]
    A --> D["RFC-007 Enterprise"]
```
...
graph LR
    ...
    V --> Core["LearningSystemOrchestrator Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["LSO Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-100"]
    This --> Source["src/learning/orchestrator.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["RFC-007 Enterprise"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute LSO"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-086 through SPEC-099" as U
rectangle "Input Validator" as V
rectangle "SPEC-100 LSO" as E
database "EKB / Artifacts" as K
rectangle "RFC-007 Enterprise" as D
U --> V
V --> E
E --> K
K --> D
...
class "LSOInputContract"
class "LSOOutputContract"
LearningSystemOrchestrator --> "LSOInputContract"
LearningSystemOrchestrator --> "LSOOutputContract"
@enduml
```
