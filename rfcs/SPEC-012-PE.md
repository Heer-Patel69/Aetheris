# SPEC-012: Database Planning Engine (DPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Database schemas, indexes, migrations, backup strategy
...
Primary Class: `DatabasePlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Database Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Designs the database engine, entity schemas, relationships, indexes, migration sequence, multi-tenancy model, and backup posture.

This document upgrades SPEC-012 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-012 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of DPE is to own the full engineering responsibility for database schemas, indexes, migrations, backup strategy. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding DPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct database schemas, indexes, migrations, backup strategy was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for database schemas, indexes, migrations, backup strategy.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-011.
- Emit structured output that can be consumed by SPEC-013.
...
======================================================================
In scope:
- Database schemas, indexes, migrations, backup strategy
- Validation of all DPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-012` input contract to `SPEC-012` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `DatabasePlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
DPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- DPE validates and transforms that evidence.
...
======================================================================
- FR-001: DPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-011.
- FR-002: DPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: DPE shall execute the primary behavior owned by `DatabasePlanningEngine`.
- FR-004: DPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-012 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat DPE output as the authoritative contract for database schemas, indexes, migrations, backup strategy unless a later validation SPEC rejects it.
...
Primary implementation class: `DatabasePlanningEngine`.
...
======================================================================
DPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-012` owns how the engine is implemented.
...
DPE must exchange data by contract, not by implicit global state.
...
4. Execute `DatabasePlanningEngine` behavior for database schemas, indexes, migrations, backup strategy.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-011.
...
- `DatabasePlanningEngine`: Owns the core database schemas, indexes, migrations, backup strategy behavior.
...
- Input interface: `DPEInput` JSON object.
- Output interface: `DPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `DatabasePlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for DPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-012-example",
  "spec_id": "SPEC-012",
  ...
  "payload": {
    "engine": "Database Planning Engine",
    "source_class": "DatabasePlanningEngine",
    "public_methods": [
...
- `AETHERIS_DPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_DPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_DPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. DPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `DPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `DPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `DPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- DPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
DPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. DPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `DatabasePlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before DPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before DPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before DPE starts. |
| `SPEC-011` | Upstream | Must provide validated evidence before DPE starts. |
| `SPEC-013` | Downstream | Must consume only the validated DPE output contract. |
...
| FR-003 core execution | Direct `DatabasePlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: DPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
Guidance:
- Keep `DatabasePlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_dpe(payload: dict) -> dict:
    input_contract = validate_dpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = DatabasePlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_dpe_output(input_contract, result)
    validate_dpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-012 DPE: Database Planning Engine"]
...
graph LR
    ...
    V --> Core["DatabasePlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["DPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute DPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-012 DPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-013" as D
U --> V
V --> E
E --> K
K --> D
...
class "DPEInputContract"
class "DPEOutputContract"
DatabasePlanningEngine --> "DPEInputContract"
DatabasePlanningEngine --> "DPEOutputContract"
@enduml
```
