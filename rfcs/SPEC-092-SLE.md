# SPEC-092: Skill Learning Engine (SLE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Skill performance learning and routing improvement
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/skill_learning.py` (source implementation pending)
Primary Class: `SkillLearningEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Skill Learning Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Learns which skills perform well for which task families, captures skill failure modes, and feeds validated performance signals back to skill selection and evolution engines.

This document upgrades SPEC-092 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-092 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of SLE is to own the full engineering responsibility for skill performance learning and routing improvement. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding SLE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct skill performance learning and routing improvement was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for skill performance learning and routing improvement.
- Accept only validated upstream artifacts from SPEC-034 SSE, SPEC-063 SBE, SPEC-088 BPE2.
- Emit structured output that can be consumed by SPEC-034 SSE, SPEC-062 DSEE.
...
======================================================================
In scope:
- Skill performance learning and routing improvement
- Validation of all SLE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-092` input contract to `SPEC-092` output contract.
- Use `src/learning/skill_learning.py` as the implementation boundary and `SkillLearningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
SLE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- SLE validates and transforms that evidence.
...
======================================================================
- FR-001: SLE shall load the upstream artifacts declared by SPEC-034 SSE, SPEC-063 SBE, SPEC-088 BPE2.
- FR-002: SLE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SLE shall execute the primary behavior owned by `SkillLearningEngine`.
- FR-004: SLE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-092 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SLE output as the authoritative contract for skill performance learning and routing improvement unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/skill_learning.py`.

Primary implementation class: `SkillLearningEngine`.
...
======================================================================
SLE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-092` owns how the engine is implemented.
...
SLE must exchange data by contract, not by implicit global state.
...
4. Execute `SkillLearningEngine` behavior for skill performance learning and routing improvement.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-034 SSE, SPEC-063 SBE, SPEC-088 BPE2.
...
- `SkillLearningEngine`: Owns the core skill performance learning and routing improvement behavior.
...
- Learns which skills perform well for which task families, captures skill failure modes, and feeds validated performance signals back to skill selection and evolution engines.
...
- Input interface: `SLEInput` JSON object.
- Output interface: `SLEOutput` JSON object.
- Source interface: `src/learning/skill_learning.py`.
- Public class boundary: `SkillLearningEngine`.
...
|---|---|---|
| `SkillLearningEngine.run(payload: dict) -> dict` | Executes the `SkillLearningEngine.run` responsibility for SLE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/skill_learning.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-092-example",
  "spec_id": "SPEC-092",
  ...
  "payload": {
    "engine": "Skill Learning Engine",
    "source_class": "SkillLearningEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_SLE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_SLE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_SLE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/skill_learning.py` implementation is primarily in-memory. The enterprise contract still requires SLE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. SLE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `SLE_STARTED` with request ID, spec ID, source path, and upstream references.
- `SLE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `SLE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- SLE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
SLE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. SLE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `SkillLearningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-034 SSE` | Upstream | Must provide validated evidence before SLE starts. |
| `SPEC-063 SBE` | Upstream | Must provide validated evidence before SLE starts. |
| `SPEC-088 BPE2` | Upstream | Must provide validated evidence before SLE starts. |
| `SPEC-034 SSE` | Downstream | Must consume only the validated SLE output contract. |
| `SPEC-062 DSEE` | Downstream | Must consume only the validated SLE output contract. |
| `src/learning/skill_learning.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `SkillLearningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: SLE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/skill_learning.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-092-SLE.md
```

Guidance:
- Keep `SkillLearningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_sle(payload: dict) -> dict:
    input_contract = validate_sle_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = SkillLearningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_sle_output(input_contract, result)
    validate_sle_output(output)
...
- `SkillLearningEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/skill_learning.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-034 SSE"] --> I["Input Validator"]
    I --> E["SPEC-092 SLE: Skill Learning Engine"]
...
graph LR
    ...
    V --> Core["SkillLearningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["SLE Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-092"]
    This --> Source["src/learning/skill_learning.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-034 SSE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute SLE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-034 SSE" as U
rectangle "Input Validator" as V
rectangle "SPEC-092 SLE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-034 SSE" as D
U --> V
V --> E
E --> K
K --> D
...
class "SLEInputContract"
class "SLEOutputContract"
SkillLearningEngine --> "SLEInputContract"
SkillLearningEngine --> "SLEOutputContract"
@enduml
```
