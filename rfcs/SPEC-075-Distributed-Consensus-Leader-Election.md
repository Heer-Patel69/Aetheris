# SPEC-075: Distributed Consensus Leader Election (DCLE)
...
Implementation: `src/runtime/consensus.py` (source implementation pending)
Primary Class: `DistributedConsensusLeaderElection`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Distributed Consensus Leader Election exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Provides consensus and leader election for cluster coordination, exclusive locks, failover decisions, and runtime orchestration ownership.

This document upgrades SPEC-075 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-075 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of DCLE is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding DCLE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-073 CLM, SPEC-074 NDCR.
...
- Validation of all DCLE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-075` input contract to `SPEC-075` output contract.
- Use `src/runtime/consensus.py` as the implementation boundary and `DistributedConsensusLeaderElection` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
DCLE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- DCLE validates and transforms that evidence.
...
======================================================================
- FR-001: DCLE shall load the upstream artifacts declared by SPEC-073 CLM, SPEC-074 NDCR.
- FR-002: DCLE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: DCLE shall execute the primary behavior owned by `DistributedConsensusLeaderElection`.
- FR-004: DCLE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-075 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat DCLE output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/consensus.py`.

Primary implementation class: `DistributedConsensusLeaderElection`.
...
======================================================================
DCLE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-075` owns how the engine is implemented.
...
DCLE must exchange data by contract, not by implicit global state.
...
4. Execute `DistributedConsensusLeaderElection` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-073 CLM, SPEC-074 NDCR.
...
4. Run the DCLE core transformation.
...
- `DistributedConsensusLeaderElection`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Input interface: `DCLEInput` JSON object.
- Output interface: `DCLEOutput` JSON object.
- Source interface: `src/runtime/consensus.py`.
- Public class boundary: `DistributedConsensusLeaderElection`.
...
|---|---|---|
| `DistributedConsensusLeaderElection.run(payload: dict) -> dict` | Executes the `DistributedConsensusLeaderElection.run` responsibility for DCLE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/runtime/consensus.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-075-example",
  "spec_id": "SPEC-075",
  ...
  "payload": {
    "engine": "Distributed Consensus Leader Election",
    "source_class": "DistributedConsensusLeaderElection",
    "public_methods": []
  }
}
```
...
- `AETHERIS_DCLE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_DCLE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_DCLE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/consensus.py` implementation is primarily in-memory. The enterprise contract still requires DCLE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. DCLE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `DCLE_STARTED` with request ID, spec ID, source path, and upstream references.
- `DCLE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `DCLE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- DCLE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
DCLE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. DCLE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `DistributedConsensusLeaderElection` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-073 CLM` | Upstream | Must provide validated evidence before DCLE starts. |
| `SPEC-074 NDCR` | Upstream | Must provide validated evidence before DCLE starts. |
| `SPEC-085 GRO` | Downstream | Must consume only the validated DCLE output contract. |
| `src/runtime/consensus.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `DistributedConsensusLeaderElection` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: DCLE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/consensus.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-075-Distributed-Consensus-Leader-Election.md
```

Guidance:
- Keep `DistributedConsensusLeaderElection` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_dcle(payload: dict) -> dict:
    input_contract = validate_dcle_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = DistributedConsensusLeaderElection(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_dcle_output(input_contract, result)
    validate_dcle_output(output)
...
- `DistributedConsensusLeaderElection.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/consensus.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-075 DCLE: Distributed Consensus Leader Election"]
...
graph LR
    ...
    V --> Core["DistributedConsensusLeaderElection Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["DCLE Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-075"]
    This --> Source["src/runtime/consensus.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute DCLE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
class DistributedConsensusLeaderElection {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "DCLEInputContract"
class "DCLEOutputContract"
DistributedConsensusLeaderElection --> "DCLEInputContract"
DistributedConsensusLeaderElection --> "DCLEOutputContract"
@enduml
```
