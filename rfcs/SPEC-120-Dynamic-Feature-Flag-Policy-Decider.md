# SPEC-120: Dynamic Feature Flag & Policy Decider (DFFP)
...
Primary Class: `FeatureFlagPolicyDecider`
Test Reference: `tests/test_rfc007_core.py`

======================================================================
1. PURPOSE & BUSINESS VALUE
======================================================================
The Dynamic Feature Flag & Policy Decider (DFFP) exists to provide a foundational enterprise capability for Aetheris. It ensures secure, production-scale, multi-tenant operations, maintaining compliance, security boundaries, and strict resource isolation.
...
======================================================================
- FR-101: The engine shall load configurations conforming to the `SPEC-120Input` schema.
- FR-102: The engine shall authorize all operations against tenant context parameters.
- FR-103: The engine shall execute the core `FeatureFlagPolicyDecider` behaviors.
- FR-104: The engine shall output structured results conforming to the `SPEC-120Output` schema.
...
======================================================================
Primary Path: `src/enterprise/feature_flags.py`.
The class `FeatureFlagPolicyDecider` acts as the main domain service layer, validating API structures, interacting with local storage adapters, and dispatching transaction logs to the audit service.
...
- `FeatureFlagPolicyDeciderCore`: Executes subsystem transformations.
...
3. Call `FeatureFlagPolicyDecider` core behaviors.
...
======================================================================
Primary logic is defined in `src/enterprise/feature_flags.py`. Sub-modules handle encryption, parsing, and database schemas.
...
|---|---|---|
| `is_feature_enabled(tenant_id: str, user_id: str, feature_key: str) -> bool` | Checks if the requested enterprise feature is enabled for the context. | Validate input, enforce security boundaries, return deterministic output, and emit telemetry. |
...
18. OBSERVABILITY
======================================================================
Structured logging includes standard event codes like `DFFP_STARTED`, `DFFP_SUCCESS`, and `DFFP_FAILED` with request ID attributes.
