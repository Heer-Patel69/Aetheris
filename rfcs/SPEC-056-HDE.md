# SPEC-056: Hallucination Detection Engine (HDE)
...
Implementation: `src/intelligence/hde.py` (validated from source metadata)
Primary Class: `HallucinationDetectionEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Hallucination Detection Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Scans generated text for unsupported claims, unverifiable dependencies, invented APIs, and assertion drift before output is accepted.

This document upgrades SPEC-056 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-056 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of HDE is to own the full engineering responsibility for wave 2 - knowledge intelligence. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding HDE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-055 FVE, SPEC-050 ERE.
- Emit structured output that can be consumed by SPEC-051 SRE.
...
- Validation of all HDE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-056` input contract to `SPEC-056` output contract.
- Use `src/intelligence/hde.py` as the implementation boundary and `HallucinationDetectionEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
HDE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- HDE validates and transforms that evidence.
...
======================================================================
- FR-001: HDE shall load the upstream artifacts declared by SPEC-055 FVE, SPEC-050 ERE.
- FR-002: HDE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: HDE shall execute the primary behavior owned by `HallucinationDetectionEngine`.
- FR-004: HDE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-056 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat HDE output as the authoritative contract for wave 2 - knowledge intelligence unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/hde.py`.

Primary implementation class: `HallucinationDetectionEngine`.
...
======================================================================
HDE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-056` owns how the engine is implemented.
...
HDE must exchange data by contract, not by implicit global state.
...
4. Execute `HallucinationDetectionEngine` behavior for wave 2 - knowledge intelligence.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-055 FVE, SPEC-050 ERE.
...
- `HallucinationDetectionEngine`: Owns the core wave 2 - knowledge intelligence behavior.
...
- Scans generated text for unsupported claims, unverifiable dependencies, invented APIs, and assertion drift before output is accepted.
...
- Input interface: `HDEInput` JSON object.
- Output interface: `HDEOutput` JSON object.
- Source interface: `src/intelligence/hde.py`.
- Public class boundary: `HallucinationDetectionEngine`.
...
|---|---|---|
| `scan_for_hallucinations(text: str) -> list` | Executes the `scan_for_hallucinations` responsibility for HDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-056-example",
  "spec_id": "SPEC-056",
  ...
  "payload": {
    "engine": "Hallucination Detection Engine",
    "source_class": "HallucinationDetectionEngine",
    "public_methods": [
      "scan_for_hallucinations(text: str) -> list"
    ]
  }
}
```
...
- `AETHERIS_HDE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_HDE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_HDE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/hde.py` implementation is primarily in-memory. The enterprise contract still requires HDE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. HDE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `HDE_STARTED` with request ID, spec ID, source path, and upstream references.
- `HDE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `HDE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- HDE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
HDE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. HDE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `HallucinationDetectionEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-055 FVE` | Upstream | Must provide validated evidence before HDE starts. |
| `SPEC-050 ERE` | Upstream | Must provide validated evidence before HDE starts. |
| `SPEC-051 SRE` | Downstream | Must consume only the validated HDE output contract. |
| `src/intelligence/hde.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `HallucinationDetectionEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: HDE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-056-HDE.md
```

Guidance:
- Keep `HallucinationDetectionEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_hde(payload: dict) -> dict:
    input_contract = validate_hde_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = HallucinationDetectionEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_hde_output(input_contract, result)
    validate_hde_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/hde.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-055 FVE"] --> I["Input Validator"]
    I --> E["SPEC-056 HDE: Hallucination Detection Engine"]
...
graph LR
    ...
    V --> Core["HallucinationDetectionEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["HDE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-056"]
    This --> Source["src/intelligence/hde.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-051 SRE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute HDE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-055 FVE" as U
rectangle "Input Validator" as V
rectangle "SPEC-056 HDE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-051 SRE" as D
U --> V
V --> E
E --> K
K --> D
...
class "HDEInputContract"
class "HDEOutputContract"
HallucinationDetectionEngine --> "HDEInputContract"
HallucinationDetectionEngine --> "HDEOutputContract"
@enduml
```
