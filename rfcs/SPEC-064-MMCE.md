# SPEC-064: Multi-Model Consensus Engine (MMCE)
...
Implementation: `src/intelligence/mmce.py` (validated from source metadata)
Primary Class: `MultiModelConsensusEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Multi-Model Consensus Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Combines candidate model outputs, resolves consensus, estimates confidence, and flags disagreement requiring verification or human review.

This document upgrades SPEC-064 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-064 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of MMCE is to own the full engineering responsibility for wave 4 - evolution. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding MMCE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-047 MIE, SPEC-050 ERE, SPEC-063 SBE.
- Emit structured output that can be consumed by SPEC-065 IO, SPEC-055 FVE.
...
In scope:
- Wave 4 - Evolution
- Validation of all MMCE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-064` input contract to `SPEC-064` output contract.
- Use `src/intelligence/mmce.py` as the implementation boundary and `MultiModelConsensusEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
MMCE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- MMCE validates and transforms that evidence.
...
======================================================================
- FR-001: MMCE shall load the upstream artifacts declared by SPEC-047 MIE, SPEC-050 ERE, SPEC-063 SBE.
- FR-002: MMCE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: MMCE shall execute the primary behavior owned by `MultiModelConsensusEngine`.
- FR-004: MMCE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-064 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat MMCE output as the authoritative contract for wave 4 - evolution unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/mmce.py`.

Primary implementation class: `MultiModelConsensusEngine`.
...
======================================================================
MMCE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-064` owns how the engine is implemented.
...
MMCE must exchange data by contract, not by implicit global state.
...
4. Execute `MultiModelConsensusEngine` behavior for wave 4 - evolution.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-047 MIE, SPEC-050 ERE, SPEC-063 SBE.
...
- `MultiModelConsensusEngine`: Owns the core wave 4 - evolution behavior.
...
- Combines candidate model outputs, resolves consensus, estimates confidence, and flags disagreement requiring verification or human review.
...
- Input interface: `MMCEInput` JSON object.
- Output interface: `MMCEOutput` JSON object.
- Source interface: `src/intelligence/mmce.py`.
- Public class boundary: `MultiModelConsensusEngine`.
...
|---|---|---|
| `resolve_consensus(outputs: list) -> dict` | Executes the `resolve_consensus` responsibility for MMCE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/intelligence/mmce.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-064-example",
  "spec_id": "SPEC-064",
  ...
  "payload": {
    "engine": "Multi-Model Consensus Engine",
    "source_class": "MultiModelConsensusEngine",
    "public_methods": [
      "resolve_consensus(outputs: list) -> dict"
    ]
  }
}
```
...
- `AETHERIS_MMCE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_MMCE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_MMCE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/mmce.py` implementation is primarily in-memory. The enterprise contract still requires MMCE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. MMCE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `MMCE_STARTED` with request ID, spec ID, source path, and upstream references.
- `MMCE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `MMCE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- MMCE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
MMCE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. MMCE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `MultiModelConsensusEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-047 MIE` | Upstream | Must provide validated evidence before MMCE starts. |
| `SPEC-050 ERE` | Upstream | Must provide validated evidence before MMCE starts. |
| `SPEC-063 SBE` | Upstream | Must provide validated evidence before MMCE starts. |
| `SPEC-065 IO` | Downstream | Must consume only the validated MMCE output contract. |
| `SPEC-055 FVE` | Downstream | Must consume only the validated MMCE output contract. |
| `src/intelligence/mmce.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `MultiModelConsensusEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: MMCE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/mmce.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-064-MMCE.md
```

Guidance:
- Keep `MultiModelConsensusEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_mmce(payload: dict) -> dict:
    input_contract = validate_mmce_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = MultiModelConsensusEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_mmce_output(input_contract, result)
    validate_mmce_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/mmce.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-064 MMCE: Multi-Model Consensus Engine"]
...
graph LR
    ...
    V --> Core["MultiModelConsensusEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["MMCE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-064"]
    This --> Source["src/intelligence/mmce.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-065 IO"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute MMCE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-064 MMCE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-065 IO" as D
U --> V
V --> E
E --> K
K --> D
...
class "MMCEInputContract"
class "MMCEOutputContract"
MultiModelConsensusEngine --> "MMCEInputContract"
MultiModelConsensusEngine --> "MMCEOutputContract"
@enduml
```
