# SPEC-032: Task Decomposition Engine (TDE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Blueprint-to-task decomposition
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/tde.py` (validated from source metadata)
Primary Class: `TaskDecompositionEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Task Decomposition Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Decomposes the engineering blueprint into atomic tasks, modules, stories, subtasks, effort estimates, and execution tree exports.

This document upgrades SPEC-032 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-032 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of TDE is to own the full engineering responsibility for blueprint-to-task decomposition. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding TDE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct blueprint-to-task decomposition was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for blueprint-to-task decomposition.
- Accept only validated upstream artifacts from SPEC-030 FEBC.
- Emit structured output that can be consumed by SPEC-033.
...
======================================================================
In scope:
- Blueprint-to-task decomposition
- Validation of all TDE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-032` input contract to `SPEC-032` output contract.
- Use `src/execution/tde.py` as the implementation boundary and `TaskDecompositionEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
TDE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- TDE validates and transforms that evidence.
...
======================================================================
- FR-001: TDE shall load the upstream artifacts declared by SPEC-030 FEBC.
- FR-002: TDE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: TDE shall execute the primary behavior owned by `TaskDecompositionEngine`.
- FR-004: TDE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-032 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat TDE output as the authoritative contract for blueprint-to-task decomposition unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/tde.py`.

Primary implementation class: `TaskDecompositionEngine`.
...
======================================================================
TDE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-032` owns how the engine is implemented.
...
TDE must exchange data by contract, not by implicit global state.
...
4. Execute `TaskDecompositionEngine` behavior for blueprint-to-task decomposition.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-030 FEBC.
...
- `TaskDecompositionEngine`: Owns the core blueprint-to-task decomposition behavior.
...
- Input interface: `TDEInput` JSON object.
- Output interface: `TDEOutput` JSON object.
- Source interface: `src/execution/tde.py`.
- Public class boundary: `TaskDecompositionEngine`.
...
|---|---|---|
| `generate_tasks(blueprint: dict) -> dict` | Executes the `generate_tasks` responsibility for TDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `split_task(task_id: str, criteria: str) -> List[dict]` | Executes the `split_task` responsibility for TDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `merge_tasks(task_ids: List[str]) -> dict` | Executes the `merge_tasks` responsibility for TDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `validate_task(task: dict) -> bool` | Executes the `validate_task` responsibility for TDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `estimate_effort(task: dict) -> float` | Executes the `estimate_effort` responsibility for TDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `export_execution_tree() -> dict` | Executes the `export_execution_tree` responsibility for TDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-032-example",
  "spec_id": "SPEC-032",
  ...
  "payload": {
    "engine": "Task Decomposition Engine",
    "source_class": "TaskDecompositionEngine",
    "public_methods": [
      "generate_tasks(blueprint: dict) -> dict",
      "split_task(task_id: str, criteria: str) -> List[dict]",
      "merge_tasks(task_ids: List[str]) -> dict",
      "validate_task(task: dict) -> bool",
      "estimate_effort(task: dict) -> float"
    ]
  }
}
```
...
- `AETHERIS_TDE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_TDE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_TDE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. TDE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `TDE_STARTED` with request ID, spec ID, source path, and upstream references.
- `TDE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `TDE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- TDE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
TDE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. TDE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `TaskDecompositionEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-030 FEBC` | Upstream | Must provide validated evidence before TDE starts. |
| `SPEC-033` | Downstream | Must consume only the validated TDE output contract. |
| `src/execution/tde.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `TaskDecompositionEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: TDE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-032-TDE.md
```

Guidance:
- Keep `TaskDecompositionEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_tde(payload: dict) -> dict:
    input_contract = validate_tde_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = TaskDecompositionEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_tde_output(input_contract, result)
    validate_tde_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/tde.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-030 FEBC"] --> I["Input Validator"]
    I --> E["SPEC-032 TDE: Task Decomposition Engine"]
...
graph LR
    ...
    V --> Core["TaskDecompositionEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["TDE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-032"]
    This --> Source["src/execution/tde.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute TDE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-030 FEBC" as U
rectangle "Input Validator" as V
rectangle "SPEC-032 TDE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-033" as D
U --> V
V --> E
E --> K
K --> D
...
class "TDEInputContract"
class "TDEOutputContract"
TaskDecompositionEngine --> "TDEInputContract"
TaskDecompositionEngine --> "TDEOutputContract"
@enduml
```
