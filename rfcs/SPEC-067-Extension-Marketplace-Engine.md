# SPEC-067: Extension Marketplace Engine (EME2)
...
Implementation: `src/marketplace/engine.py` (source implementation pending)
Primary Class: `ExtensionMarketplaceEngine`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Extension Marketplace Engine exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Manages extension discovery, trust metadata, publishing workflows, compatibility scoring, and marketplace governance for Aetheris plugins.

This document upgrades SPEC-067 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-067 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of EME2 is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Accept only validated upstream artifacts from SPEC-066 PSE, extension metadata, trust policies.
- Emit structured output that can be consumed by SPEC-068 RPL.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-067` input contract to `SPEC-067` output contract.
- Use `src/marketplace/engine.py` as the implementation boundary and `ExtensionMarketplaceEngine` as the primary class boundary.
...
======================================================================
- FR-001: EME2 shall load the upstream artifacts declared by SPEC-066 PSE, extension metadata, trust policies.
- FR-002: EME2 shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EME2 shall execute the primary behavior owned by `ExtensionMarketplaceEngine`.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-067 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EME2 output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/marketplace/engine.py`.

Primary implementation class: `ExtensionMarketplaceEngine`.

Implementation classes in scope:
- `ExtensionMarketplaceEngine`
...
- `SPEC-067` owns how the engine is implemented.
...
4. Execute `ExtensionMarketplaceEngine` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-066 PSE, extension metadata, trust policies.
...
7. Release the output to SPEC-068 RPL.
...
- `ExtensionMarketplaceEngine`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Manages extension discovery, trust metadata, publishing workflows, compatibility scoring, and marketplace governance for Aetheris plugins.
...
- Source interface: `src/marketplace/engine.py`.
- Public class boundary: `ExtensionMarketplaceEngine`.
...
|---|---|---|
| `ExtensionMarketplaceEngine.run(payload: dict) -> dict` | Executes the `ExtensionMarketplaceEngine.run` responsibility for EME2. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/marketplace/engine.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-067-example",
  "spec_id": "SPEC-067",
  ...
  "payload": {
    "engine": "Extension Marketplace Engine",
    "source_class": "ExtensionMarketplaceEngine",
    "public_methods": []
  }
}
```
...
======================================================================
The current `src/marketplace/engine.py` implementation is primarily in-memory. The enterprise contract still requires EME2 to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
- Unit tests should instantiate `ExtensionMarketplaceEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-066 PSE` | Upstream | Must provide validated evidence before EME2 starts. |
| `extension metadata` | Upstream | Must provide validated evidence before EME2 starts. |
| `trust policies` | Upstream | Must provide validated evidence before EME2 starts. |
| `SPEC-068 RPL` | Downstream | Must consume only the validated EME2 output contract. |
| `src/marketplace/engine.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ExtensionMarketplaceEngine` public API tests |
...
```text
src/marketplace/engine.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-067-Extension-Marketplace-Engine.md
```

Guidance:
- Keep `ExtensionMarketplaceEngine` as the primary orchestration class for this SPEC.
...
def run_eme2(payload: dict) -> dict:
    ...
    result = ExtensionMarketplaceEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
...
- `ExtensionMarketplaceEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/marketplace/engine.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-066 PSE"] --> I["Input Validator"]
    I --> E["SPEC-067 EME2: Extension Marketplace Engine"]
...
graph LR
    ...
    V --> Core["ExtensionMarketplaceEngine Core"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-067"]
    This --> Source["src/marketplace/engine.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-068 RPL"]
```
...
```plantuml
@startuml
rectangle "SPEC-066 PSE" as U
rectangle "Input Validator" as V
rectangle "SPEC-067 EME2" as E
database "EKB / Artifacts" as K
rectangle "SPEC-068 RPL" as D
U --> V
V --> E
E --> K
K --> D
...
```plantuml
@startuml
class ExtensionMarketplaceEngine {
  +execute(input)
...
ExtensionMarketplaceEngine --> "EME2InputContract"
ExtensionMarketplaceEngine --> "EME2OutputContract"
@enduml
```
