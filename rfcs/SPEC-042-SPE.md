# SPEC-042: State Persistence Engine (SPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Checkpointing and resumable execution state
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/spe.py` (validated from source metadata)
Primary Class: `StatePersistenceEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
State Persistence Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Serializes checkpoints, snapshots, resume metadata, active task state, and blueprint version for crash-safe execution continuation.

This document upgrades SPEC-042 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-042 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of SPE is to own the full engineering responsibility for checkpointing and resumable execution state. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding SPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct checkpointing and resumable execution state was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for checkpointing and resumable execution state.
- Accept only validated upstream artifacts from SPEC-041.
- Emit structured output that can be consumed by SPEC-043.
...
======================================================================
In scope:
- Checkpointing and resumable execution state
- Validation of all SPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-042` input contract to `SPEC-042` output contract.
- Use `src/execution/spe.py` as the implementation boundary and `StatePersistenceEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
SPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- SPE validates and transforms that evidence.
...
======================================================================
- FR-001: SPE shall load the upstream artifacts declared by SPEC-041.
- FR-002: SPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SPE shall execute the primary behavior owned by `StatePersistenceEngine`.
- FR-004: SPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-042 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SPE output as the authoritative contract for checkpointing and resumable execution state unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/spe.py`.

Primary implementation class: `StatePersistenceEngine`.
...
======================================================================
SPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-042` owns how the engine is implemented.
...
SPE must exchange data by contract, not by implicit global state.
...
4. Execute `StatePersistenceEngine` behavior for checkpointing and resumable execution state.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-041.
...
- `StatePersistenceEngine`: Owns the core checkpointing and resumable execution state behavior.
...
- Input interface: `SPEInput` JSON object.
- Output interface: `SPEOutput` JSON object.
- Source interface: `src/execution/spe.py`.
- Public class boundary: `StatePersistenceEngine`.
...
|---|---|---|
| `checkpoint(state: dict) -> dict` | Executes the `checkpoint` responsibility for SPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `resume() -> dict` | Executes the `resume` responsibility for SPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `restore(checkpoint_id: str) -> bool` | Executes the `restore` responsibility for SPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `save_state(state: dict) -> bool` | Executes the `save_state` responsibility for SPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `load_state() -> dict` | Executes the `load_state` responsibility for SPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `validate_checkpoint(snapshot: dict) -> bool` | Executes the `validate_checkpoint` responsibility for SPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-042-example",
  "spec_id": "SPEC-042",
  ...
  "payload": {
    "engine": "State Persistence Engine",
    "source_class": "StatePersistenceEngine",
    "public_methods": [
      "checkpoint(state: dict) -> dict",
      "resume() -> dict",
      "restore(checkpoint_id: str) -> bool",
...
- `AETHERIS_SPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_SPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_SPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. SPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `SPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `SPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `SPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- SPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
SPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. SPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `StatePersistenceEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-041` | Upstream | Must provide validated evidence before SPE starts. |
| `SPEC-043` | Downstream | Must consume only the validated SPE output contract. |
| `src/execution/spe.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `StatePersistenceEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: SPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-042-SPE.md
```

Guidance:
- Keep `StatePersistenceEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_spe(payload: dict) -> dict:
    input_contract = validate_spe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = StatePersistenceEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_spe_output(input_contract, result)
    validate_spe_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/spe.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-041"] --> I["Input Validator"]
    I --> E["SPEC-042 SPE: State Persistence Engine"]
...
graph LR
    ...
    V --> Core["StatePersistenceEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["SPE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-042"]
    This --> Source["src/execution/spe.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute SPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-041" as U
rectangle "Input Validator" as V
rectangle "SPEC-042 SPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-043" as D
U --> V
V --> E
E --> K
K --> D
...
class "SPEInputContract"
class "SPEOutputContract"
StatePersistenceEngine --> "SPEInputContract"
StatePersistenceEngine --> "SPEOutputContract"
@enduml
```
