# SPEC-057: Planning Optimization Engine (PLE)
...
Implementation: `src/intelligence/ple.py` (validated from source metadata)
Primary Class: `PlanningOptimizationEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Planning Optimization Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Optimizes raw plans by preserving required steps, reducing redundant work, and flagging planning gaps before execution.

This document upgrades SPEC-057 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-057 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of PLE is to own the full engineering responsibility for wave 3 - optimization. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PLE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-030 FEBC, SPEC-051 SRE.
- Emit structured output that can be consumed by SPEC-058 TOE, SPEC-060 EOE.
...
In scope:
- Wave 3 - Optimization
- Validation of all PLE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-057` input contract to `SPEC-057` output contract.
- Use `src/intelligence/ple.py` as the implementation boundary and `PlanningOptimizationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PLE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PLE validates and transforms that evidence.
...
======================================================================
- FR-001: PLE shall load the upstream artifacts declared by SPEC-030 FEBC, SPEC-051 SRE.
- FR-002: PLE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PLE shall execute the primary behavior owned by `PlanningOptimizationEngine`.
- FR-004: PLE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-057 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PLE output as the authoritative contract for wave 3 - optimization unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/ple.py`.

Primary implementation class: `PlanningOptimizationEngine`.
...
======================================================================
PLE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-057` owns how the engine is implemented.
...
PLE must exchange data by contract, not by implicit global state.
...
4. Execute `PlanningOptimizationEngine` behavior for wave 3 - optimization.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-030 FEBC, SPEC-051 SRE.
...
- `PlanningOptimizationEngine`: Owns the core wave 3 - optimization behavior.
...
- Optimizes raw plans by preserving required steps, reducing redundant work, and flagging planning gaps before execution.
...
- Input interface: `PLEInput` JSON object.
- Output interface: `PLEOutput` JSON object.
- Source interface: `src/intelligence/ple.py`.
- Public class boundary: `PlanningOptimizationEngine`.
...
|---|---|---|
| `optimize_plan(raw_plan: dict) -> dict` | Executes the `optimize_plan` responsibility for PLE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-057-example",
  "spec_id": "SPEC-057",
  ...
  "payload": {
    "engine": "Planning Optimization Engine",
    "source_class": "PlanningOptimizationEngine",
    "public_methods": [
      "optimize_plan(raw_plan: dict) -> dict"
    ]
  }
}
```
...
- `AETHERIS_PLE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PLE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PLE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/ple.py` implementation is primarily in-memory. The enterprise contract still requires PLE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. PLE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PLE_STARTED` with request ID, spec ID, source path, and upstream references.
- `PLE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PLE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PLE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PLE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PLE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `PlanningOptimizationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-030 FEBC` | Upstream | Must provide validated evidence before PLE starts. |
| `SPEC-051 SRE` | Upstream | Must provide validated evidence before PLE starts. |
| `SPEC-058 TOE` | Downstream | Must consume only the validated PLE output contract. |
| `SPEC-060 EOE` | Downstream | Must consume only the validated PLE output contract. |
| `src/intelligence/ple.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `PlanningOptimizationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PLE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/ple.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-057-PLE.md
```

Guidance:
- Keep `PlanningOptimizationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ple(payload: dict) -> dict:
    input_contract = validate_ple_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = PlanningOptimizationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ple_output(input_contract, result)
    validate_ple_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/ple.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-057 PLE: Planning Optimization Engine"]
...
graph LR
    ...
    V --> Core["PlanningOptimizationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PLE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-057"]
    This --> Source["src/intelligence/ple.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-058 TOE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PLE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-057 PLE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-058 TOE" as D
U --> V
V --> E
E --> K
K --> D
...
class "PLEInputContract"
class "PLEOutputContract"
PlanningOptimizationEngine --> "PLEInputContract"
PlanningOptimizationEngine --> "PLEOutputContract"
@enduml
```
