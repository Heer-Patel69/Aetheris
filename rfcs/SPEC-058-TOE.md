# SPEC-058: Token Optimization Engine (TOE)
...
Implementation: `src/intelligence/toe.py` (validated from source metadata)
Primary Class: `TokenOptimizationEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Token Optimization Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Compresses prompt and context tokens while preserving semantics, constraints, file references, schema names, and required implementation details.

This document upgrades SPEC-058 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-058 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of TOE is to own the full engineering responsibility for wave 3 - optimization. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding TOE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-052 LCE, SPEC-057 PLE.
- Emit structured output that can be consumed by SPEC-059 COE, SPEC-061 COE2.
...
In scope:
- Wave 3 - Optimization
- Validation of all TOE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-058` input contract to `SPEC-058` output contract.
- Use `src/intelligence/toe.py` as the implementation boundary and `TokenOptimizationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
TOE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- TOE validates and transforms that evidence.
...
======================================================================
- FR-001: TOE shall load the upstream artifacts declared by SPEC-052 LCE, SPEC-057 PLE.
- FR-002: TOE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: TOE shall execute the primary behavior owned by `TokenOptimizationEngine`.
- FR-004: TOE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-058 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat TOE output as the authoritative contract for wave 3 - optimization unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/toe.py`.

Primary implementation class: `TokenOptimizationEngine`.
...
======================================================================
TOE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-058` owns how the engine is implemented.
...
TOE must exchange data by contract, not by implicit global state.
...
4. Execute `TokenOptimizationEngine` behavior for wave 3 - optimization.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-052 LCE, SPEC-057 PLE.
...
- `TokenOptimizationEngine`: Owns the core wave 3 - optimization behavior.
...
- Compresses prompt and context tokens while preserving semantics, constraints, file references, schema names, and required implementation details.
...
- Input interface: `TOEInput` JSON object.
- Output interface: `TOEOutput` JSON object.
- Source interface: `src/intelligence/toe.py`.
- Public class boundary: `TokenOptimizationEngine`.
...
|---|---|---|
| `compress_tokens(text: str) -> str` | Executes the `compress_tokens` responsibility for TOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-058-example",
  "spec_id": "SPEC-058",
  ...
  "payload": {
    "engine": "Token Optimization Engine",
    "source_class": "TokenOptimizationEngine",
    "public_methods": [
      "compress_tokens(text: str) -> str"
    ]
  }
}
```
...
- `AETHERIS_TOE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_TOE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_TOE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/toe.py` implementation is primarily in-memory. The enterprise contract still requires TOE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. TOE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `TOE_STARTED` with request ID, spec ID, source path, and upstream references.
- `TOE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `TOE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- TOE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
TOE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. TOE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `TokenOptimizationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-052 LCE` | Upstream | Must provide validated evidence before TOE starts. |
| `SPEC-057 PLE` | Upstream | Must provide validated evidence before TOE starts. |
| `SPEC-059 COE` | Downstream | Must consume only the validated TOE output contract. |
| `SPEC-061 COE2` | Downstream | Must consume only the validated TOE output contract. |
| `src/intelligence/toe.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `TokenOptimizationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: TOE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/toe.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-058-TOE.md
```

Guidance:
- Keep `TokenOptimizationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_toe(payload: dict) -> dict:
    input_contract = validate_toe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = TokenOptimizationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_toe_output(input_contract, result)
    validate_toe_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/toe.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-058 TOE: Token Optimization Engine"]
...
graph LR
    ...
    V --> Core["TokenOptimizationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["TOE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-058"]
    This --> Source["src/intelligence/toe.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-059 COE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute TOE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-058 TOE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-059 COE" as D
U --> V
V --> E
E --> K
K --> D
...
class "TOEInputContract"
class "TOEOutputContract"
TokenOptimizationEngine --> "TOEInputContract"
TokenOptimizationEngine --> "TOEOutputContract"
@enduml
```
