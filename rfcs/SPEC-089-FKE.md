# SPEC-089: Failure Knowledge Engine (FKE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Failure memory, root-cause capture, and recurrence prevention
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/failure_knowledge.py` (source implementation pending)
Primary Class: `FailureKnowledgeEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Failure Knowledge Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Turns failures, retries, rollbacks, failed tests, and recovery decisions into searchable knowledge that prevents repeated mistakes and improves future recovery choices.

This document upgrades SPEC-089 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-089 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of FKE is to own the full engineering responsibility for failure memory, root-cause capture, and recurrence prevention. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding FKE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct failure memory, root-cause capture, and recurrence prevention was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for failure memory, root-cause capture, and recurrence prevention.
- Accept only validated upstream artifacts from SPEC-086 EME2, SPEC-041 PRE, SPEC-042 SPE.
- Emit structured output that can be consumed by SPEC-095 RLE, SPEC-098 LAE.
...
======================================================================
In scope:
- Failure memory, root-cause capture, and recurrence prevention
- Validation of all FKE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-089` input contract to `SPEC-089` output contract.
- Use `src/learning/failure_knowledge.py` as the implementation boundary and `FailureKnowledgeEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
FKE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- FKE validates and transforms that evidence.
...
======================================================================
- FR-001: FKE shall load the upstream artifacts declared by SPEC-086 EME2, SPEC-041 PRE, SPEC-042 SPE.
- FR-002: FKE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: FKE shall execute the primary behavior owned by `FailureKnowledgeEngine`.
- FR-004: FKE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-089 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat FKE output as the authoritative contract for failure memory, root-cause capture, and recurrence prevention unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/failure_knowledge.py`.

Primary implementation class: `FailureKnowledgeEngine`.

Implementation classes in scope:
- `FailureKnowledgeEngine`
...
======================================================================
FKE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-089` owns how the engine is implemented.
...
FKE must exchange data by contract, not by implicit global state.
...
4. Execute `FailureKnowledgeEngine` behavior for failure memory, root-cause capture, and recurrence prevention.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-086 EME2, SPEC-041 PRE, SPEC-042 SPE.
...
- `FailureKnowledgeEngine`: Owns the core failure memory, root-cause capture, and recurrence prevention behavior.
...
- Turns failures, retries, rollbacks, failed tests, and recovery decisions into searchable knowledge that prevents repeated mistakes and improves future recovery choices.
...
- Input interface: `FKEInput` JSON object.
- Output interface: `FKEOutput` JSON object.
- Source interface: `src/learning/failure_knowledge.py`.
- Public class boundary: `FailureKnowledgeEngine`.
...
|---|---|---|
| `FailureKnowledgeEngine.run(payload: dict) -> dict` | Executes the `FailureKnowledgeEngine.run` responsibility for FKE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/failure_knowledge.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-089-example",
  "spec_id": "SPEC-089",
  ...
  "payload": {
    "engine": "Failure Knowledge Engine",
    "source_class": "FailureKnowledgeEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_FKE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_FKE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_FKE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/failure_knowledge.py` implementation is primarily in-memory. The enterprise contract still requires FKE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. FKE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `FKE_STARTED` with request ID, spec ID, source path, and upstream references.
- `FKE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `FKE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- FKE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
FKE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. FKE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `FailureKnowledgeEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-086 EME2` | Upstream | Must provide validated evidence before FKE starts. |
| `SPEC-041 PRE` | Upstream | Must provide validated evidence before FKE starts. |
| `SPEC-042 SPE` | Upstream | Must provide validated evidence before FKE starts. |
| `SPEC-095 RLE` | Downstream | Must consume only the validated FKE output contract. |
| `SPEC-098 LAE` | Downstream | Must consume only the validated FKE output contract. |
| `src/learning/failure_knowledge.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `FailureKnowledgeEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: FKE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/failure_knowledge.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-089-FKE.md
```

Guidance:
- Keep `FailureKnowledgeEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_fke(payload: dict) -> dict:
    input_contract = validate_fke_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = FailureKnowledgeEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_fke_output(input_contract, result)
    validate_fke_output(output)
...
- `FailureKnowledgeEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/failure_knowledge.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-089 FKE: Failure Knowledge Engine"]
...
graph LR
    ...
    V --> Core["FailureKnowledgeEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["FKE Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-089"]
    This --> Source["src/learning/failure_knowledge.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-095 RLE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute FKE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-089 FKE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-095 RLE" as D
U --> V
V --> E
E --> K
K --> D
...
class "FKEInputContract"
class "FKEOutputContract"
FailureKnowledgeEngine --> "FKEInputContract"
FailureKnowledgeEngine --> "FKEOutputContract"
@enduml
```
