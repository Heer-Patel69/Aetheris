# SPEC-018: Testing Planning Engine (TPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Test suite architecture and coverage planning
...
Primary Class: `TestingPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Testing Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Defines unit, integration, smoke, regression, acceptance, security, and performance test suites and the coverage gates they enforce.

This document upgrades SPEC-018 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-018 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of TPE is to own the full engineering responsibility for test suite architecture and coverage planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding TPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct test suite architecture and coverage planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for test suite architecture and coverage planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-017.
- Emit structured output that can be consumed by SPEC-019.
...
======================================================================
In scope:
- Test suite architecture and coverage planning
- Validation of all TPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-018` input contract to `SPEC-018` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `TestingPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
TPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- TPE validates and transforms that evidence.
...
======================================================================
- FR-001: TPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-017.
- FR-002: TPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: TPE shall execute the primary behavior owned by `TestingPlanningEngine`.
- FR-004: TPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-018 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat TPE output as the authoritative contract for test suite architecture and coverage planning unless a later validation SPEC rejects it.
...
Primary implementation class: `TestingPlanningEngine`.

Implementation classes in scope:
- `TestingPlanningEngine`
...
======================================================================
TPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-018` owns how the engine is implemented.
...
TPE must exchange data by contract, not by implicit global state.
...
4. Execute `TestingPlanningEngine` behavior for test suite architecture and coverage planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-017.
...
- `TestingPlanningEngine`: Owns the core test suite architecture and coverage planning behavior.
...
- Input interface: `TPEInput` JSON object.
- Output interface: `TPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `TestingPlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for TPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-018-example",
  "spec_id": "SPEC-018",
  ...
  "payload": {
    "engine": "Testing Planning Engine",
    "source_class": "TestingPlanningEngine",
    "public_methods": [
...
- `AETHERIS_TPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_TPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_TPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. TPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `TPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `TPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `TPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- TPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
TPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. TPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `TestingPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before TPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before TPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before TPE starts. |
| `SPEC-017` | Upstream | Must provide validated evidence before TPE starts. |
| `SPEC-019` | Downstream | Must consume only the validated TPE output contract. |
...
| FR-003 core execution | Direct `TestingPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: TPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-018-TPE.md
```

Guidance:
- Keep `TestingPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_tpe(payload: dict) -> dict:
    input_contract = validate_tpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = TestingPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_tpe_output(input_contract, result)
    validate_tpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-018 TPE: Testing Planning Engine"]
...
graph LR
    ...
    V --> Core["TestingPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["TPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute TPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-018 TPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-019" as D
U --> V
V --> E
E --> K
K --> D
...
class "TPEInputContract"
class "TPEOutputContract"
TestingPlanningEngine --> "TPEInputContract"
TestingPlanningEngine --> "TPEOutputContract"
@enduml
```
