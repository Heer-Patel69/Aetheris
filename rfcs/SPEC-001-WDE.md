# SPEC-001: Workspace Discovery Engine (WDE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-001
Layer: Engineering Knowledge System
Scope: Workspace discovery and inventory compilation
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/wde.py` (validated from source metadata)
Primary Class: `WorkspaceDiscoveryEngine`
Test Reference: `tests/test_wde.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Workspace Discovery Engine exists to make Engineering Knowledge System deterministic, reviewable, and safe at production scale. Compiles a deterministic inventory of files, languages, dependencies, frameworks, git metadata, fingerprints, and filesystem graph edges.

This document upgrades SPEC-001 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-001 is part of the Engineering Knowledge Compiler, where evidence is discovered, normalized, stored, and queried before planning begins.

======================================================================
2. PURPOSE
======================================================================
The purpose of WDE is to own the full engineering responsibility for workspace discovery and inventory compilation. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding WDE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct workspace discovery and inventory compilation was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for workspace discovery and inventory compilation.
- Accept only validated upstream artifacts from User goal, workspace filesystem, git metadata, project manifests.
- Emit structured output that can be consumed by SPEC-002 URUE, SPEC-003 PDE, SPEC-007 EKB.
...
======================================================================
In scope:
- Workspace discovery and inventory compilation
- Validation of all WDE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-001` input contract to `SPEC-001` output contract.
- Use `src/intelligence/wde.py` as the implementation boundary and `WorkspaceDiscoveryEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
WDE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- WDE validates and transforms that evidence.
...
======================================================================
- FR-001: WDE shall load the upstream artifacts declared by User goal, workspace filesystem, git metadata, project manifests.
- FR-002: WDE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: WDE shall execute the primary behavior owned by `WorkspaceDiscoveryEngine`.
- FR-004: WDE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
======================================================================
- Latency target: complete an incremental scan of 10,000 files in less than 30 seconds on developer hardware, excluding intentionally ignored directories.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-001 sits in `Engineering Knowledge System` under `RFC-001`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.

Upstream dependencies:
- User goal, workspace filesystem, git metadata, project manifests
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat WDE output as the authoritative contract for workspace discovery and inventory compilation unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/wde.py`.

Primary implementation class: `WorkspaceDiscoveryEngine`.
...
======================================================================
WDE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-001` owns how the engine is implemented.
...
WDE must exchange data by contract, not by implicit global state.
...
4. Execute `WorkspaceDiscoveryEngine` behavior for workspace discovery and inventory compilation.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: User goal, workspace filesystem, git metadata, project manifests.
...
4. Run the WDE core transformation.
5. Generate artifacts and EKB records.
6. Validate output against the JSON schema in this SPEC.
7. Release the output to SPEC-002 URUE, SPEC-003 PDE, SPEC-007 EKB.
...
- `WorkspaceDiscoveryEngine`: Owns the core workspace discovery and inventory compilation behavior.
...
- Input interface: `WDEInput` JSON object.
- Output interface: `WDEOutput` JSON object.
- Source interface: `src/intelligence/wde.py`.
- Public class boundary: `WorkspaceDiscoveryEngine`.
...
|---|---|---|
| `scan() -> Dict[str, dict]` | Executes the `scan` responsibility for WDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-001-example",
  "spec_id": "SPEC-001",
  ...
  "payload": {
    "engine": "Workspace Discovery Engine",
    "source_class": "WorkspaceDiscoveryEngine",
    "public_methods": [
      "scan() -> Dict[str, dict]"
    ]
  }
...
- `AETHERIS_WDE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_WDE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_WDE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. WDE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `WDE_STARTED` with request ID, spec ID, source path, and upstream references.
- `WDE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `WDE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
- Primary latency target: complete an incremental scan of 10,000 files in less than 30 seconds on developer hardware, excluding intentionally ignored directories.
...
======================================================================
Scalability model:
- WDE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
WDE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. WDE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_wde.py`.
...
======================================================================
- Unit tests should instantiate `WorkspaceDiscoveryEngine` directly and verify public method behavior.
...
|---|---|---|
| `User goal, workspace filesystem, git metadata, project manifests` | Upstream | Must provide validated evidence before WDE starts. |
| `SPEC-002 URUE` | Downstream | Must consume only the validated WDE output contract. |
| `SPEC-003 PDE` | Downstream | Must consume only the validated WDE output contract. |
| `SPEC-007 EKB` | Downstream | Must consume only the validated WDE output contract. |
| `src/intelligence/wde.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_wde.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_wde.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `WorkspaceDiscoveryEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: WDE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/wde.py
tests/test_wde.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-001-WDE.md
```

Guidance:
- Keep `WorkspaceDiscoveryEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_wde(payload: dict) -> dict:
    input_contract = validate_wde_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = WorkspaceDiscoveryEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_wde_output(input_contract, result)
    validate_wde_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/wde.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["User goal, workspace filesystem, git metadata, project manifests"] --> I["Input Validator"]
    I --> E["SPEC-001 WDE: Workspace Discovery Engine"]
...
graph LR
    ...
    V --> Core["WorkspaceDiscoveryEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["WDE Payload"]
...
```mermaid
graph TD
    Parent["RFC-001"] --> This["SPEC-001"]
    This --> Source["src/intelligence/wde.py"]
    This --> Test["tests/test_wde.py"]
    This --> Next["SPEC-002 URUE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute WDE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "User goal, workspace filesystem, git metadata, project manifests" as U
rectangle "Input Validator" as V
rectangle "SPEC-001 WDE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-002 URUE" as D
U --> V
V --> E
E --> K
K --> D
...
class "WDEInputContract"
class "WDEOutputContract"
WorkspaceDiscoveryEngine --> "WDEInputContract"
WorkspaceDiscoveryEngine --> "WDEOutputContract"
@enduml
```
