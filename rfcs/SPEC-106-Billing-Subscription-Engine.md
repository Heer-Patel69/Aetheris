# SPEC-106: Billing & Subscription Engine (BSE)
...
Primary Class: `BillingSubscriptionEngine`
Test Reference: `tests/test_rfc007_core.py`

======================================================================
1. PURPOSE & BUSINESS VALUE
======================================================================
The Billing & Subscription Engine (BSE) exists to provide a foundational enterprise capability for Aetheris. It ensures secure, production-scale, multi-tenant operations, maintaining compliance, security boundaries, and strict resource isolation.
...
======================================================================
- FR-101: The engine shall load configurations conforming to the `SPEC-106Input` schema.
- FR-102: The engine shall authorize all operations against tenant context parameters.
- FR-103: The engine shall execute the core `BillingSubscriptionEngine` behaviors.
- FR-104: The engine shall output structured results conforming to the `SPEC-106Output` schema.
...
======================================================================
Primary Path: `src/enterprise/billing.py`.
The class `BillingSubscriptionEngine` acts as the main domain service layer, validating API structures, interacting with local storage adapters, and dispatching transaction logs to the audit service.
...
- `BillingSubscriptionEngineCore`: Executes subsystem transformations.
...
3. Call `BillingSubscriptionEngine` core behaviors.
...
======================================================================
Primary logic is defined in `src/enterprise/billing.py`. Sub-modules handle encryption, parsing, and database schemas.
...
|---|---|---|
| `record_usage(tenant_id: str, resource_type: str, quantity: float) -> bool` | Records metered usage coordinates for billing calculations. | Validate input, enforce security boundaries, return deterministic output, and emit telemetry. |
...
18. OBSERVABILITY
======================================================================
Structured logging includes standard event codes like `BSE_STARTED`, `BSE_SUCCESS`, and `BSE_FAILED` with request ID attributes.
