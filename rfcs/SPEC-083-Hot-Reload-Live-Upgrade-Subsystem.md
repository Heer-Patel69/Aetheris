# SPEC-083: Hot Reload Live Upgrade Subsystem (HRLU)
...
Implementation: `src/runtime/hot_reload.py` (source implementation pending)
Primary Class: `HotReloadLiveUpgradeSubsystem`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Hot Reload Live Upgrade Subsystem exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Performs safe live upgrades, hot reload, compatibility checks, and rollback coordination for runtime components.

This document upgrades SPEC-083 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-083 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of HRLU is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding HRLU behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-081 DSCS, release artifacts.
- Emit structured output that can be consumed by SPEC-085 GRO.
...
- Validation of all HRLU input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-083` input contract to `SPEC-083` output contract.
- Use `src/runtime/hot_reload.py` as the implementation boundary and `HotReloadLiveUpgradeSubsystem` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
HRLU follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- HRLU validates and transforms that evidence.
...
======================================================================
- FR-001: HRLU shall load the upstream artifacts declared by SPEC-081 DSCS, release artifacts.
- FR-002: HRLU shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: HRLU shall execute the primary behavior owned by `HotReloadLiveUpgradeSubsystem`.
- FR-004: HRLU shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-083 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat HRLU output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/hot_reload.py`.

Primary implementation class: `HotReloadLiveUpgradeSubsystem`.

Implementation classes in scope:
- `HotReloadLiveUpgradeSubsystem`
...
======================================================================
HRLU is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-083` owns how the engine is implemented.
...
HRLU must exchange data by contract, not by implicit global state.
...
4. Execute `HotReloadLiveUpgradeSubsystem` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-081 DSCS, release artifacts.
...
- `HotReloadLiveUpgradeSubsystem`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Performs safe live upgrades, hot reload, compatibility checks, and rollback coordination for runtime components.
...
- Input interface: `HRLUInput` JSON object.
- Output interface: `HRLUOutput` JSON object.
- Source interface: `src/runtime/hot_reload.py`.
- Public class boundary: `HotReloadLiveUpgradeSubsystem`.
...
|---|---|---|
| `HotReloadLiveUpgradeSubsystem.run(payload: dict) -> dict` | Executes the `HotReloadLiveUpgradeSubsystem.run` responsibility for HRLU. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/runtime/hot_reload.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-083-example",
  "spec_id": "SPEC-083",
  ...
  "payload": {
    "engine": "Hot Reload Live Upgrade Subsystem",
    "source_class": "HotReloadLiveUpgradeSubsystem",
    "public_methods": []
  }
}
```
...
- `AETHERIS_HRLU_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_HRLU_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_HRLU_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/hot_reload.py` implementation is primarily in-memory. The enterprise contract still requires HRLU to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. HRLU may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `HRLU_STARTED` with request ID, spec ID, source path, and upstream references.
- `HRLU_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `HRLU_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- HRLU must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
HRLU is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. HRLU should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `HotReloadLiveUpgradeSubsystem` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-081 DSCS` | Upstream | Must provide validated evidence before HRLU starts. |
| `release artifacts` | Upstream | Must provide validated evidence before HRLU starts. |
| `SPEC-085 GRO` | Downstream | Must consume only the validated HRLU output contract. |
| `src/runtime/hot_reload.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `HotReloadLiveUpgradeSubsystem` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: HRLU grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/hot_reload.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-083-Hot-Reload-Live-Upgrade-Subsystem.md
```

Guidance:
- Keep `HotReloadLiveUpgradeSubsystem` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_hrlu(payload: dict) -> dict:
    input_contract = validate_hrlu_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = HotReloadLiveUpgradeSubsystem(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_hrlu_output(input_contract, result)
    validate_hrlu_output(output)
...
- `HotReloadLiveUpgradeSubsystem.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/hot_reload.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-081 DSCS"] --> I["Input Validator"]
    I --> E["SPEC-083 HRLU: Hot Reload Live Upgrade Subsystem"]
...
graph LR
    ...
    V --> Core["HotReloadLiveUpgradeSubsystem Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["HRLU Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-083"]
    This --> Source["src/runtime/hot_reload.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute HRLU"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-081 DSCS" as U
rectangle "Input Validator" as V
rectangle "SPEC-083 HRLU" as E
...
```plantuml
@startuml
class HotReloadLiveUpgradeSubsystem {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "HRLUInputContract"
class "HRLUOutputContract"
HotReloadLiveUpgradeSubsystem --> "HRLUInputContract"
HotReloadLiveUpgradeSubsystem --> "HRLUOutputContract"
@enduml
```
