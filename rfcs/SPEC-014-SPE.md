# SPEC-014: Security Planning Engine (SPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Authentication, authorization, threat model, and OWASP planning
...
Primary Class: `SecurityPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Security Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Plans authentication standards, authorization matrix, STRIDE threats, OWASP controls, CORS posture, and security headers.

This document upgrades SPEC-014 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-014 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of SPE is to own the full engineering responsibility for authentication, authorization, threat model, and owasp planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Letting downstream engines reconstruct authentication, authorization, threat model, and owasp planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for authentication, authorization, threat model, and owasp planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-013.
- Emit structured output that can be consumed by SPEC-015.
...
======================================================================
In scope:
- Authentication, authorization, threat model, and OWASP planning
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-014` input contract to `SPEC-014` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `SecurityPlanningEngine` as the primary class boundary.
...
======================================================================
- FR-001: SPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-013.
- FR-002: SPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SPE shall execute the primary behavior owned by `SecurityPlanningEngine`.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-014 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SPE output as the authoritative contract for authentication, authorization, threat model, and owasp planning unless a later validation SPEC rejects it.
...
Primary implementation class: `SecurityPlanningEngine`.

Implementation classes in scope:
- `SecurityPlanningEngine`
...
- `SPEC-014` owns how the engine is implemented.
...
4. Execute `SecurityPlanningEngine` behavior for authentication, authorization, threat model, and owasp planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-013.
...
7. Release the output to SPEC-015.
...
- `SecurityPlanningEngine`: Owns the core authentication, authorization, threat model, and owasp planning behavior.
...
- Public class boundary: `SecurityPlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for SPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-014-example",
  "spec_id": "SPEC-014",
  ...
  "payload": {
    "engine": "Security Planning Engine",
    "source_class": "SecurityPlanningEngine",
    "public_methods": [
...
- `auth.plan.json`
- `authorization.plan.json`
- `owasp.plan.json`
- `threat.model.json`

EKB object registrations owned by this SPEC:
- `spec_014_auth`
- `spec_014_authorization`
...
======================================================================
- Unit tests should instantiate `SecurityPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before SPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before SPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before SPE starts. |
| `SPEC-013` | Upstream | Must provide validated evidence before SPE starts. |
| `SPEC-015` | Downstream | Must consume only the validated SPE output contract. |
...
| FR-003 core execution | Direct `SecurityPlanningEngine` public API tests |
...
rfcs/SPEC-014-SPE.md
```

Guidance:
- Keep `SecurityPlanningEngine` as the primary orchestration class for this SPEC.
...
def run_spe(payload: dict) -> dict:
    ...
    result = SecurityPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
...
graph TD
    ...
    I --> E["SPEC-014 SPE: Security Planning Engine"]
...
graph LR
    ...
    V --> Core["SecurityPlanningEngine Core"]
...
rectangle "SPEC-014 SPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-015" as D
U --> V
V --> E
E --> K
K --> D
...
SecurityPlanningEngine --> "SPEInputContract"
SecurityPlanningEngine --> "SPEOutputContract"
@enduml
```
