...
Implementation: `src/runtime/rpc.py` (source implementation pending)
Primary Class: `RPCFramework`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
RPC Framework exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Provides typed remote procedure calls for distributed runtime components while enforcing authentication, serialization, timeout, and retry policies.

This document upgrades SPEC-069 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-069 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of RPCF is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding RPCF behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-068 RPL, distributed clients.
- Emit structured output that can be consumed by SPEC-071 DEE, SPEC-075 DCLE.
...
- Validation of all RPCF input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-069` input contract to `SPEC-069` output contract.
- Use `src/runtime/rpc.py` as the implementation boundary and `RPCFramework` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
RPCF follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- RPCF validates and transforms that evidence.
...
======================================================================
- FR-001: RPCF shall load the upstream artifacts declared by SPEC-068 RPL, distributed clients.
- FR-002: RPCF shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: RPCF shall execute the primary behavior owned by `RPCFramework`.
- FR-004: RPCF shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-069 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat RPCF output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/rpc.py`.

Primary implementation class: `RPCFramework`.
...
======================================================================
RPCF is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-069` owns how the engine is implemented.
...
RPCF must exchange data by contract, not by implicit global state.
...
4. Execute `RPCFramework` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-068 RPL, distributed clients.
...
- `RPCFramework`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Provides typed remote procedure calls for distributed runtime components while enforcing authentication, serialization, timeout, and retry policies.
...
- Input interface: `RPCFInput` JSON object.
- Output interface: `RPCFOutput` JSON object.
- Source interface: `src/runtime/rpc.py`.
- Public class boundary: `RPCFramework`.
...
|---|---|---|
| `RPCFramework.run(payload: dict) -> dict` | Executes the `RPCFramework.run` responsibility for RPCF. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-069-example",
  "spec_id": "SPEC-069",
  ...
  "payload": {
    "engine": "RPC Framework",
    "source_class": "RPCFramework",
    "public_methods": []
  }
}
```
...
- `AETHERIS_RPCF_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_RPCF_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_RPCF_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/rpc.py` implementation is primarily in-memory. The enterprise contract still requires RPCF to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. RPCF may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `RPCF_STARTED` with request ID, spec ID, source path, and upstream references.
- `RPCF_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `RPCF_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- RPCF must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
RPCF is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. RPCF should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `RPCFramework` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-068 RPL` | Upstream | Must provide validated evidence before RPCF starts. |
| `distributed clients` | Upstream | Must provide validated evidence before RPCF starts. |
| `SPEC-071 DEE` | Downstream | Must consume only the validated RPCF output contract. |
| `SPEC-075 DCLE` | Downstream | Must consume only the validated RPCF output contract. |
| `src/runtime/rpc.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `RPCFramework` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: RPCF grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-069-RPC-Framework.md
```

Guidance:
- Keep `RPCFramework` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_rpcf(payload: dict) -> dict:
    input_contract = validate_rpcf_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = RPCFramework(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_rpcf_output(input_contract, result)
    validate_rpcf_output(output)
...
- `RPCFramework.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/rpc.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-069 RPCF: RPC Framework"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["RPCF Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-069"]
    This --> Source["src/runtime/rpc.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute RPCF"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
class "RPCFInputContract"
class "RPCFOutputContract"
RPCFramework --> "RPCFInputContract"
RPCFramework --> "RPCFOutputContract"
@enduml
```
