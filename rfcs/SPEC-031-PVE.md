# SPEC-031: Planning Validation Engine (PVE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Cross-planner validation, consistency audit, and blueprint scoring
...
Primary Class: `PlanningValidationEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Planning Validation Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Performs cross-planner validation, theme consistency review, dependency checks, coverage analysis, quality scoring, and warning/failure reporting.

This document upgrades SPEC-031 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-031 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of PVE is to own the full engineering responsibility for cross-planner validation, consistency audit, and blueprint scoring. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PVE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct cross-planner validation, consistency audit, and blueprint scoring was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for cross-planner validation, consistency audit, and blueprint scoring.
- Accept only validated upstream artifacts from SPEC-012 through SPEC-030, SPEC-007 EKB.
- Emit structured output that can be consumed by SPEC-030 FEBC, SPEC-032 TDE.
...
======================================================================
In scope:
- Cross-planner validation, consistency audit, and blueprint scoring
- Validation of all PVE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-031` input contract to `SPEC-031` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `PlanningValidationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PVE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PVE validates and transforms that evidence.
...
======================================================================
- FR-001: PVE shall load the upstream artifacts declared by SPEC-012 through SPEC-030, SPEC-007 EKB.
- FR-002: PVE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PVE shall execute the primary behavior owned by `PlanningValidationEngine`.
- FR-004: PVE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-031 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.

Upstream dependencies:
- SPEC-012 through SPEC-030
- SPEC-007 EKB
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PVE output as the authoritative contract for cross-planner validation, consistency audit, and blueprint scoring unless a later validation SPEC rejects it.
...
Primary implementation class: `PlanningValidationEngine`.
...
======================================================================
PVE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-031` owns how the engine is implemented.
...
PVE must exchange data by contract, not by implicit global state.
...
4. Execute `PlanningValidationEngine` behavior for cross-planner validation, consistency audit, and blueprint scoring.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-012 through SPEC-030, SPEC-007 EKB.
...
- `PlanningValidationEngine`: Owns the core cross-planner validation, consistency audit, and blueprint scoring behavior.
...
- Input interface: `PVEInput` JSON object.
- Output interface: `PVEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `PlanningValidationEngine`.
...
|---|---|---|
| `validate_all(blueprint_context: dict) -> dict` | Executes the `validate_all` responsibility for PVE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-031-example",
  "spec_id": "SPEC-031",
  ...
  "payload": {
    "engine": "Planning Validation Engine",
    "source_class": "PlanningValidationEngine",
    "public_methods": [
      "validate_all(blueprint_context: dict) -> dict"
    ]
  }
}
```
...
- `AETHERIS_PVE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PVE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PVE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. PVE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PVE_STARTED` with request ID, spec ID, source path, and upstream references.
- `PVE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PVE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PVE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PVE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PVE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `PlanningValidationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-012 through SPEC-030` | Upstream | Must provide validated evidence before PVE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before PVE starts. |
| `SPEC-030 FEBC` | Downstream | Must consume only the validated PVE output contract. |
| `SPEC-032 TDE` | Downstream | Must consume only the validated PVE output contract. |
...
| FR-003 core execution | Direct `PlanningValidationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PVE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-031-PVE.md
```

Guidance:
- Keep `PlanningValidationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_pve(payload: dict) -> dict:
    input_contract = validate_pve_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = PlanningValidationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_pve_output(input_contract, result)
    validate_pve_output(output)
...
```mermaid
graph TD
    U["SPEC-012 through SPEC-030"] --> I["Input Validator"]
    I --> E["SPEC-031 PVE: Planning Validation Engine"]
...
graph LR
    ...
    V --> Core["PlanningValidationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PVE Payload"]
...
```mermaid
graph TD
    Parent["RFC-002"] --> This["SPEC-031"]
    This --> Source["src/intelligence/planners.py"]
    This --> Test["tests/test_planners.py"]
    This --> Next["SPEC-030 FEBC"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PVE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-012 through SPEC-030" as U
rectangle "Input Validator" as V
rectangle "SPEC-031 PVE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-030 FEBC" as D
U --> V
V --> E
E --> K
K --> D
...
class "PVEInputContract"
class "PVEOutputContract"
PlanningValidationEngine --> "PVEInputContract"
PlanningValidationEngine --> "PVEOutputContract"
@enduml
```
