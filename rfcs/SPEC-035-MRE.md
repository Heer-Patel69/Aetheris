# SPEC-035: Model Routing Engine (MRE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Task-specific model routing
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/mre.py` (validated from source metadata)
Primary Class: `ModelRoutingEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Model Routing Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Selects execution models according to skill, complexity, latency, quality confidence, provider health, and estimated input/output token cost.

This document upgrades SPEC-035 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-035 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of MRE is to own the full engineering responsibility for task-specific model routing. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Letting downstream engines reconstruct task-specific model routing was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for task-specific model routing.
- Accept only validated upstream artifacts from SPEC-034.
- Emit structured output that can be consumed by SPEC-036.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-035` input contract to `SPEC-035` output contract.
- Use `src/execution/mre.py` as the implementation boundary and `ModelRoutingEngine` as the primary class boundary.
...
======================================================================
- FR-001: MRE shall load the upstream artifacts declared by SPEC-034.
- FR-002: MRE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: MRE shall execute the primary behavior owned by `ModelRoutingEngine`.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-035 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat MRE output as the authoritative contract for task-specific model routing unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/mre.py`.

Primary implementation class: `ModelRoutingEngine`.
...
- `SPEC-035` owns how the engine is implemented.
...
4. Execute `ModelRoutingEngine` behavior for task-specific model routing.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-034.
...
7. Release the output to SPEC-036.
...
- `ModelRoutingEngine`: Owns the core task-specific model routing behavior.
...
- Source interface: `src/execution/mre.py`.
- Public class boundary: `ModelRoutingEngine`.
...
|---|---|---|
| `route_model(task: dict, skill: dict) -> dict` | Executes the `route_model` responsibility for MRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `rank_models_for_task(task: dict) -> List[dict]` | Executes the `rank_models_for_task` responsibility for MRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `estimate_cost(model: str, input_tokens: int, output_tokens: int) -> float` | Executes the `estimate_cost` responsibility for MRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `estimate_latency(model: str) -> float` | Executes the `estimate_latency` responsibility for MRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `confidence(model: str, task: dict) -> float` | Executes the `confidence` responsibility for MRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `provider_health(provider: str) -> str` | Executes the `provider_health` responsibility for MRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `reroute(task: dict) -> dict` | Executes the `reroute` responsibility for MRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-035-example",
  "spec_id": "SPEC-035",
  ...
  "payload": {
    "engine": "Model Routing Engine",
    "source_class": "ModelRoutingEngine",
    "public_methods": [
      "route_model(task: dict, skill: dict) -> dict",
      "rank_models_for_task(task: dict) -> List[dict]",
      "estimate_cost(model: str, input_tokens: int, output_tokens: int) -> float",
      "estimate_latency(model: str) -> float",
      "confidence(model: str, task: dict) -> float"
    ]
  }
}
```
...
======================================================================
- Unit tests should instantiate `ModelRoutingEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-034` | Upstream | Must provide validated evidence before MRE starts. |
| `SPEC-036` | Downstream | Must consume only the validated MRE output contract. |
| `src/execution/mre.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ModelRoutingEngine` public API tests |
...
```text
src/execution/mre.py
tests/test_execution.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-035-MRE.md
```

Guidance:
- Keep `ModelRoutingEngine` as the primary orchestration class for this SPEC.
...
def run_mre(payload: dict) -> dict:
    ...
    result = ModelRoutingEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/mre.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-034"] --> I["Input Validator"]
    I --> E["SPEC-035 MRE: Model Routing Engine"]
...
graph LR
    ...
    V --> Core["ModelRoutingEngine Core"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-035"]
    This --> Source["src/execution/mre.py"]
...
```plantuml
@startuml
rectangle "SPEC-034" as U
rectangle "Input Validator" as V
rectangle "SPEC-035 MRE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-036" as D
U --> V
V --> E
E --> K
K --> D
...
ModelRoutingEngine --> "MREInputContract"
ModelRoutingEngine --> "MREOutputContract"
@enduml
```
