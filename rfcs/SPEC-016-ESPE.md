# SPEC-016: External Services Planning Engine (ESPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Vendor integrations and external service contracts
...
Primary Class: `ExternalServicesPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
External Services Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Inventories vendor services, integration contracts, credential boundaries, outbound API dependencies, and service ownership rules.

This document upgrades SPEC-016 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-016 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of ESPE is to own the full engineering responsibility for vendor integrations and external service contracts. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding ESPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct vendor integrations and external service contracts was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for vendor integrations and external service contracts.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-015.
- Emit structured output that can be consumed by SPEC-017.
...
======================================================================
In scope:
- Vendor integrations and external service contracts
- Validation of all ESPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-016` input contract to `SPEC-016` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `ExternalServicesPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
ESPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- ESPE validates and transforms that evidence.
...
======================================================================
- FR-001: ESPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-015.
- FR-002: ESPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: ESPE shall execute the primary behavior owned by `ExternalServicesPlanningEngine`.
- FR-004: ESPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-016 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat ESPE output as the authoritative contract for vendor integrations and external service contracts unless a later validation SPEC rejects it.
...
Primary implementation class: `ExternalServicesPlanningEngine`.

Implementation classes in scope:
- `ExternalServicesPlanningEngine`
...
======================================================================
ESPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-016` owns how the engine is implemented.
...
ESPE must exchange data by contract, not by implicit global state.
...
4. Execute `ExternalServicesPlanningEngine` behavior for vendor integrations and external service contracts.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-015.
...
- `ExternalServicesPlanningEngine`: Owns the core vendor integrations and external service contracts behavior.
...
- Input interface: `ESPEInput` JSON object.
- Output interface: `ESPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `ExternalServicesPlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for ESPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-016-example",
  "spec_id": "SPEC-016",
  ...
  "payload": {
    "engine": "External Services Planning Engine",
    "source_class": "ExternalServicesPlanningEngine",
    "public_methods": [
...
- `AETHERIS_ESPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_ESPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_ESPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. ESPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `ESPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `ESPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `ESPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- ESPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
ESPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. ESPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ExternalServicesPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before ESPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before ESPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before ESPE starts. |
| `SPEC-015` | Upstream | Must provide validated evidence before ESPE starts. |
| `SPEC-017` | Downstream | Must consume only the validated ESPE output contract. |
...
| FR-003 core execution | Direct `ExternalServicesPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: ESPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-016-ESPE.md
```

Guidance:
- Keep `ExternalServicesPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_espe(payload: dict) -> dict:
    input_contract = validate_espe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ExternalServicesPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_espe_output(input_contract, result)
    validate_espe_output(output)
...
graph TD
    ...
    I --> E["SPEC-016 ESPE: External Services Planning Engine"]
...
graph LR
    ...
    V --> Core["ExternalServicesPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["ESPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute ESPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-016 ESPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-017" as D
U --> V
V --> E
E --> K
K --> D
...
class "ESPEInputContract"
class "ESPEOutputContract"
ExternalServicesPlanningEngine --> "ESPEInputContract"
ExternalServicesPlanningEngine --> "ESPEOutputContract"
@enduml
```
