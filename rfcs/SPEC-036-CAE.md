# SPEC-036: Context Assembly Engine (CAE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Execution context packaging
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/cae.py` (validated from source metadata)
Primary Class: `ContextAssemblyEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Context Assembly Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Retrieves source evidence, dependency references, memory snippets, and task details into bounded context payloads for execution workers.

This document upgrades SPEC-036 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-036 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of CAE is to own the full engineering responsibility for execution context packaging. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding CAE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct execution context packaging was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for execution context packaging.
- Accept only validated upstream artifacts from SPEC-035.
- Emit structured output that can be consumed by SPEC-037.
...
======================================================================
In scope:
- Execution context packaging
- Validation of all CAE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-036` input contract to `SPEC-036` output contract.
- Use `src/execution/cae.py` as the implementation boundary and `ContextAssemblyEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
CAE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- CAE validates and transforms that evidence.
...
======================================================================
- FR-001: CAE shall load the upstream artifacts declared by SPEC-035.
- FR-002: CAE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: CAE shall execute the primary behavior owned by `ContextAssemblyEngine`.
- FR-004: CAE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-036 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat CAE output as the authoritative contract for execution context packaging unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/cae.py`.

Primary implementation class: `ContextAssemblyEngine`.
...
======================================================================
CAE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-036` owns how the engine is implemented.
...
CAE must exchange data by contract, not by implicit global state.
...
4. Execute `ContextAssemblyEngine` behavior for execution context packaging.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-035.
...
- `ContextAssemblyEngine`: Owns the core execution context packaging behavior.
...
- Input interface: `CAEInput` JSON object.
- Output interface: `CAEOutput` JSON object.
- Source interface: `src/execution/cae.py`.
- Public class boundary: `ContextAssemblyEngine`.
...
|---|---|---|
| `assemble_context(task: dict, dependencies: List[str], token_limit: int) -> dict` | Executes the `assemble_context` responsibility for CAE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `compress(context_payload: dict, ratio: float) -> dict` | Executes the `compress` responsibility for CAE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `estimate_tokens(text: str) -> int` | Executes the `estimate_tokens` responsibility for CAE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `retrieve_dependencies(task: dict) -> List[str]` | Executes the `retrieve_dependencies` responsibility for CAE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `collect_memory(task: dict) -> List[str]` | Executes the `collect_memory` responsibility for CAE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `validate_context(context_payload: dict) -> bool` | Executes the `validate_context` responsibility for CAE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-036-example",
  "spec_id": "SPEC-036",
  ...
  "payload": {
    "engine": "Context Assembly Engine",
    "source_class": "ContextAssemblyEngine",
    "public_methods": [
      "assemble_context(task: dict, dependencies: List[str], token_limit: int) -> dict",
      "compress(context_payload: dict, ratio: float) -> dict",
      "estimate_tokens(text: str) -> int",
      "retrieve_dependencies(task: dict) -> List[str]",
      "collect_memory(task: dict) -> List[str]"
    ]
  }
}
```
...
- `AETHERIS_CAE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_CAE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_CAE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
- `.aetheris/planning/database/entities.plan.json`
...
======================================================================
Recovery is conservative. CAE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `CAE_STARTED` with request ID, spec ID, source path, and upstream references.
- `CAE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `CAE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- CAE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
CAE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. CAE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ContextAssemblyEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-035` | Upstream | Must provide validated evidence before CAE starts. |
| `SPEC-037` | Downstream | Must consume only the validated CAE output contract. |
| `src/execution/cae.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ContextAssemblyEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: CAE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/execution/cae.py
tests/test_execution.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-036-CAE.md
```

Guidance:
- Keep `ContextAssemblyEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_cae(payload: dict) -> dict:
    input_contract = validate_cae_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ContextAssemblyEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_cae_output(input_contract, result)
    validate_cae_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/cae.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-035"] --> I["Input Validator"]
    I --> E["SPEC-036 CAE: Context Assembly Engine"]
...
graph LR
    ...
    V --> Core["ContextAssemblyEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["CAE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-036"]
    This --> Source["src/execution/cae.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute CAE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-035" as U
rectangle "Input Validator" as V
rectangle "SPEC-036 CAE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-037" as D
U --> V
V --> E
E --> K
K --> D
...
class "CAEInputContract"
class "CAEOutputContract"
ContextAssemblyEngine --> "CAEInputContract"
ContextAssemblyEngine --> "CAEOutputContract"
@enduml
```
