# SPEC-013: API Planning Engine (APIE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: API contracts, request schemas, response schemas, and endpoints
...
Primary Class: `APIPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
API Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Generates protocol choices, versioning conventions, endpoint contracts, request schemas, response schemas, and rate-limit tiers.

This document upgrades SPEC-013 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-013 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of APIE is to own the full engineering responsibility for api contracts, request schemas, response schemas, and endpoints. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding APIE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct api contracts, request schemas, response schemas, and endpoints was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for api contracts, request schemas, response schemas, and endpoints.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-012.
- Emit structured output that can be consumed by SPEC-014.
...
======================================================================
In scope:
- API contracts, request schemas, response schemas, and endpoints
- Validation of all APIE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-013` input contract to `SPEC-013` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `APIPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
APIE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- APIE validates and transforms that evidence.
...
======================================================================
- FR-001: APIE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-012.
- FR-002: APIE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: APIE shall execute the primary behavior owned by `APIPlanningEngine`.
- FR-004: APIE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-013 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat APIE output as the authoritative contract for api contracts, request schemas, response schemas, and endpoints unless a later validation SPEC rejects it.
...
Primary implementation class: `APIPlanningEngine`.
...
======================================================================
APIE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-013` owns how the engine is implemented.
...
APIE must exchange data by contract, not by implicit global state.
...
4. Execute `APIPlanningEngine` behavior for api contracts, request schemas, response schemas, and endpoints.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-012.
...
- `APIPlanningEngine`: Owns the core api contracts, request schemas, response schemas, and endpoints behavior.
...
- Input interface: `APIEInput` JSON object.
- Output interface: `APIEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `APIPlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for APIE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-013-example",
  "spec_id": "SPEC-013",
  ...
  "payload": {
    "engine": "API Planning Engine",
    "source_class": "APIPlanningEngine",
    "public_methods": [
...
- `AETHERIS_APIE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_APIE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_APIE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. APIE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `APIE_STARTED` with request ID, spec ID, source path, and upstream references.
- `APIE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `APIE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- APIE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
APIE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. APIE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `APIPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before APIE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before APIE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before APIE starts. |
| `SPEC-012` | Upstream | Must provide validated evidence before APIE starts. |
| `SPEC-014` | Downstream | Must consume only the validated APIE output contract. |
...
| FR-003 core execution | Direct `APIPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: APIE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-013-APIE.md
```

Guidance:
- Keep `APIPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_apie(payload: dict) -> dict:
    input_contract = validate_apie_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = APIPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_apie_output(input_contract, result)
    validate_apie_output(output)
...
graph TD
    ...
    I --> E["SPEC-013 APIE: API Planning Engine"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["APIE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute APIE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-013 APIE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-014" as D
U --> V
V --> E
E --> K
K --> D
...
class "APIEInputContract"
class "APIEOutputContract"
APIPlanningEngine --> "APIEInputContract"
APIPlanningEngine --> "APIEOutputContract"
@enduml
```
