# SPEC-015: Infrastructure Planning Engine (IPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Cloud topology, network, compute, and hosting planning
...
Primary Class: `InfrastructurePlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Infrastructure Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Designs provider selection, region, VPC topology, subnets, compute profile, load-balancer posture, and hosting defaults.

This document upgrades SPEC-015 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-015 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of IPE is to own the full engineering responsibility for cloud topology, network, compute, and hosting planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding IPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct cloud topology, network, compute, and hosting planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for cloud topology, network, compute, and hosting planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-014.
- Emit structured output that can be consumed by SPEC-016.
...
======================================================================
In scope:
- Cloud topology, network, compute, and hosting planning
- Validation of all IPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-015` input contract to `SPEC-015` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `InfrastructurePlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
IPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- IPE validates and transforms that evidence.
...
======================================================================
- FR-001: IPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-014.
- FR-002: IPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: IPE shall execute the primary behavior owned by `InfrastructurePlanningEngine`.
- FR-004: IPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-015 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat IPE output as the authoritative contract for cloud topology, network, compute, and hosting planning unless a later validation SPEC rejects it.
...
Primary implementation class: `InfrastructurePlanningEngine`.

Implementation classes in scope:
- `InfrastructurePlanningEngine`
...
======================================================================
IPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-015` owns how the engine is implemented.
...
IPE must exchange data by contract, not by implicit global state.
...
4. Execute `InfrastructurePlanningEngine` behavior for cloud topology, network, compute, and hosting planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-014.
...
- `InfrastructurePlanningEngine`: Owns the core cloud topology, network, compute, and hosting planning behavior.
...
- Input interface: `IPEInput` JSON object.
- Output interface: `IPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `InfrastructurePlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for IPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-015-example",
  "spec_id": "SPEC-015",
  ...
  "payload": {
    "engine": "Infrastructure Planning Engine",
    "source_class": "InfrastructurePlanningEngine",
    "public_methods": [
...
- `AETHERIS_IPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_IPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_IPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. IPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `IPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `IPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `IPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- IPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
IPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. IPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `InfrastructurePlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before IPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before IPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before IPE starts. |
| `SPEC-014` | Upstream | Must provide validated evidence before IPE starts. |
| `SPEC-016` | Downstream | Must consume only the validated IPE output contract. |
...
| FR-003 core execution | Direct `InfrastructurePlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: IPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-015-IPE.md
```

Guidance:
- Keep `InfrastructurePlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ipe(payload: dict) -> dict:
    input_contract = validate_ipe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = InfrastructurePlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ipe_output(input_contract, result)
    validate_ipe_output(output)
...
graph TD
    ...
    I --> E["SPEC-015 IPE: Infrastructure Planning Engine"]
...
graph LR
    ...
    V --> Core["InfrastructurePlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["IPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute IPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-015 IPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-016" as D
U --> V
V --> E
E --> K
K --> D
...
class "IPEInputContract"
class "IPEOutputContract"
InfrastructurePlanningEngine --> "IPEInputContract"
InfrastructurePlanningEngine --> "IPEOutputContract"
@enduml
```
