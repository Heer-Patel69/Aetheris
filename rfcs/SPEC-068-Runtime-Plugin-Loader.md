# SPEC-068: Runtime Plugin Loader (RPL)
...
Implementation: `src/runtime/plugin_loader.py` (source implementation pending)
Primary Class: `RuntimePluginLoader`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Runtime Plugin Loader exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Loads, validates, isolates, initializes, and unloads runtime plugins without weakening host process safety or contract boundaries.

This document upgrades SPEC-068 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-068 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of RPL is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding RPL behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-066 PSE, SPEC-067 EME2.
- Emit structured output that can be consumed by SPEC-069 RPCF, SPEC-070 IPCF.
...
- Validation of all RPL input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-068` input contract to `SPEC-068` output contract.
- Use `src/runtime/plugin_loader.py` as the implementation boundary and `RuntimePluginLoader` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
RPL follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- RPL validates and transforms that evidence.
...
======================================================================
- FR-001: RPL shall load the upstream artifacts declared by SPEC-066 PSE, SPEC-067 EME2.
- FR-002: RPL shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: RPL shall execute the primary behavior owned by `RuntimePluginLoader`.
- FR-004: RPL shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-068 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat RPL output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/plugin_loader.py`.

Primary implementation class: `RuntimePluginLoader`.
...
======================================================================
RPL is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-068` owns how the engine is implemented.
...
RPL must exchange data by contract, not by implicit global state.
...
4. Execute `RuntimePluginLoader` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-066 PSE, SPEC-067 EME2.
...
- `RuntimePluginLoader`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Loads, validates, isolates, initializes, and unloads runtime plugins without weakening host process safety or contract boundaries.
...
- Input interface: `RPLInput` JSON object.
- Output interface: `RPLOutput` JSON object.
- Source interface: `src/runtime/plugin_loader.py`.
- Public class boundary: `RuntimePluginLoader`.
...
|---|---|---|
| `RuntimePluginLoader.run(payload: dict) -> dict` | Executes the `RuntimePluginLoader.run` responsibility for RPL. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/runtime/plugin_loader.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-068-example",
  "spec_id": "SPEC-068",
  ...
  "payload": {
    "engine": "Runtime Plugin Loader",
    "source_class": "RuntimePluginLoader",
    "public_methods": []
  }
}
```
...
- `AETHERIS_RPL_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_RPL_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_RPL_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/plugin_loader.py` implementation is primarily in-memory. The enterprise contract still requires RPL to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. RPL may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `RPL_STARTED` with request ID, spec ID, source path, and upstream references.
- `RPL_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `RPL_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- RPL must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
RPL is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. RPL should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `RuntimePluginLoader` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-066 PSE` | Upstream | Must provide validated evidence before RPL starts. |
| `SPEC-067 EME2` | Upstream | Must provide validated evidence before RPL starts. |
| `SPEC-069 RPCF` | Downstream | Must consume only the validated RPL output contract. |
| `SPEC-070 IPCF` | Downstream | Must consume only the validated RPL output contract. |
| `src/runtime/plugin_loader.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `RuntimePluginLoader` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: RPL grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/plugin_loader.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-068-Runtime-Plugin-Loader.md
```

Guidance:
- Keep `RuntimePluginLoader` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_rpl(payload: dict) -> dict:
    input_contract = validate_rpl_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = RuntimePluginLoader(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_rpl_output(input_contract, result)
    validate_rpl_output(output)
...
- `RuntimePluginLoader.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/plugin_loader.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-068 RPL: Runtime Plugin Loader"]
...
graph LR
    ...
    V --> Core["RuntimePluginLoader Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["RPL Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-068"]
    This --> Source["src/runtime/plugin_loader.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-069 RPCF"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute RPL"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-068 RPL" as E
database "EKB / Artifacts" as K
rectangle "SPEC-069 RPCF" as D
U --> V
V --> E
E --> K
K --> D
...
class "RPLInputContract"
class "RPLOutputContract"
RuntimePluginLoader --> "RPLInputContract"
RuntimePluginLoader --> "RPLOutputContract"
@enduml
```
