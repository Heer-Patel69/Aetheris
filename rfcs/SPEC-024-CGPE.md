# SPEC-024: Compliance Governance Planning Engine (CGPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Compliance and governance planning
...
Primary Class: `ComplianceGovernancePlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Compliance Governance Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Plans GDPR, SOC 2, retention, audit trail, access review, and governance control requirements.

This document upgrades SPEC-024 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-024 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of CGPE is to own the full engineering responsibility for compliance and governance planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding CGPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct compliance and governance planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for compliance and governance planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-023.
- Emit structured output that can be consumed by SPEC-025.
...
- Validation of all CGPE input and output structures.
...
- Use `src/intelligence/planners.py` as the implementation boundary and `ComplianceGovernancePlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
CGPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- CGPE validates and transforms that evidence.
...
======================================================================
- FR-001: CGPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-023.
- FR-002: CGPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: CGPE shall execute the primary behavior owned by `ComplianceGovernancePlanningEngine`.
- FR-004: CGPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-024 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat CGPE output as the authoritative contract for compliance and governance planning unless a later validation SPEC rejects it.
...
Primary implementation class: `ComplianceGovernancePlanningEngine`.

Implementation classes in scope:
- `ComplianceGovernancePlanningEngine`
...
======================================================================
CGPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-024` owns how the engine is implemented.
...
CGPE must exchange data by contract, not by implicit global state.
...
4. Execute `ComplianceGovernancePlanningEngine` behavior for compliance and governance planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-023.
...
- `ComplianceGovernancePlanningEngine`: Owns the core compliance and governance planning behavior.
...
- Input interface: `CGPEInput` JSON object.
- Output interface: `CGPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `ComplianceGovernancePlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for CGPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-024-example",
  "spec_id": "SPEC-024",
  ...
  "payload": {
    "engine": "Compliance Governance Planning Engine",
    "source_class": "ComplianceGovernancePlanningEngine",
    "public_methods": [
...
- `AETHERIS_CGPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_CGPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_CGPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. CGPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `CGPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `CGPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `CGPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- CGPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
CGPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. CGPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ComplianceGovernancePlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before CGPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before CGPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before CGPE starts. |
| `SPEC-023` | Upstream | Must provide validated evidence before CGPE starts. |
| `SPEC-025` | Downstream | Must consume only the validated CGPE output contract. |
...
| FR-003 core execution | Direct `ComplianceGovernancePlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: CGPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-024-CGPE.md
```

Guidance:
- Keep `ComplianceGovernancePlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_cgpe(payload: dict) -> dict:
    input_contract = validate_cgpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ComplianceGovernancePlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_cgpe_output(input_contract, result)
    validate_cgpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-024 CGPE: Compliance Governance Planning Engine"]
...
graph LR
    ...
    V --> Core["ComplianceGovernancePlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["CGPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute CGPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-024 CGPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-025" as D
U --> V
V --> E
E --> K
K --> D
...
class "CGPEInputContract"
class "CGPEOutputContract"
ComplianceGovernancePlanningEngine --> "CGPEInputContract"
ComplianceGovernancePlanningEngine --> "CGPEOutputContract"
@enduml
```
