# SPEC-005: Engineering Decision Engine (EDE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-001
Layer: Decision Intelligence
Scope: Technology decision governance
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/ede.py` (validated from source metadata)
Primary Class: `EngineeringDecisionEngine`
Test Reference: `tests/test_ede.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Engineering Decision Engine exists to make Decision Intelligence deterministic, reviewable, and safe at production scale. Scores alternatives, records weighted architecture and technology decisions, versions decision history, and supports rollback to prior decision states.

This document upgrades SPEC-005 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-005 is part of the Engineering Knowledge Compiler, where evidence is discovered, normalized, stored, and queried before planning begins.

======================================================================
2. PURPOSE
======================================================================
The purpose of EDE is to own the full engineering responsibility for technology decision governance. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EDE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct technology decision governance was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for technology decision governance.
- Accept only validated upstream artifacts from SPEC-004.
- Emit structured output that can be consumed by SPEC-006.
...
- Validation of all EDE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-005` input contract to `SPEC-005` output contract.
- Use `src/intelligence/ede.py` as the implementation boundary and `EngineeringDecisionEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EDE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EDE validates and transforms that evidence.
...
======================================================================
- FR-001: EDE shall load the upstream artifacts declared by SPEC-004.
- FR-002: EDE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EDE shall execute the primary behavior owned by `EngineeringDecisionEngine`.
- FR-004: EDE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-005 sits in `Decision Intelligence` under `RFC-001`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EDE output as the authoritative contract for technology decision governance unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/ede.py`.

Primary implementation class: `EngineeringDecisionEngine`.
...
======================================================================
EDE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-005` owns how the engine is implemented.
...
EDE must exchange data by contract, not by implicit global state.
...
4. Execute `EngineeringDecisionEngine` behavior for technology decision governance.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-004.
...
- `EngineeringDecisionEngine`: Owns the core technology decision governance behavior.
...
- Input interface: `EDEInput` JSON object.
- Output interface: `EDEOutput` JSON object.
- Source interface: `src/intelligence/ede.py`.
- Public class boundary: `EngineeringDecisionEngine`.
...
|---|---|---|
| `evaluate_decision(category: str, alternatives: List[str], weights: Optional[dict]) -> dict` | Executes the `evaluate_decision` responsibility for EDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `rollback_decision(category: str, target_version: int) -> Optional[dict]` | Executes the `rollback_decision` responsibility for EDE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-005-example",
  "spec_id": "SPEC-005",
  ...
  "payload": {
    "engine": "Engineering Decision Engine",
    "source_class": "EngineeringDecisionEngine",
    "public_methods": [
      "evaluate_decision(category: str, alternatives: List[str], weights: Optional[dict]) -> dict",
      "rollback_decision(category: str, target_version: int) -> Optional[dict]"
    ]
  }
}
```
...
- `AETHERIS_EDE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EDE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EDE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. EDE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EDE_STARTED` with request ID, spec ID, source path, and upstream references.
- `EDE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EDE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- EDE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EDE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EDE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_ede.py`.
...
======================================================================
- Unit tests should instantiate `EngineeringDecisionEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-004` | Upstream | Must provide validated evidence before EDE starts. |
| `SPEC-006` | Downstream | Must consume only the validated EDE output contract. |
| `src/intelligence/ede.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_ede.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_ede.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `EngineeringDecisionEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EDE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/ede.py
tests/test_ede.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-005-EDE.md
```

Guidance:
- Keep `EngineeringDecisionEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ede(payload: dict) -> dict:
    input_contract = validate_ede_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = EngineeringDecisionEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ede_output(input_contract, result)
    validate_ede_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/ede.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-004"] --> I["Input Validator"]
    I --> E["SPEC-005 EDE: Engineering Decision Engine"]
...
graph LR
    ...
    V --> Core["EngineeringDecisionEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EDE Payload"]
...
```mermaid
graph TD
    Parent["RFC-001"] --> This["SPEC-005"]
    This --> Source["src/intelligence/ede.py"]
    This --> Test["tests/test_ede.py"]
    This --> Next["SPEC-006"]
```
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EDE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-004" as U
rectangle "Input Validator" as V
rectangle "SPEC-005 EDE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-006" as D
U --> V
V --> E
E --> K
K --> D
...
class "EDEInputContract"
class "EDEOutputContract"
EngineeringDecisionEngine --> "EDEInputContract"
EngineeringDecisionEngine --> "EDEOutputContract"
@enduml
```
