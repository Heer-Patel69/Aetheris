# SPEC-023: Risk Planning Engine (RPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Technical, security, scalability, and delivery risk planning
...
Primary Class: `RiskPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Risk Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Identifies risks, mitigation strategies, failure likelihood, impact scores, and production readiness concerns.

This document upgrades SPEC-023 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-023 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of RPE is to own the full engineering responsibility for technical, security, scalability, and delivery risk planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding RPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct technical, security, scalability, and delivery risk planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for technical, security, scalability, and delivery risk planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-022.
- Emit structured output that can be consumed by SPEC-024.
...
======================================================================
In scope:
- Technical, security, scalability, and delivery risk planning
- Validation of all RPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-023` input contract to `SPEC-023` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `RiskPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
RPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- RPE validates and transforms that evidence.
...
======================================================================
- FR-001: RPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-022.
- FR-002: RPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: RPE shall execute the primary behavior owned by `RiskPlanningEngine`.
- FR-004: RPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-023 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat RPE output as the authoritative contract for technical, security, scalability, and delivery risk planning unless a later validation SPEC rejects it.
...
Primary implementation class: `RiskPlanningEngine`.
...
======================================================================
RPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-023` owns how the engine is implemented.
...
RPE must exchange data by contract, not by implicit global state.
...
4. Execute `RiskPlanningEngine` behavior for technical, security, scalability, and delivery risk planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-022.
...
- `RiskPlanningEngine`: Owns the core technical, security, scalability, and delivery risk planning behavior.
...
- Input interface: `RPEInput` JSON object.
- Output interface: `RPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `RiskPlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for RPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-023-example",
  "spec_id": "SPEC-023",
  ...
  "payload": {
    "engine": "Risk Planning Engine",
    "source_class": "RiskPlanningEngine",
    "public_methods": [
...
- `AETHERIS_RPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_RPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_RPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. RPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `RPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `RPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `RPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- RPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
RPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. RPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `RiskPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before RPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before RPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before RPE starts. |
| `SPEC-022` | Upstream | Must provide validated evidence before RPE starts. |
| `SPEC-024` | Downstream | Must consume only the validated RPE output contract. |
...
| FR-003 core execution | Direct `RiskPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: RPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-023-RPE.md
```

Guidance:
- Keep `RiskPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_rpe(payload: dict) -> dict:
    input_contract = validate_rpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = RiskPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_rpe_output(input_contract, result)
    validate_rpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-023 RPE: Risk Planning Engine"]
...
graph LR
    ...
    V --> Core["RiskPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["RPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute RPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-023 RPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-024" as D
U --> V
V --> E
E --> K
K --> D
...
class "RPEInputContract"
class "RPEOutputContract"
RiskPlanningEngine --> "RPEInputContract"
RiskPlanningEngine --> "RPEOutputContract"
@enduml
```
