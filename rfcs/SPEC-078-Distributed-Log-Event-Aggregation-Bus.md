# SPEC-078: Distributed Log Event Aggregation Bus (DLEAB)
...
Implementation: `src/runtime/log_bus.py` (source implementation pending)
Primary Class: `DistributedLogEventAggregationBus`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Distributed Log Event Aggregation Bus exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Aggregates structured logs and events from runtime nodes into durable streams for observability, incident response, and learning.

This document upgrades SPEC-078 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-078 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of DLEAB is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding DLEAB behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct wave 5 - runtime infrastructure was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for wave 5 - runtime infrastructure.
- Accept only validated upstream artifacts from SPEC-070 IPCF, SPEC-071 DEE, runtime events.
- Emit structured output that can be consumed by SPEC-045 EME, SPEC-098 LAE.
...
======================================================================
In scope:
- Wave 5 - Runtime Infrastructure
- Validation of all DLEAB input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-078` input contract to `SPEC-078` output contract.
- Use `src/runtime/log_bus.py` as the implementation boundary and `DistributedLogEventAggregationBus` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
DLEAB follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- DLEAB validates and transforms that evidence.
...
======================================================================
- FR-001: DLEAB shall load the upstream artifacts declared by SPEC-070 IPCF, SPEC-071 DEE, runtime events.
- FR-002: DLEAB shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: DLEAB shall execute the primary behavior owned by `DistributedLogEventAggregationBus`.
- FR-004: DLEAB shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
======================================================================
- Latency target: complete runtime control-plane operations in less than 50 milliseconds excluding network or worker execution latency.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-078 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat DLEAB output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/log_bus.py`.

Primary implementation class: `DistributedLogEventAggregationBus`.

Implementation classes in scope:
- `DistributedLogEventAggregationBus`
...
======================================================================
DLEAB is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `RFC-005` owns why this engine exists.
- `SPEC-078` owns how the engine is implemented.
...
DLEAB must exchange data by contract, not by implicit global state.
...
4. Execute `DistributedLogEventAggregationBus` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-070 IPCF, SPEC-071 DEE, runtime events.
...
- `DistributedLogEventAggregationBus`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Aggregates structured logs and events from runtime nodes into durable streams for observability, incident response, and learning.
...
- Input interface: `DLEABInput` JSON object.
- Output interface: `DLEABOutput` JSON object.
- Source interface: `src/runtime/log_bus.py`.
- Public class boundary: `DistributedLogEventAggregationBus`.
...
|---|---|---|
| `DistributedLogEventAggregationBus.run(payload: dict) -> dict` | Executes the `DistributedLogEventAggregationBus.run` responsibility for DLEAB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-078-example",
  "spec_id": "SPEC-078",
  ...
  "payload": {
    "engine": "Distributed Log Event Aggregation Bus",
    "source_class": "DistributedLogEventAggregationBus",
    "public_methods": []
  }
}
```
...
- `AETHERIS_DLEAB_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_DLEAB_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_DLEAB_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/log_bus.py` implementation is primarily in-memory. The enterprise contract still requires DLEAB to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. DLEAB may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `DLEAB_STARTED` with request ID, spec ID, source path, and upstream references.
- `DLEAB_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `DLEAB_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
- Primary latency target: complete runtime control-plane operations in less than 50 milliseconds excluding network or worker execution latency.
...
======================================================================
Scalability model:
- DLEAB must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
DLEAB is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. DLEAB should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/plans/runtime-infrastructure-test-plan.md`.
...
======================================================================
- Unit tests should instantiate `DistributedLogEventAggregationBus` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-070 IPCF` | Upstream | Must provide validated evidence before DLEAB starts. |
| `SPEC-071 DEE` | Upstream | Must provide validated evidence before DLEAB starts. |
| `runtime events` | Upstream | Must provide validated evidence before DLEAB starts. |
| `SPEC-045 EME` | Downstream | Must consume only the validated DLEAB output contract. |
| `SPEC-098 LAE` | Downstream | Must consume only the validated DLEAB output contract. |
| `src/runtime/log_bus.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/plans/runtime-infrastructure-test-plan.md` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/plans/runtime-infrastructure-test-plan.md` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `DistributedLogEventAggregationBus` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: DLEAB grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/log_bus.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-078-Distributed-Log-Event-Aggregation-Bus.md
```

Guidance:
- Keep `DistributedLogEventAggregationBus` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_dleab(payload: dict) -> dict:
    input_contract = validate_dleab_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = DistributedLogEventAggregationBus(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_dleab_output(input_contract, result)
    validate_dleab_output(output)
...
- `DistributedLogEventAggregationBus.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/log_bus.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-070 IPCF"] --> I["Input Validator"]
    I --> E["SPEC-078 DLEAB: Distributed Log Event Aggregation Bus"]
...
graph LR
    ...
    V --> Core["DistributedLogEventAggregationBus Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["DLEAB Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-078"]
    This --> Source["src/runtime/log_bus.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-045 EME"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute DLEAB"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-070 IPCF" as U
rectangle "Input Validator" as V
rectangle "SPEC-078 DLEAB" as E
database "EKB / Artifacts" as K
rectangle "SPEC-045 EME" as D
U --> V
V --> E
E --> K
K --> D
...
```plantuml
@startuml
class DistributedLogEventAggregationBus {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "DLEABInputContract"
class "DLEABOutputContract"
DistributedLogEventAggregationBus --> "DLEABInputContract"
DistributedLogEventAggregationBus --> "DLEABOutputContract"
@enduml
```
...
- `rfcs/RFC-005`
- `src/runtime/log_bus.py`
- `tests/plans/runtime-infrastructure-test-plan.md`
