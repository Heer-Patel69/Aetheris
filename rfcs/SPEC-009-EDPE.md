# SPEC-009: Engineering Design Planning Engine (EDPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Design language, UI tokens, and accessibility planning
...
Primary Class: `DesignPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Engineering Design Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Generates design language tokens, visual style guidance, motion rules, accessibility targets, and reusable view templates.

This document upgrades SPEC-009 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-009 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of EDPE is to own the full engineering responsibility for design language, ui tokens, and accessibility planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EDPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct design language, ui tokens, and accessibility planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for design language, ui tokens, and accessibility planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB.
- Emit structured output that can be consumed by SPEC-010.
...
======================================================================
In scope:
- Design language, UI tokens, and accessibility planning
- Validation of all EDPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-009` input contract to `SPEC-009` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `DesignPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EDPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EDPE validates and transforms that evidence.
...
======================================================================
- FR-001: EDPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB.
- FR-002: EDPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EDPE shall execute the primary behavior owned by `DesignPlanningEngine`.
- FR-004: EDPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-009 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EDPE output as the authoritative contract for design language, ui tokens, and accessibility planning unless a later validation SPEC rejects it.
...
Primary implementation class: `DesignPlanningEngine`.
...
======================================================================
EDPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-009` owns how the engine is implemented.
...
EDPE must exchange data by contract, not by implicit global state.
...
4. Execute `DesignPlanningEngine` behavior for design language, ui tokens, and accessibility planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB.
...
- `DesignPlanningEngine`: Owns the core design language, ui tokens, and accessibility planning behavior.
...
- Input interface: `EDPEInput` JSON object.
- Output interface: `EDPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `DesignPlanningEngine`.
...
|---|---|---|
| `plan_design(product_plan: dict) -> dict` | Executes the `plan_design` responsibility for EDPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-009-example",
  "spec_id": "SPEC-009",
  ...
  "payload": {
    "engine": "Engineering Design Planning Engine",
    "source_class": "DesignPlanningEngine",
    "public_methods": [
      "plan_design(product_plan: dict) -> dict"
    ]
  }
}
```
...
- `AETHERIS_EDPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EDPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EDPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. EDPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EDPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `EDPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EDPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- EDPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EDPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EDPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `DesignPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before EDPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before EDPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before EDPE starts. |
| `SPEC-010` | Downstream | Must consume only the validated EDPE output contract. |
...
| FR-003 core execution | Direct `DesignPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EDPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-009-EDPE.md
```

Guidance:
- Keep `DesignPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_edpe(payload: dict) -> dict:
    input_contract = validate_edpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = DesignPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_edpe_output(input_contract, result)
    validate_edpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-009 EDPE: Engineering Design Planning Engine"]
...
graph LR
    ...
    V --> Core["DesignPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EDPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EDPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-009 EDPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-010" as D
U --> V
V --> E
E --> K
K --> D
...
class "EDPEInputContract"
class "EDPEOutputContract"
DesignPlanningEngine --> "EDPEInputContract"
DesignPlanningEngine --> "EDPEOutputContract"
@enduml
```
