# SPEC-007: Engineering Knowledge Base (EKB)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-001
Layer: Knowledge Storage
Scope: Versioned engineering memory
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/ekb.py` (validated from source metadata)
Primary Class: `EngineeringKnowledgeBase`
Test Reference: `tests/test_ekb.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Engineering Knowledge Base exists to make Knowledge Storage deterministic, reviewable, and safe at production scale. Persists typed engineering objects with checksums, version history, query filters, integrity checks, and deterministic object identifiers.

This document upgrades SPEC-007 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-007 is part of the Engineering Knowledge Compiler, where evidence is discovered, normalized, stored, and queried before planning begins.

======================================================================
2. PURPOSE
======================================================================
The purpose of EKB is to own the full engineering responsibility for versioned engineering memory. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EKB behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct versioned engineering memory was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for versioned engineering memory.
- Accept only validated upstream artifacts from SPEC-006.
- Emit structured output that can be consumed by SPEC-008.
...
======================================================================
In scope:
- Versioned engineering memory
- Validation of all EKB input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-007` input contract to `SPEC-007` output contract.
- Use `src/intelligence/ekb.py` as the implementation boundary and `EngineeringKnowledgeBase` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EKB follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EKB validates and transforms that evidence.
...
======================================================================
- FR-001: EKB shall load the upstream artifacts declared by SPEC-006.
- FR-002: EKB shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EKB shall execute the primary behavior owned by `EngineeringKnowledgeBase`.
- FR-004: EKB shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-007 sits in `Knowledge Storage` under `RFC-001`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EKB output as the authoritative contract for versioned engineering memory unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/ekb.py`.

Primary implementation class: `EngineeringKnowledgeBase`.
...
======================================================================
EKB is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-007` owns how the engine is implemented.
...
EKB must exchange data by contract, not by implicit global state.
...
4. Execute `EngineeringKnowledgeBase` behavior for versioned engineering memory.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-006.
...
- `EngineeringKnowledgeBase`: Owns the core versioned engineering memory behavior.
...
- Input interface: `EKBInput` JSON object.
- Output interface: `EKBOutput` JSON object.
- Source interface: `src/intelligence/ekb.py`.
- Public class boundary: `EngineeringKnowledgeBase`.
...
|---|---|---|
| `register_object(obj_type: str, content: dict, producer: str) -> str` | Executes the `register_object` responsibility for EKB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `get_object(obj_id: str, version: Optional[int]) -> Optional[dict]` | Executes the `get_object` responsibility for EKB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `query_objects(filters: dict) -> List[dict]` | Executes the `query_objects` responsibility for EKB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `purge_all() -> None` | Executes the `purge_all` responsibility for EKB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-007-example",
  "spec_id": "SPEC-007",
  ...
  "payload": {
    "engine": "Engineering Knowledge Base",
    "source_class": "EngineeringKnowledgeBase",
    "public_methods": [
      "register_object(obj_type: str, content: dict, producer: str) -> str",
      "get_object(obj_id: str, version: Optional[int]) -> Optional[dict]",
      "query_objects(filters: dict) -> List[dict]",
      "purge_all() -> None"
    ]
  }
}
...
- `AETHERIS_EKB_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EKB_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EKB_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
- `*.json`
- `{obj_id}.json`
- `{obj_id}_history.json`
- `{obj_id}_v{version}.json`
...
======================================================================
Recovery is conservative. EKB may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EKB_STARTED` with request ID, spec ID, source path, and upstream references.
- `EKB_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EKB_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- EKB must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EKB is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EKB should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/test_ekb.py`.
...
======================================================================
- Unit tests should instantiate `EngineeringKnowledgeBase` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-006` | Upstream | Must provide validated evidence before EKB starts. |
| `SPEC-008` | Downstream | Must consume only the validated EKB output contract. |
| `src/intelligence/ekb.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/test_ekb.py` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/test_ekb.py` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `EngineeringKnowledgeBase` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EKB grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-007-EKB.md
```

Guidance:
- Keep `EngineeringKnowledgeBase` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ekb(payload: dict) -> dict:
    input_contract = validate_ekb_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = EngineeringKnowledgeBase(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ekb_output(input_contract, result)
    validate_ekb_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/ekb.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-006"] --> I["Input Validator"]
    I --> E["SPEC-007 EKB: Engineering Knowledge Base"]
...
graph LR
    ...
    V --> Core["EngineeringKnowledgeBase Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EKB Payload"]
...
```mermaid
graph TD
    Parent["RFC-001"] --> This["SPEC-007"]
    This --> Source["src/intelligence/ekb.py"]
    This --> Test["tests/test_ekb.py"]
    This --> Next["SPEC-008"]
```
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EKB"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-006" as U
rectangle "Input Validator" as V
rectangle "SPEC-007 EKB" as E
database "EKB / Artifacts" as K
rectangle "SPEC-008" as D
U --> V
V --> E
E --> K
K --> D
...
class "EKBInputContract"
class "EKBOutputContract"
EngineeringKnowledgeBase --> "EKBInputContract"
EngineeringKnowledgeBase --> "EKBOutputContract"
@enduml
```
