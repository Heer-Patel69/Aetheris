# SPEC-061: Context Optimization Engine (COE2)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-004
Layer: Engineering Intelligence Layer
Scope: Wave 3 - Optimization
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/intelligence/coe2.py` (validated from source metadata)
Primary Class: `ContextOptimizationEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Context Optimization Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Filters context packages by relevance, confidence, freshness, dependency reachability, and available token budget.

This document upgrades SPEC-061 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-061 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of COE2 is to own the full engineering responsibility for wave 3 - optimization. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding COE2 behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct wave 3 - optimization was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for wave 3 - optimization.
- Accept only validated upstream artifacts from SPEC-052 LCE, SPEC-058 TOE, SPEC-060 EOE.
- Emit structured output that can be consumed by SPEC-062 DSEE, SPEC-065 IO.
...
In scope:
- Wave 3 - Optimization
- Validation of all COE2 input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-061` input contract to `SPEC-061` output contract.
- Use `src/intelligence/coe2.py` as the implementation boundary and `ContextOptimizationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
COE2 follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- COE2 validates and transforms that evidence.
...
======================================================================
- FR-001: COE2 shall load the upstream artifacts declared by SPEC-052 LCE, SPEC-058 TOE, SPEC-060 EOE.
- FR-002: COE2 shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: COE2 shall execute the primary behavior owned by `ContextOptimizationEngine`.
- FR-004: COE2 shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-061 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat COE2 output as the authoritative contract for wave 3 - optimization unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/coe2.py`.

Primary implementation class: `ContextOptimizationEngine`.
...
======================================================================
COE2 is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-061` owns how the engine is implemented.
...
COE2 must exchange data by contract, not by implicit global state.
...
4. Execute `ContextOptimizationEngine` behavior for wave 3 - optimization.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-052 LCE, SPEC-058 TOE, SPEC-060 EOE.
...
- `ContextOptimizationEngine`: Owns the core wave 3 - optimization behavior.
...
- Filters context packages by relevance, confidence, freshness, dependency reachability, and available token budget.
...
- Input interface: `COE2Input` JSON object.
- Output interface: `COE2Output` JSON object.
- Source interface: `src/intelligence/coe2.py`.
- Public class boundary: `ContextOptimizationEngine`.
...
|---|---|---|
| `filter_context(context: list) -> list` | Executes the `filter_context` responsibility for COE2. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/intelligence/coe2.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-061-example",
  "spec_id": "SPEC-061",
  ...
  "payload": {
    "engine": "Context Optimization Engine",
    "source_class": "ContextOptimizationEngine",
    "public_methods": [
      "filter_context(context: list) -> list"
    ]
  }
}
```
...
- `AETHERIS_COE2_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_COE2_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_COE2_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/coe2.py` implementation is primarily in-memory. The enterprise contract still requires COE2 to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. COE2 may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `COE2_STARTED` with request ID, spec ID, source path, and upstream references.
- `COE2_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `COE2_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- COE2 must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
COE2 is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. COE2 should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ContextOptimizationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-052 LCE` | Upstream | Must provide validated evidence before COE2 starts. |
| `SPEC-058 TOE` | Upstream | Must provide validated evidence before COE2 starts. |
| `SPEC-060 EOE` | Upstream | Must provide validated evidence before COE2 starts. |
| `SPEC-062 DSEE` | Downstream | Must consume only the validated COE2 output contract. |
| `SPEC-065 IO` | Downstream | Must consume only the validated COE2 output contract. |
| `src/intelligence/coe2.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ContextOptimizationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: COE2 grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/coe2.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-061-COE2.md
```

Guidance:
- Keep `ContextOptimizationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_coe2(payload: dict) -> dict:
    input_contract = validate_coe2_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ContextOptimizationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_coe2_output(input_contract, result)
    validate_coe2_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/coe2.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-052 LCE"] --> I["Input Validator"]
    I --> E["SPEC-061 COE2: Context Optimization Engine"]
...
graph LR
    ...
    V --> Core["ContextOptimizationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["COE2 Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-061"]
    This --> Source["src/intelligence/coe2.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-062 DSEE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute COE2"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-052 LCE" as U
rectangle "Input Validator" as V
rectangle "SPEC-061 COE2" as E
database "EKB / Artifacts" as K
rectangle "SPEC-062 DSEE" as D
U --> V
V --> E
E --> K
K --> D
...
class "COE2InputContract"
class "COE2OutputContract"
ContextOptimizationEngine --> "COE2InputContract"
ContextOptimizationEngine --> "COE2OutputContract"
@enduml
```
