# SPEC-090: Success Knowledge Engine (SKE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Success memory, quality signal extraction, and reuse ranking
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/success_knowledge.py` (source implementation pending)
Primary Class: `SuccessKnowledgeEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Success Knowledge Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Captures successful delivery paths, high-quality outputs, passing verification evidence, and low-cost execution strategies for future reuse.

This document upgrades SPEC-090 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-090 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of SKE is to own the full engineering responsibility for success memory, quality signal extraction, and reuse ranking. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding SKE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct success memory, quality signal extraction, and reuse ranking was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for success memory, quality signal extraction, and reuse ranking.
- Accept only validated upstream artifacts from SPEC-086 EME2, SPEC-040 SRE, passing tests.
- Emit structured output that can be consumed by SPEC-088 BPE2, SPEC-098 LAE.
...
======================================================================
In scope:
- Success memory, quality signal extraction, and reuse ranking
- Validation of all SKE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-090` input contract to `SPEC-090` output contract.
- Use `src/learning/success_knowledge.py` as the implementation boundary and `SuccessKnowledgeEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
SKE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- SKE validates and transforms that evidence.
...
======================================================================
- FR-001: SKE shall load the upstream artifacts declared by SPEC-086 EME2, SPEC-040 SRE, passing tests.
- FR-002: SKE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SKE shall execute the primary behavior owned by `SuccessKnowledgeEngine`.
- FR-004: SKE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-090 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SKE output as the authoritative contract for success memory, quality signal extraction, and reuse ranking unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/success_knowledge.py`.

Primary implementation class: `SuccessKnowledgeEngine`.

Implementation classes in scope:
- `SuccessKnowledgeEngine`
...
======================================================================
SKE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-090` owns how the engine is implemented.
...
SKE must exchange data by contract, not by implicit global state.
...
4. Execute `SuccessKnowledgeEngine` behavior for success memory, quality signal extraction, and reuse ranking.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-086 EME2, SPEC-040 SRE, passing tests.
...
- `SuccessKnowledgeEngine`: Owns the core success memory, quality signal extraction, and reuse ranking behavior.
...
- Captures successful delivery paths, high-quality outputs, passing verification evidence, and low-cost execution strategies for future reuse.
...
- Input interface: `SKEInput` JSON object.
- Output interface: `SKEOutput` JSON object.
- Source interface: `src/learning/success_knowledge.py`.
- Public class boundary: `SuccessKnowledgeEngine`.
...
|---|---|---|
| `SuccessKnowledgeEngine.run(payload: dict) -> dict` | Executes the `SuccessKnowledgeEngine.run` responsibility for SKE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/success_knowledge.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-090-example",
  "spec_id": "SPEC-090",
  ...
  "payload": {
    "engine": "Success Knowledge Engine",
    "source_class": "SuccessKnowledgeEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_SKE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_SKE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_SKE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/success_knowledge.py` implementation is primarily in-memory. The enterprise contract still requires SKE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. SKE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `SKE_STARTED` with request ID, spec ID, source path, and upstream references.
- `SKE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `SKE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- SKE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
SKE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. SKE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `SuccessKnowledgeEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-086 EME2` | Upstream | Must provide validated evidence before SKE starts. |
| `SPEC-040 SRE` | Upstream | Must provide validated evidence before SKE starts. |
| `passing tests` | Upstream | Must provide validated evidence before SKE starts. |
| `SPEC-088 BPE2` | Downstream | Must consume only the validated SKE output contract. |
| `SPEC-098 LAE` | Downstream | Must consume only the validated SKE output contract. |
| `src/learning/success_knowledge.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `SuccessKnowledgeEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: SKE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/success_knowledge.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-090-SKE.md
```

Guidance:
- Keep `SuccessKnowledgeEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ske(payload: dict) -> dict:
    input_contract = validate_ske_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = SuccessKnowledgeEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ske_output(input_contract, result)
    validate_ske_output(output)
...
- `SuccessKnowledgeEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/success_knowledge.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-090 SKE: Success Knowledge Engine"]
...
graph LR
    ...
    V --> Core["SuccessKnowledgeEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["SKE Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-090"]
    This --> Source["src/learning/success_knowledge.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute SKE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
class "SKEInputContract"
class "SKEOutputContract"
SuccessKnowledgeEngine --> "SKEInputContract"
SuccessKnowledgeEngine --> "SKEOutputContract"
@enduml
```
