# SPEC-010: Frontend Planning Engine (FPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Frontend route, page, component, and state planning
...
Primary Class: `FrontendPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Frontend Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Generates frontend routing, application shells, component hierarchy, state context strategy, local caching, and responsive layout expectations.

This document upgrades SPEC-010 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-010 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of FPE is to own the full engineering responsibility for frontend route, page, component, and state planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding FPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct frontend route, page, component, and state planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for frontend route, page, component, and state planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-009.
- Emit structured output that can be consumed by SPEC-011.
...
======================================================================
In scope:
- Frontend route, page, component, and state planning
- Validation of all FPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-010` input contract to `SPEC-010` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `FrontendPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
FPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- FPE validates and transforms that evidence.
...
======================================================================
- FR-001: FPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-009.
- FR-002: FPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: FPE shall execute the primary behavior owned by `FrontendPlanningEngine`.
- FR-004: FPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-010 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat FPE output as the authoritative contract for frontend route, page, component, and state planning unless a later validation SPEC rejects it.
...
Primary implementation class: `FrontendPlanningEngine`.
...
======================================================================
FPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-010` owns how the engine is implemented.
...
FPE must exchange data by contract, not by implicit global state.
...
4. Execute `FrontendPlanningEngine` behavior for frontend route, page, component, and state planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-009.
...
- `FrontendPlanningEngine`: Owns the core frontend route, page, component, and state planning behavior.
...
- Input interface: `FPEInput` JSON object.
- Output interface: `FPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `FrontendPlanningEngine`.
...
|---|---|---|
| `plan_frontend(product_plan: dict, design_plan: dict) -> dict` | Executes the `plan_frontend` responsibility for FPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-010-example",
  "spec_id": "SPEC-010",
  ...
  "payload": {
    "engine": "Frontend Planning Engine",
    "source_class": "FrontendPlanningEngine",
    "public_methods": [
      "plan_frontend(product_plan: dict, design_plan: dict) -> dict"
    ]
  }
}
```
...
- `AETHERIS_FPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_FPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_FPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
- `components.plan.json`
- `pages.plan.json`
- `routing.plan.json`
- `states.plan.json`

EKB object registrations owned by this SPEC:
- `frontend_components_plan`
- `frontend_pages_plan`
- `frontend_routing_plan`
- `frontend_states_plan`
...
======================================================================
Recovery is conservative. FPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `FPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `FPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `FPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- FPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
FPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. FPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `FrontendPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before FPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before FPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before FPE starts. |
| `SPEC-009` | Upstream | Must provide validated evidence before FPE starts. |
| `SPEC-011` | Downstream | Must consume only the validated FPE output contract. |
...
| FR-003 core execution | Direct `FrontendPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: FPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-010-FPE.md
```

Guidance:
- Keep `FrontendPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_fpe(payload: dict) -> dict:
    input_contract = validate_fpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = FrontendPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_fpe_output(input_contract, result)
    validate_fpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-010 FPE: Frontend Planning Engine"]
...
graph LR
    ...
    V --> Core["FrontendPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["FPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute FPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-010 FPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-011" as D
U --> V
V --> E
E --> K
K --> D
...
class "FPEInputContract"
class "FPEOutputContract"
FrontendPlanningEngine --> "FPEInputContract"
FrontendPlanningEngine --> "FPEOutputContract"
@enduml
```
