# SPEC-040: Self Review Engine (SRE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Generated-code review and quality scoring
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/sre.py` (validated from source metadata)
Primary Class: `SelfReviewEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Self Review Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Reviews generated modifications for style, architecture, security, smells, recommendations, approval, and rejection decisions.

This document upgrades SPEC-040 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-040 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of SRE is to own the full engineering responsibility for generated-code review and quality scoring. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding SRE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct generated-code review and quality scoring was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for generated-code review and quality scoring.
- Accept only validated upstream artifacts from SPEC-039.
- Emit structured output that can be consumed by SPEC-041.
...
======================================================================
In scope:
- Generated-code review and quality scoring
- Validation of all SRE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-040` input contract to `SPEC-040` output contract.
- Use `src/execution/sre.py` as the implementation boundary and `SelfReviewEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
SRE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- SRE validates and transforms that evidence.
...
======================================================================
- FR-001: SRE shall load the upstream artifacts declared by SPEC-039.
- FR-002: SRE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SRE shall execute the primary behavior owned by `SelfReviewEngine`.
- FR-004: SRE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-040 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SRE output as the authoritative contract for generated-code review and quality scoring unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/sre.py`.

Primary implementation class: `SelfReviewEngine`.
...
======================================================================
SRE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-040` owns how the engine is implemented.
...
SRE must exchange data by contract, not by implicit global state.
...
4. Execute `SelfReviewEngine` behavior for generated-code review and quality scoring.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-039.
...
- `SelfReviewEngine`: Owns the core generated-code review and quality scoring behavior.
...
- Input interface: `SREInput` JSON object.
- Output interface: `SREOutput` JSON object.
- Source interface: `src/execution/sre.py`.
- Public class boundary: `SelfReviewEngine`.
...
|---|---|---|
| `review(files: List[dict]) -> dict` | Executes the `review` responsibility for SRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `score_quality(findings: List[dict]) -> float` | Executes the `score_quality` responsibility for SRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `detect_smells(files: List[dict]) -> List[dict]` | Executes the `detect_smells` responsibility for SRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `recommend(findings: List[dict]) -> List[dict]` | Executes the `recommend` responsibility for SRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `approve(review_id: str) -> bool` | Executes the `approve` responsibility for SRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `reject(review_id: str) -> bool` | Executes the `reject` responsibility for SRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-040-example",
  "spec_id": "SPEC-040",
  ...
  "payload": {
    ...
    "source_class": "SelfReviewEngine",
    "public_methods": [
      "review(files: List[dict]) -> dict",
      "score_quality(findings: List[dict]) -> float",
      "detect_smells(files: List[dict]) -> List[dict]",
      "recommend(findings: List[dict]) -> List[dict]",
      "approve(review_id: str) -> bool"
    ]
  }
}
```
...
- `AETHERIS_SRE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_SRE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_SRE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. SRE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `SRE_STARTED` with request ID, spec ID, source path, and upstream references.
- `SRE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `SRE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- SRE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
SRE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. SRE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `SelfReviewEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-039` | Upstream | Must provide validated evidence before SRE starts. |
| `SPEC-041` | Downstream | Must consume only the validated SRE output contract. |
| `src/execution/sre.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `SelfReviewEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: SRE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/execution/sre.py
tests/test_execution.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-040-SRE.md
```

Guidance:
- Keep `SelfReviewEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_sre(payload: dict) -> dict:
    input_contract = validate_sre_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = SelfReviewEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_sre_output(input_contract, result)
    validate_sre_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/sre.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-039"] --> I["Input Validator"]
    I --> E["SPEC-040 SRE: Self Review Engine"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["SRE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-040"]
    This --> Source["src/execution/sre.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute SRE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-039" as U
rectangle "Input Validator" as V
rectangle "SPEC-040 SRE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-041" as D
U --> V
V --> E
E --> K
K --> D
...
class "SREInputContract"
class "SREOutputContract"
SelfReviewEngine --> "SREInputContract"
SelfReviewEngine --> "SREOutputContract"
@enduml
```
