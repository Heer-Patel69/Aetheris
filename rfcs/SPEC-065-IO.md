# SPEC-065: Intelligence Orchestrator (IO)
...
Implementation: `src/intelligence/io.py` (validated from source metadata)
Primary Class: `IntelligenceOrchestrator`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Intelligence Orchestrator exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Coordinates the intelligence pipeline, assembles the final intelligence package, and hands verified guidance to the execution runtime.

This document upgrades SPEC-065 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-065 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of IO is to own the full engineering responsibility for wave 4 - evolution. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding IO behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-048 through SPEC-064.
- Emit structured output that can be consumed by RFC-003 AEE, SPEC-032 TDE.
...
In scope:
- Wave 4 - Evolution
- Validation of all IO input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-065` input contract to `SPEC-065` output contract.
- Use `src/intelligence/io.py` as the implementation boundary and `IntelligenceOrchestrator` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
IO follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- IO validates and transforms that evidence.
...
======================================================================
- FR-001: IO shall load the upstream artifacts declared by SPEC-048 through SPEC-064.
- FR-002: IO shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: IO shall execute the primary behavior owned by `IntelligenceOrchestrator`.
- FR-004: IO shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-065 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.

Upstream dependencies:
- SPEC-048 through SPEC-064

Downstream consumers:
- RFC-003 AEE
- SPEC-032 TDE

The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat IO output as the authoritative contract for wave 4 - evolution unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/io.py`.

Primary implementation class: `IntelligenceOrchestrator`.
...
======================================================================
IO is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-065` owns how the engine is implemented.
...
IO must exchange data by contract, not by implicit global state.
...
4. Execute `IntelligenceOrchestrator` behavior for wave 4 - evolution.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-048 through SPEC-064.
...
- `IntelligenceOrchestrator`: Owns the core wave 4 - evolution behavior.
...
- Coordinates the intelligence pipeline, assembles the final intelligence package, and hands verified guidance to the execution runtime.
...
- Input interface: `IOInput` JSON object.
- Output interface: `IOOutput` JSON object.
- Source interface: `src/intelligence/io.py`.
- Public class boundary: `IntelligenceOrchestrator`.
...
|---|---|---|
| `assemble_package(goal: str) -> dict` | Executes the `assemble_package` responsibility for IO. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-065-example",
  "spec_id": "SPEC-065",
  ...
  "payload": {
    ...
    "source_class": "IntelligenceOrchestrator",
    "public_methods": [
      "assemble_package(goal: str) -> dict"
    ]
  }
}
```
...
- `AETHERIS_IO_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_IO_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_IO_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/io.py` implementation is primarily in-memory. The enterprise contract still requires IO to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. IO may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `IO_STARTED` with request ID, spec ID, source path, and upstream references.
- `IO_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `IO_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- IO must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
IO is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. IO should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `IntelligenceOrchestrator` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-048 through SPEC-064` | Upstream | Must provide validated evidence before IO starts. |
| `RFC-003 AEE` | Downstream | Must consume only the validated IO output contract. |
| `SPEC-032 TDE` | Downstream | Must consume only the validated IO output contract. |
| `src/intelligence/io.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `IntelligenceOrchestrator` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: IO grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
Guidance:
- Keep `IntelligenceOrchestrator` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_io(payload: dict) -> dict:
    input_contract = validate_io_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = IntelligenceOrchestrator(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_io_output(input_contract, result)
    validate_io_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/io.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-048 through SPEC-064"] --> I["Input Validator"]
    I --> E["SPEC-065 IO: Intelligence Orchestrator"]
...
graph LR
    ...
    V --> Core["IntelligenceOrchestrator Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["IO Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-065"]
    This --> Source["src/intelligence/io.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["RFC-003 AEE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute IO"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-048 through SPEC-064" as U
rectangle "Input Validator" as V
rectangle "SPEC-065 IO" as E
database "EKB / Artifacts" as K
rectangle "RFC-003 AEE" as D
U --> V
V --> E
E --> K
K --> D
...
class "IOInputContract"
class "IOOutputContract"
IntelligenceOrchestrator --> "IOInputContract"
IntelligenceOrchestrator --> "IOOutputContract"
@enduml
```
