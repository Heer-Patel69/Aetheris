# SPEC-077: Secure Vault Secrets Service (SVSS)
...
Implementation: `src/runtime/vault.py` (source implementation pending)
Primary Class: `SecureVaultSecretsService`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Secure Vault Secrets Service exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Stores, retrieves, rotates, and audits runtime secrets without exposing sensitive values to generated artifacts, logs, or model context.

This document upgrades SPEC-077 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-077 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of SVSS is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding SVSS behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from operator secrets, runtime credentials.
- Emit structured output that can be consumed by SPEC-076 SEE, SPEC-082 CITE.
...
- Validation of all SVSS input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-077` input contract to `SPEC-077` output contract.
- Use `src/runtime/vault.py` as the implementation boundary and `SecureVaultSecretsService` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
SVSS follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- SVSS validates and transforms that evidence.
...
======================================================================
- FR-001: SVSS shall load the upstream artifacts declared by operator secrets, runtime credentials.
- FR-002: SVSS shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SVSS shall execute the primary behavior owned by `SecureVaultSecretsService`.
- FR-004: SVSS shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-077 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SVSS output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/vault.py`.

Primary implementation class: `SecureVaultSecretsService`.

Implementation classes in scope:
- `SecureVaultSecretsService`
...
======================================================================
SVSS is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-077` owns how the engine is implemented.
...
SVSS must exchange data by contract, not by implicit global state.
...
4. Execute `SecureVaultSecretsService` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: operator secrets, runtime credentials.
...
- `SecureVaultSecretsService`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Stores, retrieves, rotates, and audits runtime secrets without exposing sensitive values to generated artifacts, logs, or model context.
...
- Input interface: `SVSSInput` JSON object.
- Output interface: `SVSSOutput` JSON object.
- Source interface: `src/runtime/vault.py`.
- Public class boundary: `SecureVaultSecretsService`.
...
|---|---|---|
| `SecureVaultSecretsService.run(payload: dict) -> dict` | Executes the `SecureVaultSecretsService.run` responsibility for SVSS. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-077-example",
  "spec_id": "SPEC-077",
  ...
  "payload": {
    "engine": "Secure Vault Secrets Service",
    "source_class": "SecureVaultSecretsService",
    "public_methods": []
  }
}
```
...
- `AETHERIS_SVSS_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_SVSS_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_SVSS_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/vault.py` implementation is primarily in-memory. The enterprise contract still requires SVSS to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. SVSS may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `SVSS_STARTED` with request ID, spec ID, source path, and upstream references.
- `SVSS_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `SVSS_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- SVSS must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
SVSS is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. SVSS should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `SecureVaultSecretsService` directly and verify public method behavior.
...
|---|---|---|
| `operator secrets` | Upstream | Must provide validated evidence before SVSS starts. |
| `runtime credentials` | Upstream | Must provide validated evidence before SVSS starts. |
| `SPEC-076 SEE` | Downstream | Must consume only the validated SVSS output contract. |
| `SPEC-082 CITE` | Downstream | Must consume only the validated SVSS output contract. |
| `src/runtime/vault.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `SecureVaultSecretsService` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: SVSS grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/vault.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-077-Secure-Vault-Secrets-Service.md
```

Guidance:
- Keep `SecureVaultSecretsService` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_svss(payload: dict) -> dict:
    input_contract = validate_svss_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = SecureVaultSecretsService(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_svss_output(input_contract, result)
    validate_svss_output(output)
...
- `SecureVaultSecretsService.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/vault.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["operator secrets"] --> I["Input Validator"]
    I --> E["SPEC-077 SVSS: Secure Vault Secrets Service"]
...
graph LR
    ...
    V --> Core["SecureVaultSecretsService Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["SVSS Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-077"]
    This --> Source["src/runtime/vault.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-076 SEE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute SVSS"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "operator secrets" as U
rectangle "Input Validator" as V
rectangle "SPEC-077 SVSS" as E
database "EKB / Artifacts" as K
rectangle "SPEC-076 SEE" as D
U --> V
V --> E
E --> K
K --> D
...
```plantuml
@startuml
class SecureVaultSecretsService {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "SVSSInputContract"
class "SVSSOutputContract"
SecureVaultSecretsService --> "SVSSInputContract"
SecureVaultSecretsService --> "SVSSOutputContract"
@enduml
```
