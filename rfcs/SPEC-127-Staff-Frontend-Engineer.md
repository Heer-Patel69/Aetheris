# SPEC-127: Staff Frontend Engineer (SFE)
...
Implementation: `src/organization/frontend.py`
Primary Class: `StaffFrontendEngineerAgent`
Test Reference: `tests/test_rfc008_core.py`

======================================================================
1. MISSION
======================================================================
The Staff Frontend Engineer (SFE) exists to provide a permanent specialist role or orchestration component within the Aetheris AI Organization. It represents a professional software engineering persona designed to operate collaboratively, enforce quality, and deliver specific deliverables.
...
The agent consumes inputs defined by the `SPEC-127Input` schema. Key variables include:
- `request_id`: Unique transaction identifier.
- `spec_id`: The ID of this specification (SPEC-127).
- `payload`: Subsystem parameters.

Required Context Inputs:
| Input Source | Format | Purpose |
|---|---|---|
| UI design layouts, component styles, API payload definitions, accessibility reports. | Structured JSON | Contextual parameters for execution loops |
...
The agent produces outputs conforming to the `SPEC-127Output` schema. Outputs include:
...
|---|---|---|
| Web files, interface styles, test suites, accessibility statements. | Markdown/JSON | Workspace directories & EKB objects |
...
======================================================================
Interaction Scopes:
- Consults with UX/UI Designer; hooks up endpoints with Backend Engineer; reports to EM.
...
8. SUGGESTED MODULES
======================================================================
Suggested path structure: `src/organization/frontend.py`.
Modules in scope: `frontend_agent.py, component_gen.py, style_validator.py, accessibility_audit.py`
...
class StaffFrontendEngineerAgent:
    """
    Enterprise agent implementation representing the Staff Frontend Engineer role.
    Handles inputs, validates context, and executes role responsibilities.
    """
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.info("Initializing StaffFrontendEngineerAgent agent.")

    def execute_task(self, request_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        self.logger.info(f"Processing task {request_id} for SFE agent.")
        # 1. Input Validation
        if not payload:
        ...
        return {
            ...
            "result": result,
            "metadata": {
                "agent_role": "Staff Frontend Engineer",
                "version": "2.0.0"
            }
        }
    ...
    def _process_domain_logic(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"message": "Processed successfully by StaffFrontendEngineerAgent"}
```
...
======================================================================
Prometheus Metrics:
- `aetheris_agent_runs_total{role="SFE"}`: Total task runs.
- `aetheris_agent_failures_total{role="SFE"}`: Total execution errors.
- `aetheris_agent_latency_ms{role="SFE"}`: Task durations.
...
Integration Tests:
- Test message passing and handoff events between SFE and adjacent agents.

Test Command:
`pytest tests/test_rfc008_core.py -k "StaffFrontendEngineerAgent"`
