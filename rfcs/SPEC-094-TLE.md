# SPEC-094: Testing Learning Engine (TLE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Testing strategy learning and coverage improvement
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/testing_learning.py` (source implementation pending)
Primary Class: `TestingLearningEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Testing Learning Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Learns which tests catch defects, which verification gaps recur, and which test templates should be recommended for similar future systems.

This document upgrades SPEC-094 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-094 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of TLE is to own the full engineering responsibility for testing strategy learning and coverage improvement. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding TLE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct testing strategy learning and coverage improvement was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for testing strategy learning and coverage improvement.
- Accept only validated upstream artifacts from SPEC-018 TPE, SPEC-040 SRE, SPEC-087 PME.
- Emit structured output that can be consumed by SPEC-018 TPE, SPEC-031 PVE.
...
======================================================================
In scope:
- Testing strategy learning and coverage improvement
- Validation of all TLE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-094` input contract to `SPEC-094` output contract.
- Use `src/learning/testing_learning.py` as the implementation boundary and `TestingLearningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
TLE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- TLE validates and transforms that evidence.
...
======================================================================
- FR-001: TLE shall load the upstream artifacts declared by SPEC-018 TPE, SPEC-040 SRE, SPEC-087 PME.
- FR-002: TLE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: TLE shall execute the primary behavior owned by `TestingLearningEngine`.
- FR-004: TLE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-094 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat TLE output as the authoritative contract for testing strategy learning and coverage improvement unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/testing_learning.py`.

Primary implementation class: `TestingLearningEngine`.

Implementation classes in scope:
- `TestingLearningEngine`
...
======================================================================
TLE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-094` owns how the engine is implemented.
...
TLE must exchange data by contract, not by implicit global state.
...
4. Execute `TestingLearningEngine` behavior for testing strategy learning and coverage improvement.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-018 TPE, SPEC-040 SRE, SPEC-087 PME.
...
- `TestingLearningEngine`: Owns the core testing strategy learning and coverage improvement behavior.
...
- Learns which tests catch defects, which verification gaps recur, and which test templates should be recommended for similar future systems.
...
- Input interface: `TLEInput` JSON object.
- Output interface: `TLEOutput` JSON object.
- Source interface: `src/learning/testing_learning.py`.
- Public class boundary: `TestingLearningEngine`.
...
|---|---|---|
| `TestingLearningEngine.run(payload: dict) -> dict` | Executes the `TestingLearningEngine.run` responsibility for TLE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/testing_learning.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-094-example",
  "spec_id": "SPEC-094",
  ...
  "payload": {
    "engine": "Testing Learning Engine",
    "source_class": "TestingLearningEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_TLE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_TLE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_TLE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/testing_learning.py` implementation is primarily in-memory. The enterprise contract still requires TLE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. TLE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `TLE_STARTED` with request ID, spec ID, source path, and upstream references.
- `TLE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `TLE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- TLE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
TLE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. TLE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `TestingLearningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-018 TPE` | Upstream | Must provide validated evidence before TLE starts. |
| `SPEC-040 SRE` | Upstream | Must provide validated evidence before TLE starts. |
| `SPEC-087 PME` | Upstream | Must provide validated evidence before TLE starts. |
| `SPEC-018 TPE` | Downstream | Must consume only the validated TLE output contract. |
| `SPEC-031 PVE` | Downstream | Must consume only the validated TLE output contract. |
| `src/learning/testing_learning.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `TestingLearningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: TLE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/testing_learning.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-094-TLE.md
```

Guidance:
- Keep `TestingLearningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_tle(payload: dict) -> dict:
    input_contract = validate_tle_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = TestingLearningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_tle_output(input_contract, result)
    validate_tle_output(output)
...
- `TestingLearningEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/testing_learning.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-018 TPE"] --> I["Input Validator"]
    I --> E["SPEC-094 TLE: Testing Learning Engine"]
...
graph LR
    ...
    V --> Core["TestingLearningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["TLE Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-094"]
    This --> Source["src/learning/testing_learning.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-018 TPE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute TLE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-018 TPE" as U
rectangle "Input Validator" as V
rectangle "SPEC-094 TLE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-018 TPE" as D
U --> V
V --> E
E --> K
K --> D
...
class "TLEInputContract"
class "TLEOutputContract"
TestingLearningEngine --> "TLEInputContract"
TestingLearningEngine --> "TLEOutputContract"
@enduml
```
