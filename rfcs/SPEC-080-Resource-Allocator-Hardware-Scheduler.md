# SPEC-080: Resource Allocator Hardware Scheduler (RAHS)
...
Implementation: `src/runtime/resource_scheduler.py` (source implementation pending)
Primary Class: `ResourceAllocatorHardwareScheduler`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Resource Allocator Hardware Scheduler exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Allocates CPU, memory, disk, network, and accelerator resources according to task priority, capacity, fairness, and safety policy.

This document upgrades SPEC-080 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-080 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of RAHS is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding RAHS behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from worker telemetry, hardware inventory.
- Emit structured output that can be consumed by SPEC-072 WPM, SPEC-076 SEE.
...
- Validation of all RAHS input and output structures.
...
- Use `src/runtime/resource_scheduler.py` as the implementation boundary and `ResourceAllocatorHardwareScheduler` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
RAHS follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- RAHS validates and transforms that evidence.
...
======================================================================
- FR-001: RAHS shall load the upstream artifacts declared by worker telemetry, hardware inventory.
- FR-002: RAHS shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: RAHS shall execute the primary behavior owned by `ResourceAllocatorHardwareScheduler`.
- FR-004: RAHS shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-080 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat RAHS output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/resource_scheduler.py`.

Primary implementation class: `ResourceAllocatorHardwareScheduler`.

Implementation classes in scope:
- `ResourceAllocatorHardwareScheduler`
...
======================================================================
RAHS is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-080` owns how the engine is implemented.
...
RAHS must exchange data by contract, not by implicit global state.
...
4. Execute `ResourceAllocatorHardwareScheduler` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: worker telemetry, hardware inventory.
...
- `ResourceAllocatorHardwareScheduler`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Input interface: `RAHSInput` JSON object.
- Output interface: `RAHSOutput` JSON object.
- Source interface: `src/runtime/resource_scheduler.py`.
- Public class boundary: `ResourceAllocatorHardwareScheduler`.
...
|---|---|---|
| `ResourceAllocatorHardwareScheduler.run(payload: dict) -> dict` | Executes the `ResourceAllocatorHardwareScheduler.run` responsibility for RAHS. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/runtime/resource_scheduler.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-080-example",
  "spec_id": "SPEC-080",
  ...
  "payload": {
    "engine": "Resource Allocator Hardware Scheduler",
    "source_class": "ResourceAllocatorHardwareScheduler",
    "public_methods": []
  }
}
```
...
- `AETHERIS_RAHS_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_RAHS_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_RAHS_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/resource_scheduler.py` implementation is primarily in-memory. The enterprise contract still requires RAHS to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. RAHS may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `RAHS_STARTED` with request ID, spec ID, source path, and upstream references.
- `RAHS_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `RAHS_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- RAHS must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
RAHS is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. RAHS should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ResourceAllocatorHardwareScheduler` directly and verify public method behavior.
...
|---|---|---|
| `worker telemetry` | Upstream | Must provide validated evidence before RAHS starts. |
| `hardware inventory` | Upstream | Must provide validated evidence before RAHS starts. |
| `SPEC-072 WPM` | Downstream | Must consume only the validated RAHS output contract. |
| `SPEC-076 SEE` | Downstream | Must consume only the validated RAHS output contract. |
| `src/runtime/resource_scheduler.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ResourceAllocatorHardwareScheduler` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: RAHS grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/resource_scheduler.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-080-Resource-Allocator-Hardware-Scheduler.md
```

Guidance:
- Keep `ResourceAllocatorHardwareScheduler` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_rahs(payload: dict) -> dict:
    input_contract = validate_rahs_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ResourceAllocatorHardwareScheduler(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_rahs_output(input_contract, result)
    validate_rahs_output(output)
...
- `ResourceAllocatorHardwareScheduler.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/resource_scheduler.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["worker telemetry"] --> I["Input Validator"]
    I --> E["SPEC-080 RAHS: Resource Allocator Hardware Scheduler"]
...
graph LR
    ...
    V --> Core["ResourceAllocatorHardwareScheduler Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["RAHS Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-080"]
    This --> Source["src/runtime/resource_scheduler.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-072 WPM"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute RAHS"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "worker telemetry" as U
rectangle "Input Validator" as V
rectangle "SPEC-080 RAHS" as E
database "EKB / Artifacts" as K
rectangle "SPEC-072 WPM" as D
U --> V
V --> E
E --> K
K --> D
...
```plantuml
@startuml
class ResourceAllocatorHardwareScheduler {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "RAHSInputContract"
class "RAHSOutputContract"
ResourceAllocatorHardwareScheduler --> "RAHSInputContract"
ResourceAllocatorHardwareScheduler --> "RAHSOutputContract"
@enduml
```
