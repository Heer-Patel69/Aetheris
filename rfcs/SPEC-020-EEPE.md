# SPEC-020: Engineering Execution Planning Engine (EEPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Milestone and sprint execution planning
...
Primary Class: `EngineeringExecutionPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Engineering Execution Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Builds milestone roadmap, sprint dependency order, execution readiness criteria, and delivery planning for the engineering blueprint.

This document upgrades SPEC-020 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-020 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of EEPE is to own the full engineering responsibility for milestone and sprint execution planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EEPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct milestone and sprint execution planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for milestone and sprint execution planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-019.
- Emit structured output that can be consumed by SPEC-021.
...
======================================================================
In scope:
- Milestone and sprint execution planning
- Validation of all EEPE input and output structures.
...
- Use `src/intelligence/planners.py` as the implementation boundary and `EngineeringExecutionPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EEPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EEPE validates and transforms that evidence.
...
======================================================================
- FR-001: EEPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-019.
- FR-002: EEPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EEPE shall execute the primary behavior owned by `EngineeringExecutionPlanningEngine`.
- FR-004: EEPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-020 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EEPE output as the authoritative contract for milestone and sprint execution planning unless a later validation SPEC rejects it.
...
Primary implementation class: `EngineeringExecutionPlanningEngine`.

Implementation classes in scope:
- `EngineeringExecutionPlanningEngine`
...
======================================================================
EEPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-020` owns how the engine is implemented.
...
EEPE must exchange data by contract, not by implicit global state.
...
4. Execute `EngineeringExecutionPlanningEngine` behavior for milestone and sprint execution planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-019.
...
- `EngineeringExecutionPlanningEngine`: Owns the core milestone and sprint execution planning behavior.
...
- Input interface: `EEPEInput` JSON object.
- Output interface: `EEPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `EngineeringExecutionPlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for EEPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-020-example",
  "spec_id": "SPEC-020",
  ...
  "payload": {
    "engine": "Engineering Execution Planning Engine",
    "source_class": "EngineeringExecutionPlanningEngine",
    "public_methods": [
...
- `AETHERIS_EEPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EEPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EEPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. EEPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EEPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `EEPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EEPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- EEPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EEPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EEPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `EngineeringExecutionPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before EEPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before EEPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before EEPE starts. |
| `SPEC-019` | Upstream | Must provide validated evidence before EEPE starts. |
| `SPEC-021` | Downstream | Must consume only the validated EEPE output contract. |
...
| FR-003 core execution | Direct `EngineeringExecutionPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EEPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-020-EEPE.md
```

Guidance:
- Keep `EngineeringExecutionPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_eepe(payload: dict) -> dict:
    input_contract = validate_eepe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = EngineeringExecutionPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_eepe_output(input_contract, result)
    validate_eepe_output(output)
...
graph TD
    ...
    I --> E["SPEC-020 EEPE: Engineering Execution Planning Engine"]
...
graph LR
    ...
    V --> Core["EngineeringExecutionPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EEPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EEPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-020 EEPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-021" as D
U --> V
V --> E
E --> K
K --> D
...
class "EEPEInputContract"
class "EEPEOutputContract"
EngineeringExecutionPlanningEngine --> "EEPEInputContract"
EngineeringExecutionPlanningEngine --> "EEPEOutputContract"
@enduml
```
