# SPEC-030: Final Engineering Blueprint Compiler (FEBC)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Master blueprint compilation and technical design document generation
...
Primary Class: `FinalEngineeringBlueprintCompiler`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Final Engineering Blueprint Compiler exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Compiles validated planning outputs into the authoritative engineering blueprint, KPI report, complexity assessment, execution input, and TDD artifacts.

This document upgrades SPEC-030 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-030 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of FEBC is to own the full engineering responsibility for master blueprint compilation and technical design document generation. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding FEBC behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct master blueprint compilation and technical design document generation was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for master blueprint compilation and technical design document generation.
- Accept only validated upstream artifacts from SPEC-012 through SPEC-029, SPEC-031 PVE, SPEC-007 EKB.
- Emit structured output that can be consumed by SPEC-032 TDE, RFC-003 AEE.
...
======================================================================
In scope:
- Master blueprint compilation and technical design document generation
- Validation of all FEBC input and output structures.
...
- Use `src/intelligence/planners.py` as the implementation boundary and `FinalEngineeringBlueprintCompiler` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
FEBC follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- FEBC validates and transforms that evidence.
...
======================================================================
- FR-001: FEBC shall load the upstream artifacts declared by SPEC-012 through SPEC-029, SPEC-031 PVE, SPEC-007 EKB.
- FR-002: FEBC shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: FEBC shall execute the primary behavior owned by `FinalEngineeringBlueprintCompiler`.
- FR-004: FEBC shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-030 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.

Upstream dependencies:
- SPEC-012 through SPEC-029
- SPEC-031 PVE
- SPEC-007 EKB
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat FEBC output as the authoritative contract for master blueprint compilation and technical design document generation unless a later validation SPEC rejects it.
...
Primary implementation class: `FinalEngineeringBlueprintCompiler`.
...
======================================================================
FEBC is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-030` owns how the engine is implemented.
...
FEBC must exchange data by contract, not by implicit global state.
...
4. Execute `FinalEngineeringBlueprintCompiler` behavior for master blueprint compilation and technical design document generation.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-012 through SPEC-029, SPEC-031 PVE, SPEC-007 EKB.
...
- `FinalEngineeringBlueprintCompiler`: Owns the core master blueprint compilation and technical design document generation behavior.
...
- Input interface: `FEBCInput` JSON object.
- Output interface: `FEBCOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `FinalEngineeringBlueprintCompiler`.
...
|---|---|---|
| `save_version(blueprint: dict) -> int` | Executes the `save_version` responsibility for FEBC. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `get_latest_version() -> Optional[dict]` | Executes the `get_latest_version` responsibility for FEBC. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `diff_blueprints(old_bp: dict, new_bp: dict) -> dict` | Executes the `diff_blueprints` responsibility for FEBC. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `calculate_kpis(product_plan: dict) -> dict` | Executes the `calculate_kpis` responsibility for FEBC. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `analyze(product_plan: dict) -> dict` | Executes the `analyze` responsibility for FEBC. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `check_confidence_gate(blueprint_score: float) -> Optional[str]` | Executes the `check_confidence_gate` responsibility for FEBC. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `compile_blueprint() -> dict` | Executes the `compile_blueprint` responsibility for FEBC. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `compile_tdd_reports() -> None` | Executes the `compile_tdd_reports` responsibility for FEBC. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-030-example",
  "spec_id": "SPEC-030",
  ...
  "payload": {
    "engine": "Final Engineering Blueprint Compiler",
    "source_class": "FinalEngineeringBlueprintCompiler",
    "public_methods": [
      "save_version(blueprint: dict) -> int",
      "get_latest_version() -> Optional[dict]",
      "diff_blueprints(old_bp: dict, new_bp: dict) -> dict",
      "calculate_kpis(product_plan: dict) -> dict",
      "analyze(product_plan: dict) -> dict"
    ]
  }
}
```
...
- `AETHERIS_FEBC_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_FEBC_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_FEBC_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. FEBC may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `FEBC_STARTED` with request ID, spec ID, source path, and upstream references.
- `FEBC_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `FEBC_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- FEBC must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
FEBC is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. FEBC should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `FinalEngineeringBlueprintCompiler` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-012 through SPEC-029` | Upstream | Must provide validated evidence before FEBC starts. |
| `SPEC-031 PVE` | Upstream | Must provide validated evidence before FEBC starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before FEBC starts. |
| `SPEC-032 TDE` | Downstream | Must consume only the validated FEBC output contract. |
| `RFC-003 AEE` | Downstream | Must consume only the validated FEBC output contract. |
...
| FR-003 core execution | Direct `FinalEngineeringBlueprintCompiler` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: FEBC grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-030-FEBC.md
```

Guidance:
- Keep `FinalEngineeringBlueprintCompiler` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_febc(payload: dict) -> dict:
    input_contract = validate_febc_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = FinalEngineeringBlueprintCompiler(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_febc_output(input_contract, result)
    validate_febc_output(output)
...
```mermaid
graph TD
    U["SPEC-012 through SPEC-029"] --> I["Input Validator"]
    I --> E["SPEC-030 FEBC: Final Engineering Blueprint Compiler"]
...
graph LR
    ...
    V --> Core["FinalEngineeringBlueprintCompiler Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["FEBC Payload"]
...
```mermaid
graph TD
    Parent["RFC-002"] --> This["SPEC-030"]
    This --> Source["src/intelligence/planners.py"]
    This --> Test["tests/test_planners.py"]
    This --> Next["SPEC-032 TDE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute FEBC"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-012 through SPEC-029" as U
rectangle "Input Validator" as V
rectangle "SPEC-030 FEBC" as E
database "EKB / Artifacts" as K
rectangle "SPEC-032 TDE" as D
U --> V
V --> E
E --> K
K --> D
...
class "FEBCInputContract"
class "FEBCOutputContract"
FinalEngineeringBlueprintCompiler --> "FEBCInputContract"
FinalEngineeringBlueprintCompiler --> "FEBCOutputContract"
@enduml
```
