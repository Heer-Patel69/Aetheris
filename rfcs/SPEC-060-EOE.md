# SPEC-060: Execution Optimization Engine (EOE)
...
Implementation: `src/intelligence/eoe.py` (validated from source metadata)
Primary Class: `ExecutionOptimizationEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Execution Optimization Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Optimizes execution task ordering, batching recommendations, and runtime decisions using cost, latency, dependency, and risk signals.

This document upgrades SPEC-060 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-060 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of EOE is to own the full engineering responsibility for wave 3 - optimization. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EOE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-037 ES, SPEC-059 COE.
- Emit structured output that can be consumed by SPEC-061 COE2, SPEC-065 IO.
...
In scope:
- Wave 3 - Optimization
- Validation of all EOE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-060` input contract to `SPEC-060` output contract.
- Use `src/intelligence/eoe.py` as the implementation boundary and `ExecutionOptimizationEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EOE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EOE validates and transforms that evidence.
...
======================================================================
- FR-001: EOE shall load the upstream artifacts declared by SPEC-037 ES, SPEC-059 COE.
- FR-002: EOE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EOE shall execute the primary behavior owned by `ExecutionOptimizationEngine`.
- FR-004: EOE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-060 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EOE output as the authoritative contract for wave 3 - optimization unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/eoe.py`.

Primary implementation class: `ExecutionOptimizationEngine`.
...
======================================================================
EOE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-060` owns how the engine is implemented.
...
EOE must exchange data by contract, not by implicit global state.
...
4. Execute `ExecutionOptimizationEngine` behavior for wave 3 - optimization.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-037 ES, SPEC-059 COE.
...
- `ExecutionOptimizationEngine`: Owns the core wave 3 - optimization behavior.
...
- Optimizes execution task ordering, batching recommendations, and runtime decisions using cost, latency, dependency, and risk signals.
...
- Input interface: `EOEInput` JSON object.
- Output interface: `EOEOutput` JSON object.
- Source interface: `src/intelligence/eoe.py`.
- Public class boundary: `ExecutionOptimizationEngine`.
...
|---|---|---|
| `optimize_execution(tasks: list) -> list` | Executes the `optimize_execution` responsibility for EOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-060-example",
  "spec_id": "SPEC-060",
  ...
  "payload": {
    "engine": "Execution Optimization Engine",
    "source_class": "ExecutionOptimizationEngine",
    "public_methods": [
      "optimize_execution(tasks: list) -> list"
    ]
  }
}
```
...
- `AETHERIS_EOE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EOE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EOE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/eoe.py` implementation is primarily in-memory. The enterprise contract still requires EOE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. EOE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EOE_STARTED` with request ID, spec ID, source path, and upstream references.
- `EOE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EOE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- EOE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EOE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EOE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ExecutionOptimizationEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-037 ES` | Upstream | Must provide validated evidence before EOE starts. |
| `SPEC-059 COE` | Upstream | Must provide validated evidence before EOE starts. |
| `SPEC-061 COE2` | Downstream | Must consume only the validated EOE output contract. |
| `SPEC-065 IO` | Downstream | Must consume only the validated EOE output contract. |
| `src/intelligence/eoe.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ExecutionOptimizationEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EOE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/eoe.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-060-EOE.md
```

Guidance:
- Keep `ExecutionOptimizationEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_eoe(payload: dict) -> dict:
    input_contract = validate_eoe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ExecutionOptimizationEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_eoe_output(input_contract, result)
    validate_eoe_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/eoe.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-037 ES"] --> I["Input Validator"]
    I --> E["SPEC-060 EOE: Execution Optimization Engine"]
...
graph LR
    ...
    V --> Core["ExecutionOptimizationEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EOE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-060"]
    This --> Source["src/intelligence/eoe.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-061 COE2"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EOE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-037 ES" as U
rectangle "Input Validator" as V
rectangle "SPEC-060 EOE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-061 COE2" as D
U --> V
V --> E
E --> K
K --> D
...
class "EOEInputContract"
class "EOEOutputContract"
ExecutionOptimizationEngine --> "EOEInputContract"
ExecutionOptimizationEngine --> "EOEOutputContract"
@enduml
```
