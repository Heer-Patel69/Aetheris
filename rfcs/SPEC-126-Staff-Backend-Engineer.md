# SPEC-126: Staff Backend Engineer (SBE)
...
Implementation: `src/organization/backend.py`
Primary Class: `StaffBackendEngineerAgent`
Test Reference: `tests/test_rfc008_core.py`

======================================================================
1. MISSION
======================================================================
The Staff Backend Engineer (SBE) exists to provide a permanent specialist role or orchestration component within the Aetheris AI Organization. It represents a professional software engineering persona designed to operate collaboratively, enforce quality, and deliver specific deliverables.
...
The agent consumes inputs defined by the `SPEC-126Input` schema. Key variables include:
- `request_id`: Unique transaction identifier.
- `spec_id`: The ID of this specification (SPEC-126).
- `payload`: Subsystem parameters.

Required Context Inputs:
| Input Source | Format | Purpose |
|---|---|---|
| Technical specifications, API models, database profiles, code coverage reports. | Structured JSON | Contextual parameters for execution loops |
...
The agent produces outputs conforming to the `SPEC-126Output` schema. Outputs include:
...
|---|---|---|
| Python code files, unit tests, SQL migration scripts, database indexes. | Markdown/JSON | Workspace directories & EKB objects |
...
======================================================================
Interaction Scopes:
- Syncs with Frontend Engineer; designs database tables with Architect; reports to EM.
...
8. SUGGESTED MODULES
======================================================================
Suggested path structure: `src/organization/backend.py`.
Modules in scope: `backend_agent.py, code_writer.py, query_optimizer.py, migration_tool.py`
...
class StaffBackendEngineerAgent:
    """
    Enterprise agent implementation representing the Staff Backend Engineer role.
    Handles inputs, validates context, and executes role responsibilities.
    """
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.info("Initializing StaffBackendEngineerAgent agent.")

    def execute_task(self, request_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        self.logger.info(f"Processing task {request_id} for SBE agent.")
        # 1. Input Validation
        if not payload:
        ...
        return {
            ...
            "result": result,
            "metadata": {
                "agent_role": "Staff Backend Engineer",
                "version": "2.0.0"
            }
        }
    ...
    def _process_domain_logic(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"message": "Processed successfully by StaffBackendEngineerAgent"}
```
...
======================================================================
Prometheus Metrics:
- `aetheris_agent_runs_total{role="SBE"}`: Total task runs.
- `aetheris_agent_failures_total{role="SBE"}`: Total execution errors.
- `aetheris_agent_latency_ms{role="SBE"}`: Task durations.
...
Integration Tests:
- Test message passing and handoff events between SBE and adjacent agents.

Test Command:
`pytest tests/test_rfc008_core.py -k "StaffBackendEngineerAgent"`
