# SPEC-019: Documentation Planning Engine (DOPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Runbooks, README, ADR, and API documentation planning
...
Primary Class: `DocumentationPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Documentation Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Outlines documentation artifacts, operating runbooks, ADR requirements, API references, and maintenance ownership.

This document upgrades SPEC-019 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-019 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of DOPE is to own the full engineering responsibility for runbooks, readme, adr, and api documentation planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding DOPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct runbooks, readme, adr, and api documentation planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for runbooks, readme, adr, and api documentation planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-018.
- Emit structured output that can be consumed by SPEC-020.
...
======================================================================
In scope:
- Runbooks, README, ADR, and API documentation planning
- Validation of all DOPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-019` input contract to `SPEC-019` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `DocumentationPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
DOPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- DOPE validates and transforms that evidence.
...
======================================================================
- FR-001: DOPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-018.
- FR-002: DOPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: DOPE shall execute the primary behavior owned by `DocumentationPlanningEngine`.
- FR-004: DOPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-019 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat DOPE output as the authoritative contract for runbooks, readme, adr, and api documentation planning unless a later validation SPEC rejects it.
...
Primary implementation class: `DocumentationPlanningEngine`.

Implementation classes in scope:
- `DocumentationPlanningEngine`
...
======================================================================
DOPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-019` owns how the engine is implemented.
...
DOPE must exchange data by contract, not by implicit global state.
...
4. Execute `DocumentationPlanningEngine` behavior for runbooks, readme, adr, and api documentation planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-018.
...
- `DocumentationPlanningEngine`: Owns the core runbooks, readme, adr, and api documentation planning behavior.
...
- Input interface: `DOPEInput` JSON object.
- Output interface: `DOPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `DocumentationPlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for DOPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-019-example",
  "spec_id": "SPEC-019",
  ...
  "payload": {
    "engine": "Documentation Planning Engine",
    "source_class": "DocumentationPlanningEngine",
    "public_methods": [
...
- `AETHERIS_DOPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_DOPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_DOPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. DOPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `DOPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `DOPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `DOPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- DOPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
DOPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. DOPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `DocumentationPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before DOPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before DOPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before DOPE starts. |
| `SPEC-018` | Upstream | Must provide validated evidence before DOPE starts. |
| `SPEC-020` | Downstream | Must consume only the validated DOPE output contract. |
...
| FR-003 core execution | Direct `DocumentationPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: DOPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-019-DOPE.md
```

Guidance:
- Keep `DocumentationPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_dope(payload: dict) -> dict:
    input_contract = validate_dope_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = DocumentationPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_dope_output(input_contract, result)
    validate_dope_output(output)
...
graph TD
    ...
    I --> E["SPEC-019 DOPE: Documentation Planning Engine"]
...
graph LR
    ...
    V --> Core["DocumentationPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["DOPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute DOPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-019 DOPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-020" as D
U --> V
V --> E
E --> K
K --> D
...
class "DOPEInputContract"
class "DOPEOutputContract"
DocumentationPlanningEngine --> "DOPEInputContract"
DocumentationPlanningEngine --> "DOPEOutputContract"
@enduml
```
