# SPEC-038: Parallel Execution Engine (PEE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Parallel task batching and conflict control
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/pee.py` (validated from source metadata)
Primary Class: `ParallelExecutionEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Parallel Execution Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Groups independent tasks into parallel batches, maps file locks, detects conflicts, allocates resources, and supports batch control.

This document upgrades SPEC-038 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-038 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of PEE is to own the full engineering responsibility for parallel task batching and conflict control. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PEE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct parallel task batching and conflict control was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for parallel task batching and conflict control.
- Accept only validated upstream artifacts from SPEC-037.
- Emit structured output that can be consumed by SPEC-039.
...
======================================================================
In scope:
- Parallel task batching and conflict control
- Validation of all PEE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-038` input contract to `SPEC-038` output contract.
- Use `src/execution/pee.py` as the implementation boundary and `ParallelExecutionEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PEE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PEE validates and transforms that evidence.
...
======================================================================
- FR-001: PEE shall load the upstream artifacts declared by SPEC-037.
- FR-002: PEE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PEE shall execute the primary behavior owned by `ParallelExecutionEngine`.
- FR-004: PEE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-038 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PEE output as the authoritative contract for parallel task batching and conflict control unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/pee.py`.

Primary implementation class: `ParallelExecutionEngine`.
...
======================================================================
PEE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-038` owns how the engine is implemented.
...
PEE must exchange data by contract, not by implicit global state.
...
4. Execute `ParallelExecutionEngine` behavior for parallel task batching and conflict control.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-037.
...
- `ParallelExecutionEngine`: Owns the core parallel task batching and conflict control behavior.
...
- Input interface: `PEEInput` JSON object.
- Output interface: `PEEOutput` JSON object.
- Source interface: `src/execution/pee.py`.
- Public class boundary: `ParallelExecutionEngine`.
...
|---|---|---|
| `create_batches(queue: List[dict]) -> dict` | Executes the `create_batches` responsibility for PEE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `allocate_resources(batch: dict) -> dict` | Executes the `allocate_resources` responsibility for PEE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `detect_conflicts(tasks: List[dict]) -> dict` | Executes the `detect_conflicts` responsibility for PEE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `merge_batches(batches_list: List[dict]) -> dict` | Executes the `merge_batches` responsibility for PEE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `pause_batch(batch_id: str) -> bool` | Executes the `pause_batch` responsibility for PEE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `resume_batch(batch_id: str) -> bool` | Executes the `resume_batch` responsibility for PEE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `cancel_batch(batch_id: str) -> bool` | Executes the `cancel_batch` responsibility for PEE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-038-example",
  "spec_id": "SPEC-038",
  ...
  "payload": {
    ...
    "source_class": "ParallelExecutionEngine",
    "public_methods": [
      "create_batches(queue: List[dict]) -> dict",
      "allocate_resources(batch: dict) -> dict",
      "detect_conflicts(tasks: List[dict]) -> dict",
      "merge_batches(batches_list: List[dict]) -> dict",
      "pause_batch(batch_id: str) -> bool"
    ]
  }
}
```
...
- `AETHERIS_PEE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PEE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PEE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. PEE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PEE_STARTED` with request ID, spec ID, source path, and upstream references.
- `PEE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PEE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PEE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PEE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PEE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ParallelExecutionEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-037` | Upstream | Must provide validated evidence before PEE starts. |
| `SPEC-039` | Downstream | Must consume only the validated PEE output contract. |
| `src/execution/pee.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ParallelExecutionEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PEE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-038-PEE.md
```

Guidance:
- Keep `ParallelExecutionEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_pee(payload: dict) -> dict:
    input_contract = validate_pee_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ParallelExecutionEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_pee_output(input_contract, result)
    validate_pee_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/pee.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-037"] --> I["Input Validator"]
    I --> E["SPEC-038 PEE: Parallel Execution Engine"]
...
graph LR
    ...
    V --> Core["ParallelExecutionEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PEE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-038"]
    This --> Source["src/execution/pee.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PEE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-037" as U
rectangle "Input Validator" as V
rectangle "SPEC-038 PEE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-039" as D
U --> V
V --> E
E --> K
K --> D
...
class "PEEInputContract"
class "PEEOutputContract"
ParallelExecutionEngine --> "PEEInputContract"
ParallelExecutionEngine --> "PEEOutputContract"
@enduml
```
