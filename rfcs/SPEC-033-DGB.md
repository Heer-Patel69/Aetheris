# SPEC-033: Dependency Graph Builder (DGB)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Execution dependency graph and topological scheduling
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/dgb.py` (validated from source metadata)
Primary Class: `DependencyGraphBuilder`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Dependency Graph Builder exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Transforms decomposed tasks into a cycle-free directed graph, topological execution order, critical path, and parallel groups.

This document upgrades SPEC-033 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-033 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of DGB is to own the full engineering responsibility for execution dependency graph and topological scheduling. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding DGB behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct execution dependency graph and topological scheduling was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for execution dependency graph and topological scheduling.
- Accept only validated upstream artifacts from SPEC-032.
- Emit structured output that can be consumed by SPEC-034.
...
======================================================================
In scope:
- Execution dependency graph and topological scheduling
- Validation of all DGB input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-033` input contract to `SPEC-033` output contract.
- Use `src/execution/dgb.py` as the implementation boundary and `DependencyGraphBuilder` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
DGB follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- DGB validates and transforms that evidence.
...
======================================================================
- FR-001: DGB shall load the upstream artifacts declared by SPEC-032.
- FR-002: DGB shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: DGB shall execute the primary behavior owned by `DependencyGraphBuilder`.
- FR-004: DGB shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-033 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat DGB output as the authoritative contract for execution dependency graph and topological scheduling unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/dgb.py`.

Primary implementation class: `DependencyGraphBuilder`.
...
======================================================================
DGB is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-033` owns how the engine is implemented.
...
DGB must exchange data by contract, not by implicit global state.
...
4. Execute `DependencyGraphBuilder` behavior for execution dependency graph and topological scheduling.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-032.
...
- `DependencyGraphBuilder`: Owns the core execution dependency graph and topological scheduling behavior.
...
- Input interface: `DGBInput` JSON object.
- Output interface: `DGBOutput` JSON object.
- Source interface: `src/execution/dgb.py`.
- Public class boundary: `DependencyGraphBuilder`.
...
|---|---|---|
| `build_graph(tasks: List[dict]) -> dict` | Executes the `build_graph` responsibility for DGB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `detect_cycles() -> List[List[str]]` | Executes the `detect_cycles` responsibility for DGB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `get_topological_order() -> List[str]` | Executes the `get_topological_order` responsibility for DGB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `calculate_critical_path() -> List[str]` | Executes the `calculate_critical_path` responsibility for DGB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `get_parallel_groups() -> List[List[str]]` | Executes the `get_parallel_groups` responsibility for DGB. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-033-example",
  "spec_id": "SPEC-033",
  ...
  "payload": {
    "engine": "Dependency Graph Builder",
    "source_class": "DependencyGraphBuilder",
    "public_methods": [
      "build_graph(tasks: List[dict]) -> dict",
      "detect_cycles() -> List[List[str]]",
      "get_topological_order() -> List[str]",
      "calculate_critical_path() -> List[str]",
...
- `AETHERIS_DGB_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_DGB_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_DGB_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. DGB may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `DGB_STARTED` with request ID, spec ID, source path, and upstream references.
- `DGB_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `DGB_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- DGB must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
DGB is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. DGB should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `DependencyGraphBuilder` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-032` | Upstream | Must provide validated evidence before DGB starts. |
| `SPEC-034` | Downstream | Must consume only the validated DGB output contract. |
| `src/execution/dgb.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `DependencyGraphBuilder` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: DGB grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-033-DGB.md
```

Guidance:
- Keep `DependencyGraphBuilder` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_dgb(payload: dict) -> dict:
    input_contract = validate_dgb_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = DependencyGraphBuilder(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_dgb_output(input_contract, result)
    validate_dgb_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/dgb.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-032"] --> I["Input Validator"]
    I --> E["SPEC-033 DGB: Dependency Graph Builder"]
...
graph LR
    ...
    V --> Core["DependencyGraphBuilder Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["DGB Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-033"]
    This --> Source["src/execution/dgb.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute DGB"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-032" as U
rectangle "Input Validator" as V
rectangle "SPEC-033 DGB" as E
database "EKB / Artifacts" as K
rectangle "SPEC-034" as D
U --> V
V --> E
E --> K
K --> D
...
class "DGBInputContract"
class "DGBOutputContract"
DependencyGraphBuilder --> "DGBInputContract"
DependencyGraphBuilder --> "DGBOutputContract"
@enduml
```
