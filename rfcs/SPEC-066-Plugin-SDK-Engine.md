# SPEC-066: Plugin SDK Engine (PSE)
...
Implementation: `src/sdk/engine.py` (source implementation pending)
Primary Class: `PluginSDKEngine`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Plugin SDK Engine exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Provides the SDK contracts, base interfaces, lifecycle hooks, and validation envelopes required for safe third-party Aetheris extensions.

This document upgrades SPEC-066 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-066 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of PSE is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PSE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-065 IO, plugin manifests, developer SDK contracts.
- Emit structured output that can be consumed by SPEC-067 EME2, SPEC-068 RPL.
...
- Validation of all PSE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-066` input contract to `SPEC-066` output contract.
- Use `src/sdk/engine.py` as the implementation boundary and `PluginSDKEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PSE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PSE validates and transforms that evidence.
...
======================================================================
- FR-001: PSE shall load the upstream artifacts declared by SPEC-065 IO, plugin manifests, developer SDK contracts.
- FR-002: PSE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PSE shall execute the primary behavior owned by `PluginSDKEngine`.
- FR-004: PSE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-066 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PSE output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/sdk/engine.py`.

Primary implementation class: `PluginSDKEngine`.
...
======================================================================
PSE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-066` owns how the engine is implemented.
...
PSE must exchange data by contract, not by implicit global state.
...
4. Execute `PluginSDKEngine` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-065 IO, plugin manifests, developer SDK contracts.
...
- `PluginSDKEngine`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Provides the SDK contracts, base interfaces, lifecycle hooks, and validation envelopes required for safe third-party Aetheris extensions.
...
- Input interface: `PSEInput` JSON object.
- Output interface: `PSEOutput` JSON object.
- Source interface: `src/sdk/engine.py`.
- Public class boundary: `PluginSDKEngine`.
...
|---|---|---|
| `PluginSDKEngine.run(payload: dict) -> dict` | Executes the `PluginSDKEngine.run` responsibility for PSE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-066-example",
  "spec_id": "SPEC-066",
  ...
  "payload": {
    "engine": "Plugin SDK Engine",
    "source_class": "PluginSDKEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_PSE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PSE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PSE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/sdk/engine.py` implementation is primarily in-memory. The enterprise contract still requires PSE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. PSE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PSE_STARTED` with request ID, spec ID, source path, and upstream references.
- `PSE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PSE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PSE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PSE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PSE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `PluginSDKEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-065 IO` | Upstream | Must provide validated evidence before PSE starts. |
| `plugin manifests` | Upstream | Must provide validated evidence before PSE starts. |
| `developer SDK contracts` | Upstream | Must provide validated evidence before PSE starts. |
| `SPEC-067 EME2` | Downstream | Must consume only the validated PSE output contract. |
| `SPEC-068 RPL` | Downstream | Must consume only the validated PSE output contract. |
| `src/sdk/engine.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `PluginSDKEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PSE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-066-Plugin-SDK-Engine.md
```

Guidance:
- Keep `PluginSDKEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_pse(payload: dict) -> dict:
    input_contract = validate_pse_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = PluginSDKEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_pse_output(input_contract, result)
    validate_pse_output(output)
...
- `PluginSDKEngine.execute(payload: dict) -> dict`
...
```mermaid
graph TD
    U["SPEC-065 IO"] --> I["Input Validator"]
    I --> E["SPEC-066 PSE: Plugin SDK Engine"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PSE Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-066"]
    This --> Source["src/sdk/engine.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-067 EME2"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PSE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-065 IO" as U
rectangle "Input Validator" as V
rectangle "SPEC-066 PSE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-067 EME2" as D
U --> V
V --> E
E --> K
K --> D
...
class "PSEInputContract"
class "PSEOutputContract"
PluginSDKEngine --> "PSEInputContract"
PluginSDKEngine --> "PSEOutputContract"
@enduml
```
