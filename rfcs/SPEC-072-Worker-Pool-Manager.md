# SPEC-072: Worker Pool Manager (WPM)
...
Implementation: `src/runtime/worker_pool.py` (source implementation pending)
Primary Class: `WorkerPoolManager`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Worker Pool Manager exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Manages local worker capacity, queue assignment, lifecycle state, backpressure, and safe shutdown for concurrent execution workloads.

This document upgrades SPEC-072 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-072 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of WPM is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding WPM behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-070 IPCF, SPEC-080 RAHS.
...
- Validation of all WPM input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-072` input contract to `SPEC-072` output contract.
- Use `src/runtime/worker_pool.py` as the implementation boundary and `WorkerPoolManager` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
WPM follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- WPM validates and transforms that evidence.
...
======================================================================
- FR-001: WPM shall load the upstream artifacts declared by SPEC-070 IPCF, SPEC-080 RAHS.
- FR-002: WPM shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: WPM shall execute the primary behavior owned by `WorkerPoolManager`.
- FR-004: WPM shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-072 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat WPM output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/worker_pool.py`.

Primary implementation class: `WorkerPoolManager`.
...
======================================================================
WPM is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-072` owns how the engine is implemented.
...
WPM must exchange data by contract, not by implicit global state.
...
4. Execute `WorkerPoolManager` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-070 IPCF, SPEC-080 RAHS.
...
4. Run the WPM core transformation.
...
- `WorkerPoolManager`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Manages local worker capacity, queue assignment, lifecycle state, backpressure, and safe shutdown for concurrent execution workloads.
...
- Input interface: `WPMInput` JSON object.
- Output interface: `WPMOutput` JSON object.
- Source interface: `src/runtime/worker_pool.py`.
- Public class boundary: `WorkerPoolManager`.
...
|---|---|---|
| `WorkerPoolManager.run(payload: dict) -> dict` | Executes the `WorkerPoolManager.run` responsibility for WPM. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/runtime/worker_pool.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-072-example",
  "spec_id": "SPEC-072",
  ...
  "payload": {
    "engine": "Worker Pool Manager",
    "source_class": "WorkerPoolManager",
    "public_methods": []
  }
}
```
...
- `AETHERIS_WPM_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_WPM_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_WPM_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/worker_pool.py` implementation is primarily in-memory. The enterprise contract still requires WPM to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. WPM may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `WPM_STARTED` with request ID, spec ID, source path, and upstream references.
- `WPM_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `WPM_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- WPM must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
WPM is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. WPM should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `WorkerPoolManager` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-070 IPCF` | Upstream | Must provide validated evidence before WPM starts. |
| `SPEC-080 RAHS` | Upstream | Must provide validated evidence before WPM starts. |
| `SPEC-071 DEE` | Downstream | Must consume only the validated WPM output contract. |
| `SPEC-085 GRO` | Downstream | Must consume only the validated WPM output contract. |
| `src/runtime/worker_pool.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `WorkerPoolManager` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: WPM grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/worker_pool.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-072-Worker-Pool-Manager.md
```

Guidance:
- Keep `WorkerPoolManager` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_wpm(payload: dict) -> dict:
    input_contract = validate_wpm_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = WorkerPoolManager(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_wpm_output(input_contract, result)
    validate_wpm_output(output)
...
- `WorkerPoolManager.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/worker_pool.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-072 WPM: Worker Pool Manager"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["WPM Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-072"]
    This --> Source["src/runtime/worker_pool.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute WPM"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
class "WPMInputContract"
class "WPMOutputContract"
WorkerPoolManager --> "WPMInputContract"
WorkerPoolManager --> "WPMOutputContract"
@enduml
```
