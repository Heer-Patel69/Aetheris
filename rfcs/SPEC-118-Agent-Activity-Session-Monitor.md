# SPEC-118: Agent Activity & Session Monitor (AASM)
...
Primary Class: `AgentSessionMonitor`
Test Reference: `tests/test_rfc007_core.py`

======================================================================
1. PURPOSE & BUSINESS VALUE
======================================================================
The Agent Activity & Session Monitor (AASM) exists to provide a foundational enterprise capability for Aetheris. It ensures secure, production-scale, multi-tenant operations, maintaining compliance, security boundaries, and strict resource isolation.
...
======================================================================
- FR-101: The engine shall load configurations conforming to the `SPEC-118Input` schema.
- FR-102: The engine shall authorize all operations against tenant context parameters.
- FR-103: The engine shall execute the core `AgentSessionMonitor` behaviors.
- FR-104: The engine shall output structured results conforming to the `SPEC-118Output` schema.
...
======================================================================
Primary Path: `src/enterprise/monitor.py`.
The class `AgentSessionMonitor` acts as the main domain service layer, validating API structures, interacting with local storage adapters, and dispatching transaction logs to the audit service.
...
- `AgentSessionMonitorCore`: Executes subsystem transformations.
...
3. Call `AgentSessionMonitor` core behaviors.
...
======================================================================
Primary logic is defined in `src/enterprise/monitor.py`. Sub-modules handle encryption, parsing, and database schemas.
...
|---|---|---|
| `get_session_state(session_id: str) -> dict` | Returns real-time execution graphs and metrics for an active session. | Validate input, enforce security boundaries, return deterministic output, and emit telemetry. |
...
18. OBSERVABILITY
======================================================================
Structured logging includes standard event codes like `AASM_STARTED`, `AASM_SUCCESS`, and `AASM_FAILED` with request ID attributes.
