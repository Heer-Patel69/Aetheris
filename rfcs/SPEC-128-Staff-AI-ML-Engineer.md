# SPEC-128: Staff AI/ML Engineer (MLE)
...
Implementation: `src/organization/aiml.py`
Primary Class: `StaffAIMLEngineerAgent`
Test Reference: `tests/test_rfc008_core.py`

======================================================================
1. MISSION
======================================================================
The Staff AI/ML Engineer (MLE) exists to provide a permanent specialist role or orchestration component within the Aetheris AI Organization. It represents a professional software engineering persona designed to operate collaboratively, enforce quality, and deliver specific deliverables.
...
The agent consumes inputs defined by the `SPEC-128Input` schema. Key variables include:
- `request_id`: Unique transaction identifier.
- `spec_id`: The ID of this specification (SPEC-128).
- `payload`: Subsystem parameters.

Required Context Inputs:
| Input Source | Format | Purpose |
|---|---|---|
| Evaluation datasets, model logs, API rate quotas, cost spreadsheets. | Structured JSON | Contextual parameters for execution loops |
...
The agent produces outputs conforming to the `SPEC-128Output` schema. Outputs include:
...
|---|---|---|
| Prompt templates, system configurations, evaluation matrices, budget limits. | Markdown/JSON | Workspace directories & EKB objects |
...
======================================================================
Interaction Scopes:
- Advises CTO; integrates API calls with Backend Engineer; reports to EM.
...
8. SUGGESTED MODULES
======================================================================
Suggested path structure: `src/organization/aiml.py`.
Modules in scope: `ml_agent.py, prompts_store.py, model_config.py, token_counter.py`
...
class StaffAIMLEngineerAgent:
    """
    Enterprise agent implementation representing the Staff AI/ML Engineer role.
    Handles inputs, validates context, and executes role responsibilities.
    """
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.logger = logging.getLogger(self.__class__.__name__)
        self.logger.info("Initializing StaffAIMLEngineerAgent agent.")

    def execute_task(self, request_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        self.logger.info(f"Processing task {request_id} for MLE agent.")
        # 1. Input Validation
        if not payload:
        ...
        return {
            ...
            "result": result,
            "metadata": {
                "agent_role": "Staff AI/ML Engineer",
                "version": "2.0.0"
            }
        }
    ...
    def _process_domain_logic(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"message": "Processed successfully by StaffAIMLEngineerAgent"}
```
...
======================================================================
Prometheus Metrics:
- `aetheris_agent_runs_total{role="MLE"}`: Total task runs.
- `aetheris_agent_failures_total{role="MLE"}`: Total execution errors.
- `aetheris_agent_latency_ms{role="MLE"}`: Task durations.
...
Integration Tests:
- Test message passing and handoff events between MLE and adjacent agents.

Test Command:
`pytest tests/test_rfc008_core.py -k "StaffAIMLEngineerAgent"`
