# SPEC-091: Prompt Refinement Engine (PRE2)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Prompt improvement from execution feedback
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/prompt_refinement.py` (source implementation pending)
Primary Class: `PromptRefinementEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Prompt Refinement Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Learns from prompt outcomes, model responses, hallucination findings, review results, and user feedback to refine future prompt templates and prompt assembly rules.

This document upgrades SPEC-091 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-091 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of PRE2 is to own the full engineering responsibility for prompt improvement from execution feedback. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding PRE2 behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct prompt improvement from execution feedback was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for prompt improvement from execution feedback.
- Accept only validated upstream artifacts from SPEC-048 PCE, SPEC-049 POE, SPEC-056 HDE, SPEC-088 BPE2.
- Emit structured output that can be consumed by SPEC-048 PCE, SPEC-057 PLE.
...
======================================================================
In scope:
- Prompt improvement from execution feedback
- Validation of all PRE2 input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-091` input contract to `SPEC-091` output contract.
- Use `src/learning/prompt_refinement.py` as the implementation boundary and `PromptRefinementEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
PRE2 follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- PRE2 validates and transforms that evidence.
...
======================================================================
- FR-001: PRE2 shall load the upstream artifacts declared by SPEC-048 PCE, SPEC-049 POE, SPEC-056 HDE, SPEC-088 BPE2.
- FR-002: PRE2 shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: PRE2 shall execute the primary behavior owned by `PromptRefinementEngine`.
- FR-004: PRE2 shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-091 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat PRE2 output as the authoritative contract for prompt improvement from execution feedback unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/prompt_refinement.py`.

Primary implementation class: `PromptRefinementEngine`.
...
======================================================================
PRE2 is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-091` owns how the engine is implemented.
...
PRE2 must exchange data by contract, not by implicit global state.
...
4. Execute `PromptRefinementEngine` behavior for prompt improvement from execution feedback.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-048 PCE, SPEC-049 POE, SPEC-056 HDE, SPEC-088 BPE2.
...
- `PromptRefinementEngine`: Owns the core prompt improvement from execution feedback behavior.
...
- Learns from prompt outcomes, model responses, hallucination findings, review results, and user feedback to refine future prompt templates and prompt assembly rules.
...
- Input interface: `PRE2Input` JSON object.
- Output interface: `PRE2Output` JSON object.
- Source interface: `src/learning/prompt_refinement.py`.
- Public class boundary: `PromptRefinementEngine`.
...
|---|---|---|
| `PromptRefinementEngine.run(payload: dict) -> dict` | Executes the `PromptRefinementEngine.run` responsibility for PRE2. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/prompt_refinement.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-091-example",
  "spec_id": "SPEC-091",
  ...
  "payload": {
    "engine": "Prompt Refinement Engine",
    "source_class": "PromptRefinementEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_PRE2_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_PRE2_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_PRE2_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/prompt_refinement.py` implementation is primarily in-memory. The enterprise contract still requires PRE2 to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. PRE2 may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `PRE2_STARTED` with request ID, spec ID, source path, and upstream references.
- `PRE2_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `PRE2_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- PRE2 must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
PRE2 is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. PRE2 should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `PromptRefinementEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-048 PCE` | Upstream | Must provide validated evidence before PRE2 starts. |
| `SPEC-049 POE` | Upstream | Must provide validated evidence before PRE2 starts. |
| `SPEC-056 HDE` | Upstream | Must provide validated evidence before PRE2 starts. |
| `SPEC-088 BPE2` | Upstream | Must provide validated evidence before PRE2 starts. |
| `SPEC-048 PCE` | Downstream | Must consume only the validated PRE2 output contract. |
| `SPEC-057 PLE` | Downstream | Must consume only the validated PRE2 output contract. |
| `src/learning/prompt_refinement.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `PromptRefinementEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: PRE2 grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/prompt_refinement.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-091-PRE2.md
```

Guidance:
- Keep `PromptRefinementEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_pre2(payload: dict) -> dict:
    input_contract = validate_pre2_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = PromptRefinementEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_pre2_output(input_contract, result)
    validate_pre2_output(output)
...
- `PromptRefinementEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/prompt_refinement.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-048 PCE"] --> I["Input Validator"]
    I --> E["SPEC-091 PRE2: Prompt Refinement Engine"]
...
graph LR
    ...
    V --> Core["PromptRefinementEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["PRE2 Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-091"]
    This --> Source["src/learning/prompt_refinement.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-048 PCE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute PRE2"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-048 PCE" as U
rectangle "Input Validator" as V
rectangle "SPEC-091 PRE2" as E
database "EKB / Artifacts" as K
rectangle "SPEC-048 PCE" as D
U --> V
V --> E
E --> K
K --> D
...
class "PRE2InputContract"
class "PRE2OutputContract"
PromptRefinementEngine --> "PRE2InputContract"
PromptRefinementEngine --> "PRE2OutputContract"
@enduml
```
