# SPEC-086: Experience Memory Engine (EME2)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Experience capture, normalization, and replayable learning memory
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/experience_memory.py` (source implementation pending)
Primary Class: `ExperienceMemoryEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Experience Memory Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Captures execution experience from logs, metrics, prompts, outputs, failures, user feedback, code changes, and test results so future Aetheris runs can reuse validated historical knowledge.

This document upgrades SPEC-086 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-086 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of EME2 is to own the full engineering responsibility for experience capture, normalization, and replayable learning memory. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding EME2 behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct experience capture, normalization, and replayable learning memory was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for experience capture, normalization, and replayable learning memory.
- Accept only validated upstream artifacts from SPEC-045 EME, SPEC-046 EO, execution logs, user feedback.
- Emit structured output that can be consumed by SPEC-087 PME, SPEC-089 FKE, SPEC-090 SKE.
...
======================================================================
In scope:
- Experience capture, normalization, and replayable learning memory
- Validation of all EME2 input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-086` input contract to `SPEC-086` output contract.
- Use `src/learning/experience_memory.py` as the implementation boundary and `ExperienceMemoryEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
EME2 follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- EME2 validates and transforms that evidence.
...
======================================================================
- FR-001: EME2 shall load the upstream artifacts declared by SPEC-045 EME, SPEC-046 EO, execution logs, user feedback.
- FR-002: EME2 shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: EME2 shall execute the primary behavior owned by `ExperienceMemoryEngine`.
- FR-004: EME2 shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-086 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat EME2 output as the authoritative contract for experience capture, normalization, and replayable learning memory unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/experience_memory.py`.

Primary implementation class: `ExperienceMemoryEngine`.
...
======================================================================
EME2 is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-086` owns how the engine is implemented.
...
EME2 must exchange data by contract, not by implicit global state.
...
4. Execute `ExperienceMemoryEngine` behavior for experience capture, normalization, and replayable learning memory.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-045 EME, SPEC-046 EO, execution logs, user feedback.
...
4. Run the EME2 core transformation.
5. Generate artifacts and EKB records.
6. Validate output against the JSON schema in this SPEC.
7. Release the output to SPEC-087 PME, SPEC-089 FKE, SPEC-090 SKE.
...
- `ExperienceMemoryEngine`: Owns the core experience capture, normalization, and replayable learning memory behavior.
...
- Captures execution experience from logs, metrics, prompts, outputs, failures, user feedback, code changes, and test results so future Aetheris runs can reuse validated historical knowledge.
...
- Input interface: `EME2Input` JSON object.
- Output interface: `EME2Output` JSON object.
- Source interface: `src/learning/experience_memory.py`.
- Public class boundary: `ExperienceMemoryEngine`.
...
|---|---|---|
| `ExperienceMemoryEngine.run(payload: dict) -> dict` | Executes the `ExperienceMemoryEngine.run` responsibility for EME2. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/experience_memory.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-086-example",
  "spec_id": "SPEC-086",
  ...
  "payload": {
    "engine": "Experience Memory Engine",
    "source_class": "ExperienceMemoryEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_EME2_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_EME2_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_EME2_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/experience_memory.py` implementation is primarily in-memory. The enterprise contract still requires EME2 to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. EME2 may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `EME2_STARTED` with request ID, spec ID, source path, and upstream references.
- `EME2_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `EME2_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- EME2 must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
EME2 is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. EME2 should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ExperienceMemoryEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-045 EME` | Upstream | Must provide validated evidence before EME2 starts. |
| `SPEC-046 EO` | Upstream | Must provide validated evidence before EME2 starts. |
| `execution logs` | Upstream | Must provide validated evidence before EME2 starts. |
| `user feedback` | Upstream | Must provide validated evidence before EME2 starts. |
| `SPEC-087 PME` | Downstream | Must consume only the validated EME2 output contract. |
| `SPEC-089 FKE` | Downstream | Must consume only the validated EME2 output contract. |
| `SPEC-090 SKE` | Downstream | Must consume only the validated EME2 output contract. |
| `src/learning/experience_memory.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `ExperienceMemoryEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: EME2 grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/experience_memory.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-086-EME2.md
```

Guidance:
- Keep `ExperienceMemoryEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_eme2(payload: dict) -> dict:
    input_contract = validate_eme2_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ExperienceMemoryEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_eme2_output(input_contract, result)
    validate_eme2_output(output)
...
- `ExperienceMemoryEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/experience_memory.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-045 EME"] --> I["Input Validator"]
    I --> E["SPEC-086 EME2: Experience Memory Engine"]
...
graph LR
    ...
    V --> Core["ExperienceMemoryEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["EME2 Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-086"]
    This --> Source["src/learning/experience_memory.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-087 PME"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute EME2"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-045 EME" as U
rectangle "Input Validator" as V
rectangle "SPEC-086 EME2" as E
database "EKB / Artifacts" as K
rectangle "SPEC-087 PME" as D
U --> V
V --> E
E --> K
K --> D
...
class "EME2InputContract"
class "EME2OutputContract"
ExperienceMemoryEngine --> "EME2InputContract"
ExperienceMemoryEngine --> "EME2OutputContract"
@enduml
```
