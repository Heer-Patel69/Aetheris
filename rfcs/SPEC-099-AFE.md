# SPEC-099: Adaptive Feedback Engine (AFE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Feedback ingestion, scoring, and closed-loop adaptation
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/adaptive_feedback.py` (source implementation pending)
Primary Class: `AdaptiveFeedbackEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Adaptive Feedback Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Ingests explicit user feedback, implicit execution signals, review comments, production outcomes, and metric deltas to adapt learning recommendations safely.

This document upgrades SPEC-099 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-099 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of AFE is to own the full engineering responsibility for feedback ingestion, scoring, and closed-loop adaptation. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding AFE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct feedback ingestion, scoring, and closed-loop adaptation was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for feedback ingestion, scoring, and closed-loop adaptation.
- Accept only validated upstream artifacts from SPEC-098 LAE, user feedback, production outcomes.
- Emit structured output that can be consumed by SPEC-097 CLE, SPEC-100 LSO.
...
======================================================================
In scope:
- Feedback ingestion, scoring, and closed-loop adaptation
- Validation of all AFE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-099` input contract to `SPEC-099` output contract.
- Use `src/learning/adaptive_feedback.py` as the implementation boundary and `AdaptiveFeedbackEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
AFE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- AFE validates and transforms that evidence.
...
======================================================================
- FR-001: AFE shall load the upstream artifacts declared by SPEC-098 LAE, user feedback, production outcomes.
- FR-002: AFE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: AFE shall execute the primary behavior owned by `AdaptiveFeedbackEngine`.
- FR-004: AFE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-099 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat AFE output as the authoritative contract for feedback ingestion, scoring, and closed-loop adaptation unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/adaptive_feedback.py`.

Primary implementation class: `AdaptiveFeedbackEngine`.
...
======================================================================
AFE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-099` owns how the engine is implemented.
...
AFE must exchange data by contract, not by implicit global state.
...
4. Execute `AdaptiveFeedbackEngine` behavior for feedback ingestion, scoring, and closed-loop adaptation.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-098 LAE, user feedback, production outcomes.
...
- `AdaptiveFeedbackEngine`: Owns the core feedback ingestion, scoring, and closed-loop adaptation behavior.
...
- Ingests explicit user feedback, implicit execution signals, review comments, production outcomes, and metric deltas to adapt learning recommendations safely.
...
- Input interface: `AFEInput` JSON object.
- Output interface: `AFEOutput` JSON object.
- Source interface: `src/learning/adaptive_feedback.py`.
- Public class boundary: `AdaptiveFeedbackEngine`.
...
|---|---|---|
| `AdaptiveFeedbackEngine.run(payload: dict) -> dict` | Executes the `AdaptiveFeedbackEngine.run` responsibility for AFE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/adaptive_feedback.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-099-example",
  "spec_id": "SPEC-099",
  ...
  "payload": {
    "engine": "Adaptive Feedback Engine",
    "source_class": "AdaptiveFeedbackEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_AFE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_AFE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_AFE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/adaptive_feedback.py` implementation is primarily in-memory. The enterprise contract still requires AFE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. AFE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `AFE_STARTED` with request ID, spec ID, source path, and upstream references.
- `AFE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `AFE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- AFE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
AFE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. AFE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `AdaptiveFeedbackEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-098 LAE` | Upstream | Must provide validated evidence before AFE starts. |
| `user feedback` | Upstream | Must provide validated evidence before AFE starts. |
| `production outcomes` | Upstream | Must provide validated evidence before AFE starts. |
| `SPEC-097 CLE` | Downstream | Must consume only the validated AFE output contract. |
| `SPEC-100 LSO` | Downstream | Must consume only the validated AFE output contract. |
| `src/learning/adaptive_feedback.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `AdaptiveFeedbackEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: AFE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/adaptive_feedback.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-099-AFE.md
```

Guidance:
- Keep `AdaptiveFeedbackEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_afe(payload: dict) -> dict:
    input_contract = validate_afe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = AdaptiveFeedbackEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_afe_output(input_contract, result)
    validate_afe_output(output)
...
- `AdaptiveFeedbackEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/adaptive_feedback.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-098 LAE"] --> I["Input Validator"]
    I --> E["SPEC-099 AFE: Adaptive Feedback Engine"]
...
graph LR
    ...
    V --> Core["AdaptiveFeedbackEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["AFE Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-099"]
    This --> Source["src/learning/adaptive_feedback.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["SPEC-097 CLE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute AFE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-098 LAE" as U
rectangle "Input Validator" as V
rectangle "SPEC-099 AFE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-097 CLE" as D
U --> V
V --> E
E --> K
K --> D
...
class "AFEInputContract"
class "AFEOutputContract"
AdaptiveFeedbackEngine --> "AFEInputContract"
AdaptiveFeedbackEngine --> "AFEOutputContract"
@enduml
```
