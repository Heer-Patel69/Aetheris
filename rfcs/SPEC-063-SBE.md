# SPEC-063: Skill Benchmark Engine (SBE)
...
Implementation: `src/intelligence/sbe.py` (validated from source metadata)
Primary Class: `SkillBenchmarkEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Skill Benchmark Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Benchmarks skill quality, reliability, latency, task fit, and production suitability for future routing decisions.

This document upgrades SPEC-063 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-063 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of SBE is to own the full engineering responsibility for wave 4 - evolution. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding SBE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-034 SSE, execution metrics, skill history.
- Emit structured output that can be consumed by SPEC-062 DSEE, SPEC-064 MMCE.
...
In scope:
- Wave 4 - Evolution
- Validation of all SBE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-063` input contract to `SPEC-063` output contract.
- Use `src/intelligence/sbe.py` as the implementation boundary and `SkillBenchmarkEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
SBE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- SBE validates and transforms that evidence.
...
======================================================================
- FR-001: SBE shall load the upstream artifacts declared by SPEC-034 SSE, execution metrics, skill history.
- FR-002: SBE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: SBE shall execute the primary behavior owned by `SkillBenchmarkEngine`.
- FR-004: SBE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-063 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat SBE output as the authoritative contract for wave 4 - evolution unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/sbe.py`.

Primary implementation class: `SkillBenchmarkEngine`.

Implementation classes in scope:
- `SkillBenchmarkEngine`
...
======================================================================
SBE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-063` owns how the engine is implemented.
...
SBE must exchange data by contract, not by implicit global state.
...
4. Execute `SkillBenchmarkEngine` behavior for wave 4 - evolution.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-034 SSE, execution metrics, skill history.
...
- `SkillBenchmarkEngine`: Owns the core wave 4 - evolution behavior.
...
- Benchmarks skill quality, reliability, latency, task fit, and production suitability for future routing decisions.
...
- Input interface: `SBEInput` JSON object.
- Output interface: `SBEOutput` JSON object.
- Source interface: `src/intelligence/sbe.py`.
- Public class boundary: `SkillBenchmarkEngine`.
...
|---|---|---|
| `benchmark_skill(skill_name: str) -> dict` | Executes the `benchmark_skill` responsibility for SBE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-063-example",
  "spec_id": "SPEC-063",
  ...
  "payload": {
    "engine": "Skill Benchmark Engine",
    "source_class": "SkillBenchmarkEngine",
    "public_methods": [
      "benchmark_skill(skill_name: str) -> dict"
    ]
  }
}
```
...
- `AETHERIS_SBE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_SBE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_SBE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/intelligence/sbe.py` implementation is primarily in-memory. The enterprise contract still requires SBE to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. SBE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `SBE_STARTED` with request ID, spec ID, source path, and upstream references.
- `SBE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `SBE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- SBE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
SBE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. SBE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `SkillBenchmarkEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-034 SSE` | Upstream | Must provide validated evidence before SBE starts. |
| `execution metrics` | Upstream | Must provide validated evidence before SBE starts. |
| `skill history` | Upstream | Must provide validated evidence before SBE starts. |
| `SPEC-062 DSEE` | Downstream | Must consume only the validated SBE output contract. |
| `SPEC-064 MMCE` | Downstream | Must consume only the validated SBE output contract. |
| `src/intelligence/sbe.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `SkillBenchmarkEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: SBE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/intelligence/sbe.py
tests/test_rfc004_core.py
.aetheris/<engine-owned-output>/
rfcs/SPEC-063-SBE.md
```

Guidance:
- Keep `SkillBenchmarkEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_sbe(payload: dict) -> dict:
    input_contract = validate_sbe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = SkillBenchmarkEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_sbe_output(input_contract, result)
    validate_sbe_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/sbe.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-063 SBE: Skill Benchmark Engine"]
...
graph LR
    ...
    V --> Core["SkillBenchmarkEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["SBE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-063"]
    This --> Source["src/intelligence/sbe.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute SBE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
class "SBEInputContract"
class "SBEOutputContract"
SkillBenchmarkEngine --> "SBEInputContract"
SkillBenchmarkEngine --> "SBEOutputContract"
@enduml
```
