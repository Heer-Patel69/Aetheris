# SPEC-110: SAML & SSO Integration Adapter (SSOA)
...
Primary Class: `SAMLSSOAdapter`
Test Reference: `tests/test_rfc007_core.py`

======================================================================
1. PURPOSE & BUSINESS VALUE
======================================================================
The SAML & SSO Integration Adapter (SSOA) exists to provide a foundational enterprise capability for Aetheris. It ensures secure, production-scale, multi-tenant operations, maintaining compliance, security boundaries, and strict resource isolation.
...
======================================================================
- FR-101: The engine shall load configurations conforming to the `SPEC-110Input` schema.
- FR-102: The engine shall authorize all operations against tenant context parameters.
- FR-103: The engine shall execute the core `SAMLSSOAdapter` behaviors.
- FR-104: The engine shall output structured results conforming to the `SPEC-110Output` schema.
...
======================================================================
Primary Path: `src/enterprise/sso.py`.
The class `SAMLSSOAdapter` acts as the main domain service layer, validating API structures, interacting with local storage adapters, and dispatching transaction logs to the audit service.
...
- `SAMLSSOAdapterCore`: Executes subsystem transformations.
...
3. Call `SAMLSSOAdapter` core behaviors.
...
======================================================================
Primary logic is defined in `src/enterprise/sso.py`. Sub-modules handle encryption, parsing, and database schemas.
...
|---|---|---|
| `process_saml_response(saml_payload: str) -> dict` | Validates the SAML assertion, returning active user claims and session. | Validate input, enforce security boundaries, return deterministic output, and emit telemetry. |
...
18. OBSERVABILITY
======================================================================
Structured logging includes standard event codes like `SSOA_STARTED`, `SSOA_SUCCESS`, and `SSOA_FAILED` with request ID attributes.
