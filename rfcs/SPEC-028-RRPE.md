# SPEC-028: Release Rollout Planning Engine (RRPE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-002
Layer: Engineering Planning System
Scope: Release strategy, canary rollout, and rollback planning
...
Primary Class: `ReleaseRolloutPlanningEngine`
Test Reference: `tests/test_planners.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Release Rollout Planning Engine exists to make Engineering Planning System deterministic, reviewable, and safe at production scale. Plans release strategy, canary steps, promotion gates, rollback triggers, health checks, and deployment safety thresholds.

This document upgrades SPEC-028 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-028 is part of the Engineering Planning System, where product, architecture, delivery, operational, and governance plans are compiled before execution.

======================================================================
2. PURPOSE
======================================================================
The purpose of RRPE is to own the full engineering responsibility for release strategy, canary rollout, and rollback planning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding RRPE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct release strategy, canary rollout, and rollback planning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for release strategy, canary rollout, and rollback planning.
- Accept only validated upstream artifacts from SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-027.
- Emit structured output that can be consumed by SPEC-029.
...
======================================================================
In scope:
- Release strategy, canary rollout, and rollback planning
- Validation of all RRPE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-028` input contract to `SPEC-028` output contract.
- Use `src/intelligence/planners.py` as the implementation boundary and `ReleaseRolloutPlanningEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
RRPE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- RRPE validates and transforms that evidence.
...
======================================================================
- FR-001: RRPE shall load the upstream artifacts declared by SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-027.
- FR-002: RRPE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: RRPE shall execute the primary behavior owned by `ReleaseRolloutPlanningEngine`.
- FR-004: RRPE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-028 sits in `Engineering Planning System` under `RFC-002`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat RRPE output as the authoritative contract for release strategy, canary rollout, and rollback planning unless a later validation SPEC rejects it.
...
Primary implementation class: `ReleaseRolloutPlanningEngine`.

Implementation classes in scope:
- `ReleaseRolloutPlanningEngine`
...
======================================================================
RRPE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-028` owns how the engine is implemented.
...
RRPE must exchange data by contract, not by implicit global state.
...
4. Execute `ReleaseRolloutPlanningEngine` behavior for release strategy, canary rollout, and rollback planning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-003 PDE, SPEC-004 APE, SPEC-007 EKB, SPEC-027.
...
- `ReleaseRolloutPlanningEngine`: Owns the core release strategy, canary rollout, and rollback planning behavior.
...
- Input interface: `RRPEInput` JSON object.
- Output interface: `RRPEOutput` JSON object.
- Source interface: `src/intelligence/planners.py`.
- Public class boundary: `ReleaseRolloutPlanningEngine`.
...
|---|---|---|
| `plan(product_plan: dict) -> dict` | Executes the `plan` responsibility for RRPE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-028-example",
  "spec_id": "SPEC-028",
  ...
  "payload": {
    "engine": "Release Rollout Planning Engine",
    "source_class": "ReleaseRolloutPlanningEngine",
    "public_methods": [
...
- `AETHERIS_RRPE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_RRPE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_RRPE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. RRPE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `RRPE_STARTED` with request ID, spec ID, source path, and upstream references.
- `RRPE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `RRPE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- RRPE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
RRPE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. RRPE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `ReleaseRolloutPlanningEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-003 PDE` | Upstream | Must provide validated evidence before RRPE starts. |
| `SPEC-004 APE` | Upstream | Must provide validated evidence before RRPE starts. |
| `SPEC-007 EKB` | Upstream | Must provide validated evidence before RRPE starts. |
| `SPEC-027` | Upstream | Must provide validated evidence before RRPE starts. |
| `SPEC-029` | Downstream | Must consume only the validated RRPE output contract. |
...
| FR-003 core execution | Direct `ReleaseRolloutPlanningEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: RRPE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-028-RRPE.md
```

Guidance:
- Keep `ReleaseRolloutPlanningEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_rrpe(payload: dict) -> dict:
    input_contract = validate_rrpe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = ReleaseRolloutPlanningEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_rrpe_output(input_contract, result)
    validate_rrpe_output(output)
...
graph TD
    ...
    I --> E["SPEC-028 RRPE: Release Rollout Planning Engine"]
...
graph LR
    ...
    V --> Core["ReleaseRolloutPlanningEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["RRPE Payload"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute RRPE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-028 RRPE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-029" as D
U --> V
V --> E
E --> K
K --> D
...
class "RRPEInputContract"
class "RRPEOutputContract"
ReleaseRolloutPlanningEngine --> "RRPEInputContract"
ReleaseRolloutPlanningEngine --> "RRPEOutputContract"
@enduml
```
