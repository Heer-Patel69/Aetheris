# SPEC-074: Node Discovery Cluster Registry (NDCR)
...
Implementation: `src/runtime/node_registry.py` (source implementation pending)
Primary Class: `NodeDiscoveryClusterRegistry`
Test Reference: `tests/plans/runtime-infrastructure-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Node Discovery Cluster Registry exists to make Runtime Infrastructure Layer deterministic, reviewable, and safe at production scale. Discovers runtime nodes, maintains registry metadata, validates node identity, and publishes cluster topology changes.

This document upgrades SPEC-074 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-074 is part of the Runtime Infrastructure Layer, where plugins, workers, clusters, queues, secrets, logs, state, identity, upgrades, chaos testing, and distributed execution are coordinated under production safety boundaries.

======================================================================
2. PURPOSE
======================================================================
The purpose of NDCR is to own the full engineering responsibility for wave 5 - runtime infrastructure. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding NDCR behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from runtime nodes, cryptographic identities.
- Emit structured output that can be consumed by SPEC-073 CLM, SPEC-082 CITE.
...
- Validation of all NDCR input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-074` input contract to `SPEC-074` output contract.
- Use `src/runtime/node_registry.py` as the implementation boundary and `NodeDiscoveryClusterRegistry` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
NDCR follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- NDCR validates and transforms that evidence.
...
======================================================================
- FR-001: NDCR shall load the upstream artifacts declared by runtime nodes, cryptographic identities.
- FR-002: NDCR shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: NDCR shall execute the primary behavior owned by `NodeDiscoveryClusterRegistry`.
- FR-004: NDCR shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-074 sits in `Runtime Infrastructure Layer` under `RFC-005`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat NDCR output as the authoritative contract for wave 5 - runtime infrastructure unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/runtime/node_registry.py`.

Primary implementation class: `NodeDiscoveryClusterRegistry`.

Implementation classes in scope:
- `NodeDiscoveryClusterRegistry`
...
======================================================================
NDCR is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-074` owns how the engine is implemented.
...
NDCR must exchange data by contract, not by implicit global state.
...
4. Execute `NodeDiscoveryClusterRegistry` behavior for wave 5 - runtime infrastructure.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: runtime nodes, cryptographic identities.
...
- `NodeDiscoveryClusterRegistry`: Owns the core wave 5 - runtime infrastructure behavior.
...
- Discovers runtime nodes, maintains registry metadata, validates node identity, and publishes cluster topology changes.
...
- Input interface: `NDCRInput` JSON object.
- Output interface: `NDCROutput` JSON object.
- Source interface: `src/runtime/node_registry.py`.
- Public class boundary: `NodeDiscoveryClusterRegistry`.
...
|---|---|---|
| `NodeDiscoveryClusterRegistry.run(payload: dict) -> dict` | Executes the `NodeDiscoveryClusterRegistry.run` responsibility for NDCR. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/runtime/node_registry.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-074-example",
  "spec_id": "SPEC-074",
  ...
  "payload": {
    "engine": "Node Discovery Cluster Registry",
    "source_class": "NodeDiscoveryClusterRegistry",
    "public_methods": []
  }
}
```
...
- `AETHERIS_NDCR_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_NDCR_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_NDCR_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/runtime/node_registry.py` implementation is primarily in-memory. The enterprise contract still requires NDCR to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. NDCR may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `NDCR_STARTED` with request ID, spec ID, source path, and upstream references.
- `NDCR_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `NDCR_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- NDCR must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
NDCR is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. NDCR should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `NodeDiscoveryClusterRegistry` directly and verify public method behavior.
...
|---|---|---|
| `runtime nodes` | Upstream | Must provide validated evidence before NDCR starts. |
| `cryptographic identities` | Upstream | Must provide validated evidence before NDCR starts. |
| `SPEC-073 CLM` | Downstream | Must consume only the validated NDCR output contract. |
| `SPEC-082 CITE` | Downstream | Must consume only the validated NDCR output contract. |
| `src/runtime/node_registry.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `NodeDiscoveryClusterRegistry` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: NDCR grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/runtime/node_registry.py
tests/runtime-infrastructure-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-074-Node-Discovery-Cluster-Registry.md
```

Guidance:
- Keep `NodeDiscoveryClusterRegistry` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_ndcr(payload: dict) -> dict:
    input_contract = validate_ndcr_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = NodeDiscoveryClusterRegistry(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_ndcr_output(input_contract, result)
    validate_ndcr_output(output)
...
- `NodeDiscoveryClusterRegistry.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/runtime/node_registry.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["runtime nodes"] --> I["Input Validator"]
    I --> E["SPEC-074 NDCR: Node Discovery Cluster Registry"]
...
graph LR
    ...
    V --> Core["NodeDiscoveryClusterRegistry Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["NDCR Payload"]
...
```mermaid
graph TD
    Parent["RFC-005"] --> This["SPEC-074"]
    This --> Source["src/runtime/node_registry.py"]
    This --> Test["tests/plans/runtime-infrastructure-test-plan.md"]
    This --> Next["SPEC-073 CLM"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute NDCR"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "runtime nodes" as U
rectangle "Input Validator" as V
rectangle "SPEC-074 NDCR" as E
database "EKB / Artifacts" as K
rectangle "SPEC-073 CLM" as D
U --> V
V --> E
E --> K
K --> D
...
```plantuml
@startuml
class NodeDiscoveryClusterRegistry {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "NDCRInputContract"
class "NDCROutputContract"
NodeDiscoveryClusterRegistry --> "NDCRInputContract"
NodeDiscoveryClusterRegistry --> "NDCROutputContract"
@enduml
```
