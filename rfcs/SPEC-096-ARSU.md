# SPEC-096: Automatic RFC SPEC Update Engine (ARSU)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-006
Layer: Learning System
Scope: Controlled documentation update recommendations from validated learning
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/learning/automatic_rfc_spec_update.py` (source implementation pending)
Primary Class: `AutomaticRFCSpecUpdateEngine`
Test Reference: `tests/plans/learning-system-test-plan.md`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Automatic RFC SPEC Update Engine exists to make Learning System deterministic, reviewable, and safe at production scale. Converts validated learning into proposed RFC and SPEC updates while preserving governance, traceability, approval, and human-review boundaries.

This document upgrades SPEC-096 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-096 is part of the Learning System, where execution experience is captured, validated, mined, ranked, and converted into safe recommendations that improve future planning, intelligence, execution, recovery, and governance.

======================================================================
2. PURPOSE
======================================================================
The purpose of ARSU is to own the full engineering responsibility for controlled documentation update recommendations from validated learning. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding ARSU behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct controlled documentation update recommendations from validated learning was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for controlled documentation update recommendations from validated learning.
- Accept only validated upstream artifacts from SPEC-088 BPE2, SPEC-093 ALE, SPEC-094 TLE, SPEC-095 RLE.
- Emit structured output that can be consumed by RFC-006 governance workflow, ARB review.
...
======================================================================
In scope:
- Controlled documentation update recommendations from validated learning
- Validation of all ARSU input and output structures.
...
- Use `src/learning/automatic_rfc_spec_update.py` as the implementation boundary and `AutomaticRFCSpecUpdateEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
ARSU follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- ARSU validates and transforms that evidence.
...
======================================================================
- FR-001: ARSU shall load the upstream artifacts declared by SPEC-088 BPE2, SPEC-093 ALE, SPEC-094 TLE, SPEC-095 RLE.
- FR-002: ARSU shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: ARSU shall execute the primary behavior owned by `AutomaticRFCSpecUpdateEngine`.
- FR-004: ARSU shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
======================================================================
- Latency target: complete one learning pass in less than 5 seconds for 10,000 historical events, excluding optional vector index rebuilds.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-096 sits in `Learning System` under `RFC-006`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
- RFC-006 governance workflow
- ARB review

The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat ARSU output as the authoritative contract for controlled documentation update recommendations from validated learning unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/learning/automatic_rfc_spec_update.py`.

Primary implementation class: `AutomaticRFCSpecUpdateEngine`.

Implementation classes in scope:
- `AutomaticRFCSpecUpdateEngine`
...
======================================================================
ARSU is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `RFC-006` owns why this engine exists.
- `SPEC-096` owns how the engine is implemented.
...
ARSU must exchange data by contract, not by implicit global state.
...
4. Execute `AutomaticRFCSpecUpdateEngine` behavior for controlled documentation update recommendations from validated learning.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-088 BPE2, SPEC-093 ALE, SPEC-094 TLE, SPEC-095 RLE.
...
4. Run the ARSU core transformation.
5. Generate artifacts and EKB records.
6. Validate output against the JSON schema in this SPEC.
7. Release the output to RFC-006 governance workflow, ARB review.
...
- `AutomaticRFCSpecUpdateEngine`: Owns the core controlled documentation update recommendations from validated learning behavior.
...
- Converts validated learning into proposed RFC and SPEC updates while preserving governance, traceability, approval, and human-review boundaries.
...
- Input interface: `ARSUInput` JSON object.
- Output interface: `ARSUOutput` JSON object.
- Source interface: `src/learning/automatic_rfc_spec_update.py`.
- Public class boundary: `AutomaticRFCSpecUpdateEngine`.
...
|---|---|---|
| `AutomaticRFCSpecUpdateEngine.run(payload: dict) -> dict` | Executes the `AutomaticRFCSpecUpdateEngine.run` responsibility for ARSU. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
  ...
  "properties": {
    ...
    "telemetry": {
      ...
      "properties": {
        ...
        "source_path": {
          "const": "src/learning/automatic_rfc_spec_update.py"
        }
      }
    }
  }
...
```json
{
  "request_id": "req-spec-096-example",
  "spec_id": "SPEC-096",
  ...
  "payload": {
    "engine": "Automatic RFC SPEC Update Engine",
    "source_class": "AutomaticRFCSpecUpdateEngine",
    "public_methods": []
  }
}
```
...
- `AETHERIS_ARSU_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_ARSU_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_ARSU_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
The current `src/learning/automatic_rfc_spec_update.py` implementation is primarily in-memory. The enterprise contract still requires ARSU to expose generated artifacts through the common result envelope and to persist telemetry through the EKB when promoted to production.
...
======================================================================
Recovery is conservative. ARSU may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `ARSU_STARTED` with request ID, spec ID, source path, and upstream references.
- `ARSU_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `ARSU_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
- Primary latency target: complete one learning pass in less than 5 seconds for 10,000 historical events, excluding optional vector index rebuilds.
...
======================================================================
Scalability model:
- ARSU must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
ARSU is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. ARSU should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
37. TESTING STRATEGY
======================================================================
Primary test reference: `tests/plans/learning-system-test-plan.md`.
...
======================================================================
- Unit tests should instantiate `AutomaticRFCSpecUpdateEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-088 BPE2` | Upstream | Must provide validated evidence before ARSU starts. |
| `SPEC-093 ALE` | Upstream | Must provide validated evidence before ARSU starts. |
| `SPEC-094 TLE` | Upstream | Must provide validated evidence before ARSU starts. |
| `SPEC-095 RLE` | Upstream | Must provide validated evidence before ARSU starts. |
| `RFC-006 governance workflow` | Downstream | Must consume only the validated ARSU output contract. |
| `ARB review` | Downstream | Must consume only the validated ARSU output contract. |
| `src/learning/automatic_rfc_spec_update.py` | Implementation | Must remain aligned with this SPEC. |
| `tests/plans/learning-system-test-plan.md` | Verification | Must cover the behavior and artifact contract. |
...
|---|---|
| FR-001 upstream loading | `tests/plans/learning-system-test-plan.md` fixtures and upstream artifact validation |
| FR-002 input validation | Schema tests and malformed payload cases |
| FR-003 core execution | Direct `AutomaticRFCSpecUpdateEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: ARSU grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
```text
src/learning/automatic_rfc_spec_update.py
tests/learning-system-test-plan.md
.aetheris/<engine-owned-output>/
rfcs/SPEC-096-ARSU.md
```

Guidance:
- Keep `AutomaticRFCSpecUpdateEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_arsu(payload: dict) -> dict:
    input_contract = validate_arsu_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = AutomaticRFCSpecUpdateEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_arsu_output(input_contract, result)
    validate_arsu_output(output)
...
- `AutomaticRFCSpecUpdateEngine.execute(payload: dict) -> dict`
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/learning/automatic_rfc_spec_update.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-088 BPE2"] --> I["Input Validator"]
    I --> E["SPEC-096 ARSU: Automatic RFC SPEC Update Engine"]
    E --> C["Contract Validator"]
    C --> A["Owned Artifacts and EKB Objects"]
    A --> D["RFC-006 governance workflow"]
```
...
graph LR
    ...
    V --> Core["AutomaticRFCSpecUpdateEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["ARSU Payload"]
...
```mermaid
graph TD
    Parent["RFC-006"] --> This["SPEC-096"]
    This --> Source["src/learning/automatic_rfc_spec_update.py"]
    This --> Test["tests/plans/learning-system-test-plan.md"]
    This --> Next["RFC-006 governance workflow"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute ARSU"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-088 BPE2" as U
rectangle "Input Validator" as V
rectangle "SPEC-096 ARSU" as E
database "EKB / Artifacts" as K
rectangle "RFC-006 governance workflow" as D
U --> V
V --> E
E --> K
K --> D
...
```plantuml
@startuml
class AutomaticRFCSpecUpdateEngine {
  +execute(input)
  +validate_output(output)
  +emit_telemetry(result)
}
class "ARSUInputContract"
class "ARSUOutputContract"
AutomaticRFCSpecUpdateEngine --> "ARSUInputContract"
AutomaticRFCSpecUpdateEngine --> "ARSUOutputContract"
@enduml
```
...
- `rfcs/RFC-006`
- `src/learning/automatic_rfc_spec_update.py`
- `tests/plans/learning-system-test-plan.md`
