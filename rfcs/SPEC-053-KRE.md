# SPEC-053: Knowledge Retrieval Engine (KRE)
...
Implementation: `src/intelligence/kre.py` (validated from source metadata)
Primary Class: `KnowledgeRetrievalEngine`
Test Reference: `tests/test_rfc004_core.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Knowledge Retrieval Engine exists to make Engineering Intelligence Layer deterministic, reviewable, and safe at production scale. Retrieves relevant engineering evidence from RFCs, SPECs, code, tests, documentation, memories, and constitution-level constraints.

This document upgrades SPEC-053 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-053 is part of the Engineering Intelligence Layer, where prompts, evidence, model choices, reasoning, verification, optimization, and skill evolution are coordinated.

======================================================================
2. PURPOSE
======================================================================
The purpose of KRE is to own the full engineering responsibility for wave 2 - knowledge intelligence. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding KRE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
...
- Accept only validated upstream artifacts from SPEC-052 LCE, RFCs, SPECs, source code, tests.
- Emit structured output that can be consumed by SPEC-047 MIE, SPEC-055 FVE.
...
- Validation of all KRE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-053` input contract to `SPEC-053` output contract.
- Use `src/intelligence/kre.py` as the implementation boundary and `KnowledgeRetrievalEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
KRE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- KRE validates and transforms that evidence.
...
======================================================================
- FR-001: KRE shall load the upstream artifacts declared by SPEC-052 LCE, RFCs, SPECs, source code, tests.
- FR-002: KRE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: KRE shall execute the primary behavior owned by `KnowledgeRetrievalEngine`.
- FR-004: KRE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-053 sits in `Engineering Intelligence Layer` under `RFC-004`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat KRE output as the authoritative contract for wave 2 - knowledge intelligence unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/intelligence/kre.py`.

Primary implementation class: `KnowledgeRetrievalEngine`.
...
======================================================================
KRE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-053` owns how the engine is implemented.
...
KRE must exchange data by contract, not by implicit global state.
...
4. Execute `KnowledgeRetrievalEngine` behavior for wave 2 - knowledge intelligence.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-052 LCE, RFCs, SPECs, source code, tests.
...
- `KnowledgeRetrievalEngine`: Owns the core wave 2 - knowledge intelligence behavior.
...
- Retrieves relevant engineering evidence from RFCs, SPECs, code, tests, documentation, memories, and constitution-level constraints.
...
- Input interface: `KREInput` JSON object.
- Output interface: `KREOutput` JSON object.
- Source interface: `src/intelligence/kre.py`.
- Public class boundary: `KnowledgeRetrievalEngine`.
...
|---|---|---|
| `retrieve_evidence(query: str) -> list` | Executes the `retrieve_evidence` responsibility for KRE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-053-example",
  "spec_id": "SPEC-053",
  ...
  "payload": {
    ...
    "source_class": "KnowledgeRetrievalEngine",
    "public_methods": [
      "retrieve_evidence(query: str) -> list"
    ]
  }
}
```
...
- `AETHERIS_KRE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_KRE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_KRE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. KRE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `KRE_STARTED` with request ID, spec ID, source path, and upstream references.
- `KRE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `KRE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- KRE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
KRE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. KRE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `KnowledgeRetrievalEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-052 LCE` | Upstream | Must provide validated evidence before KRE starts. |
| `RFCs` | Upstream | Must provide validated evidence before KRE starts. |
| `SPECs` | Upstream | Must provide validated evidence before KRE starts. |
| `source code` | Upstream | Must provide validated evidence before KRE starts. |
| `tests` | Upstream | Must provide validated evidence before KRE starts. |
| `SPEC-047 MIE` | Downstream | Must consume only the validated KRE output contract. |
| `SPEC-055 FVE` | Downstream | Must consume only the validated KRE output contract. |
| `src/intelligence/kre.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `KnowledgeRetrievalEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: KRE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-053-KRE.md
```

Guidance:
- Keep `KnowledgeRetrievalEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_kre(payload: dict) -> dict:
    input_contract = validate_kre_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = KnowledgeRetrievalEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_kre_output(input_contract, result)
    validate_kre_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/intelligence/kre.py",
    "duration_ms": 12.5
  }
}
```
...
graph TD
    ...
    I --> E["SPEC-053 KRE: Knowledge Retrieval Engine"]
...
graph LR
    ...
    V --> Core["KnowledgeRetrievalEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["KRE Payload"]
...
```mermaid
graph TD
    Parent["RFC-004"] --> This["SPEC-053"]
    This --> Source["src/intelligence/kre.py"]
    This --> Test["tests/test_rfc004_core.py"]
    This --> Next["SPEC-047 MIE"]
```

Execution pipeline:
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute KRE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
rectangle "SPEC-053 KRE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-047 MIE" as D
U --> V
V --> E
E --> K
K --> D
...
class "KREInputContract"
class "KREOutputContract"
KnowledgeRetrievalEngine --> "KREInputContract"
KnowledgeRetrievalEngine --> "KREOutputContract"
@enduml
```
