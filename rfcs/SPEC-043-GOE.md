# SPEC-043: Git Operations Engine (GOE)

Status: Enterprise Standard Draft
Version: 2.0.0
Parent RFC: RFC-003
Layer: Autonomous Engineering Engine
Scope: Git branch, commit, tag, rollback, and repository health operations
Canonical Standard: SPEC-047 Enterprise Standard
Upgrade Date: 2026-07-01
Implementation: `src/execution/goe.py` (validated from source metadata)
Primary Class: `GitOperationsEngine`
Test Reference: `tests/test_execution.py`

======================================================================
1. EXECUTIVE SUMMARY
======================================================================
Git Operations Engine exists to make Autonomous Engineering Engine deterministic, reviewable, and safe at production scale. Generates commit plans, branches, tags, merge controls, rollback controls, repository health reports, and traceable commit messages.

This document upgrades SPEC-043 from a lightweight subsystem note into an enterprise engineering specification. It defines why the engine exists, how it operates internally, which contracts it owns, how it interacts with neighboring engines, how it fails, how it recovers, how engineers should test it, and how future changes should preserve compatibility with the Aetheris constitution.

SPEC-043 is part of the Autonomous Engineering Engine, where a validated blueprint is decomposed, scheduled, executed, recovered, reviewed, and persisted.

======================================================================
2. PURPOSE
======================================================================
The purpose of GOE is to own the full engineering responsibility for git branch, commit, tag, rollback, and repository health operations. It converts upstream evidence into a downstream-safe contract without asking later engines to infer missing details.
...
- Embedding GOE behavior into a general orchestrator was rejected because it hides ownership and makes failures hard to isolate.
- Letting downstream engines reconstruct git branch, commit, tag, rollback, and repository health operations was rejected because it duplicates logic and creates inconsistent plans.
...
3. GOALS
======================================================================
- Provide a deterministic implementation contract for git branch, commit, tag, rollback, and repository health operations.
- Accept only validated upstream artifacts from SPEC-042.
- Emit structured output that can be consumed by SPEC-044.
...
======================================================================
In scope:
- Git branch, commit, tag, rollback, and repository health operations
- Validation of all GOE input and output structures.
...
5. RESPONSIBILITIES
======================================================================
- Own the transformation from `SPEC-043` input contract to `SPEC-043` output contract.
- Use `src/execution/goe.py` as the implementation boundary and `GitOperationsEngine` as the primary class boundary.
...
6. DESIGN PHILOSOPHY
======================================================================
GOE follows the Aetheris principle: think more, generate less, reuse whenever possible. The engine should perform the minimum deterministic transformation that completely satisfies its contract and should reuse upstream artifacts instead of rescanning or re-deriving evidence.

The design is intentionally layered:
- Upstream engines provide evidence.
- GOE validates and transforms that evidence.
...
======================================================================
- FR-001: GOE shall load the upstream artifacts declared by SPEC-042.
- FR-002: GOE shall validate `request_id`, `workspace_path`, `spec_id`, and payload shape before execution.
- FR-003: GOE shall execute the primary behavior owned by `GitOperationsEngine`.
- FR-004: GOE shall emit an output contract containing status, artifacts, EKB object identifiers, warnings, and telemetry.
...
10. SYSTEM CONTEXT
======================================================================
SPEC-043 sits in `Autonomous Engineering Engine` under `RFC-003`. It is not an isolated utility; it is one stage in the Aetheris operating-system pipeline.
...
The engine must treat upstream data as evidence, not truth. It must validate structure and consistency before use. Downstream engines must treat GOE output as the authoritative contract for git branch, commit, tag, rollback, and repository health operations unless a later validation SPEC rejects it.

======================================================================
11. INTERNAL ARCHITECTURE
======================================================================
Primary implementation path: `src/execution/goe.py`.

Primary implementation class: `GitOperationsEngine`.
...
======================================================================
GOE is invoked by the Aetheris kernel, an orchestrator, or a parent pipeline runner. It should not require callers to know implementation internals.
...
- `SPEC-043` owns how the engine is implemented.
...
GOE must exchange data by contract, not by implicit global state.
...
4. Execute `GitOperationsEngine` behavior for git branch, commit, tag, rollback, and repository health operations.
...
======================================================================
Canonical sequence:
1. Load upstream dependencies: SPEC-042.
...
- `GitOperationsEngine`: Owns the core git branch, commit, tag, rollback, and repository health operations behavior.
...
- Input interface: `GOEInput` JSON object.
- Output interface: `GOEOutput` JSON object.
- Source interface: `src/execution/goe.py`.
- Public class boundary: `GitOperationsEngine`.
...
|---|---|---|
| `commit(task_id: str, message: str, files: List[str]) -> dict` | Executes the `commit` responsibility for GOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `create_branch(branch_name: str) -> bool` | Executes the `create_branch` responsibility for GOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `merge(source_branch: str, target_branch: str) -> bool` | Executes the `merge` responsibility for GOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `tag_release(tag_name: str) -> bool` | Executes the `tag_release` responsibility for GOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `repository_health() -> dict` | Executes the `repository_health` responsibility for GOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `generate_commit_message(task_id: str, action: str) -> str` | Executes the `generate_commit_message` responsibility for GOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
| `rollback(commit_hash: str) -> bool` | Executes the `rollback` responsibility for GOE. | Validate input, avoid hidden side effects outside owned artifact paths, return deterministic structured output, and emit traceable failures. |
...
```json
{
  "request_id": "req-spec-043-example",
  "spec_id": "SPEC-043",
  ...
  "payload": {
    "engine": "Git Operations Engine",
    "source_class": "GitOperationsEngine",
    "public_methods": [
      "commit(task_id: str, message: str, files: List[str]) -> dict",
      "create_branch(branch_name: str) -> bool",
      "merge(source_branch: str, target_branch: str) -> bool",
      "tag_release(tag_name: str) -> bool",
...
- `AETHERIS_GOE_ENABLED`: Optional feature gate for this engine.
- `AETHERIS_GOE_STRICT`: When true, converts warnings to failures for CI or release validation.
- `AETHERIS_GOE_OUTPUT_DIR`: Optional override for generated artifacts during tests.
...
======================================================================
Recovery is conservative. GOE may retry idempotent reads, regenerate deterministic artifacts, or fall back to a documented safe default. It must not fabricate architecture, code, security, cost, or evidence decisions to hide missing data.
...
======================================================================
Required telemetry:
- `GOE_STARTED` with request ID, spec ID, source path, and upstream references.
- `GOE_COMPLETED` with status, duration, artifact count, EKB object count, and warning count.
- `GOE_FAILED` with failure class, retryability, and recovery guidance.
...
======================================================================
Scalability model:
- GOE must scale by consuming scoped upstream artifacts rather than global mutable state.
...
======================================================================
GOE is inside the Aetheris trust boundary but must still defend against unsafe project content, prompt injection embedded in source files, path traversal, malformed schemas, and poisoned upstream artifacts.
...
36. ERROR HANDLING
======================================================================
Errors must be explicit and typed. GOE should prefer structured failure returns over unhandled exceptions once an invocation context exists.
...
======================================================================
- Unit tests should instantiate `GitOperationsEngine` directly and verify public method behavior.
...
|---|---|---|
| `SPEC-042` | Upstream | Must provide validated evidence before GOE starts. |
| `SPEC-044` | Downstream | Must consume only the validated GOE output contract. |
| `src/execution/goe.py` | Implementation | Must remain aligned with this SPEC. |
...
| FR-003 core execution | Direct `GitOperationsEngine` public API tests |
...
41. RISK ASSESSMENT
======================================================================
- Risk: GOE grows beyond its single responsibility. Mitigation: reject cross-SPEC behavior and keep responsibilities explicit.
...
rfcs/SPEC-043-GOE.md
```

Guidance:
- Keep `GitOperationsEngine` as the primary orchestration class for this SPEC.
...
Example pseudocode:
```python
def run_goe(payload: dict) -> dict:
    input_contract = validate_goe_input(payload)
    upstream = load_upstream_artifacts(input_contract["upstream_artifacts"])
    result = GitOperationsEngine(workspace_path=input_contract["workspace_path"]).execute(upstream)
    output = build_goe_output(input_contract, result)
    validate_goe_output(output)
...
```json
{
  ...
  "warnings": [],
  "telemetry": {
    "source_path": "src/execution/goe.py",
    "duration_ms": 12.5
  }
}
```
...
```mermaid
graph TD
    U["SPEC-042"] --> I["Input Validator"]
    I --> E["SPEC-043 GOE: Git Operations Engine"]
...
graph LR
    ...
    V --> Core["GitOperationsEngine Core"]
...
```mermaid
flowchart LR
    Evidence["Validated Evidence"] --> Payload["GOE Payload"]
...
```mermaid
graph TD
    Parent["RFC-003"] --> This["SPEC-043"]
    This --> Source["src/execution/goe.py"]
...
```mermaid
graph LR
    A["Load Inputs"] --> B["Validate Contracts"] --> C["Execute GOE"] --> D["Persist Artifacts"] --> E["Emit Telemetry"] --> F["Release Downstream"]
```
...
```plantuml
@startuml
rectangle "SPEC-042" as U
rectangle "Input Validator" as V
rectangle "SPEC-043 GOE" as E
database "EKB / Artifacts" as K
rectangle "SPEC-044" as D
U --> V
V --> E
E --> K
K --> D
...
class "GOEInputContract"
class "GOEOutputContract"
GitOperationsEngine --> "GOEInputContract"
GitOperationsEngine --> "GOEOutputContract"
@enduml
```
