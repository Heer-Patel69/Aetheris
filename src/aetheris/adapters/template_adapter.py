...
class TemplateAdapter:
    """
    Adapter engine that scans third-party Claude Code templates,
    programmatically wraps and converts them into valid Aetheris Skills
    and RFC standards.
    """
    ...
    def __init__(self, workspace_path: str):
        ...
        # Locate the ECC-main templates directory, handling nesting
        ...
        # Define targets under Aetheris namespace
    ...
    def _convert_and_write_skill(self, src_path: Path, folder_name: str) -> None:
        """
        Parses YAML frontmatter of SKILL.md, restructures it to Aetheris standards,
        ...
        # Map to Aetheris standard keys, ensuring defaults exist
        name = frontmatter.get("name", folder_name.replace("-", " ").title())
        desc = frontmatter.get("description", "No description provided.")
        
        # Standardize multiline descriptions
        desc = " ".join(desc.split())

        # Construct Aetheris standard frontmatter
        ...
        # Format as standard Aetheris Skill markdown
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        sys.stderr.write("Usage: python template_adapter.py <workspace_root>\n")
        sys.exit(1)
    adapter = TemplateAdapter(sys.argv[1])
    res = adapter.sync()
    import json
