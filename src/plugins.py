...
from utils import is_safe_path, initialize_perimeter
...
class PluginManager:
    def __init__(self, workspace_path):
        ...
        self.global_plugin_registry = Path("~/.univoid/brain/plugins.yaml").expanduser()
    ...
    def load_registry(self):
        """
        Loads the list of registered plugins from plugins.yaml.
        """
    ...
    def register_plugin(self, plugin_dir_path):
        ...
        try:
            ...
            if plugin_major != master_major:
                return {
                    "status": "error",
                    "message": f"Plugin version incompatible. Plugin requires v{plugin_major}, Brain OS is v{master_major}"
                }
